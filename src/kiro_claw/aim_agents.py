...
# and finds kiroclaw.json, registering it as a project agent at project_path=$HOME.
# That creates a duplicate "kiroclaw" entry (global + project) with the same name but
# different project_path keys, which triggers the 409 ambiguity check and breaks the
# kiroclaw switch entirely until the user manually deletes the registry entry.
...
class AimAgent:
    ...
    source: str = "builtin"  # "aim" | "kiroclaw" | "builtin" | "project"
...
def auto_register_project(project_path: str) -> None:
    ...
    and create duplicate kiroclaw entries triggering 409 on every agent switch).
    """

...
from kiro_claw.config.loader import _subtract_overlay, config_local_path, config_path
    ...
    if action == "get":
        ...
        if file_path:
            ...
            if not key or value is None:
                print("Usage: kiroclaw config set <key> <value>", file=sys.stderr)
                print("       kiroclaw config set --local <key> <value>", file=sys.stderr)
                print("       kiroclaw config set --file <path.json>", file=sys.stderr)
                sys.exit(1)
            parsed = _parse_value(value)
            if use_local:
                top_key = key.split(".")[0]
                _known_sections = {f.name for f in dataclasses.fields(KiroClawConfig)}
        ...
        print("Usage: kiroclaw config {get,set,edit}", file=sys.stderr)
        sys.exit(1)

...
This package wires an asyncio unix-socket broker into KiroClaw as a
sidecar subprocess. When enabled, it:

1. Writes rewritten kiro agent JSON into an overlay directory
   (``~/.kiroclaw/mcp-gateway/agents/``) — never touches the user's
   ``~/.kiro/agents/`` files on disk.
2. Spawns ``python -m kiro_claw.mcp_gateway.gatewayd`` at KiroClaw
...
if TYPE_CHECKING:  # names for static type checkers only
    from kiro_claw.mcp_gateway.backend import Backend, send_initialize, spawn_backend
    from kiro_claw.mcp_gateway.gatewayd import run_gatewayd
    from kiro_claw.mcp_gateway.manager import GatewayManager, GatewaySpec
    from kiro_claw.mcp_gateway.pool import BackendPool, PoolKey
    from kiro_claw.mcp_gateway.rewriter import (
        UNPOOLABLE_SERVERS,
        default_socket_path,
        rewrite_agents,
    )
    from kiro_claw.mcp_gateway.stub import main as stub_main

"""Asyncio unix-socket server for the KiroClaw MCP gateway.

This module is the entry point for ``python -m
kiro_claw.mcp_gateway.gatewayd`` and for in-process use by
:class:`kiro_claw.mcp_gateway.manager.GatewayManager`.
...
from kiro_claw.mcp_gateway.breaker import CircuitBreaker
from kiro_claw.mcp_gateway.manager import _scrub_sensitive_env
from kiro_claw.mcp_gateway.pool import (
...
from kiro_claw.mcp_gateway.prewarm import (
    HotKeyStore,
...
_DEFAULT_SOCKET_SUBDIR = "kiroclaw"
...
def _default_cli_socket_path() -> Path:
    ...
    This is used ONLY when ``python -m kiro_claw.mcp_gateway.gatewayd`` is
    invoked without an explicit ``--socket`` flag — a rare operator path,
    typically ad-hoc debugging. The KiroClaw production path always
    derives the socket from ``McpGatewayConfig.socket_path`` / the
    ``default_socket_path()`` in :mod:`kiro_claw.mcp_gateway.rewriter`,
    which returns ``$KIROCLAW_HOME/mcp-gateway/gateway.sock``.

    Preference order for this CLI fallback:
    1. ``$XDG_RUNTIME_DIR/kiroclaw/mcp-gateway.sock`` when XDG is set.
    2. ``/tmp/kiroclaw-mcp-gateway.sock`` fallback.
    """
    xdg = os.environ.get("XDG_RUNTIME_DIR")
    if xdg:
        return Path(xdg) / _DEFAULT_SOCKET_SUBDIR / _DEFAULT_SOCKET_NAME
    return Path("/tmp") / f"kiroclaw-{_DEFAULT_SOCKET_NAME}"
...
async def run_gatewayd(
    ...
    *,
    ...
    try:
        ...
        # 0700 $KIROCLAW_HOME directory; the per-connection SO_PEERCRED check in
...
def env_target_resolver(pool_key: PoolKey) -> Optional[tuple[str, list[str], dict[str, str], str]]:
    ...
    inherited from the gateway process with ``KIROCLAW_CHANNEL_ID``
    ...
    :func:`kiro_claw.mcp_gateway.manager._scrub_sensitive_env` so even if
    ...
    # Strip PYTHONPATH/PYTHONHOME so the KiroClaw process's own Python
    # environment doesn't leak into Python-based MCP backends (import conflicts).
    env.pop("PYTHONPATH", None)
    env.pop("PYTHONHOME", None)
    if pool_key.channel_id:
        env["KIROCLAW_CHANNEL_ID"] = pool_key.channel_id
...
def _audit_peer_denied(reason: str) -> None:
    ...
    (:mod:`kiro_claw.sel`) in addition to the WARNING log line. Wrapped
...
def _audit_peer_allowed(caller: str, pool_label: str) -> None:
    ...
    HMAC-chained security event log (:mod:`kiro_claw.sel`) alongside the
...
def _prepare_socket_dir(socket_path: Path) -> None:
    ...
    # guarantee holds even when $KIROCLAW_HOME/mcp-gateway already existed
...
def _zombie_diagnostic_path() -> Path:
    ...
    ``$KIROCLAW_HOME/logs/gatewayd_zombie_diagnostic.jsonl`` so a single
    ``tail -f`` follows both heartbeat (gatewayd.log) and any detected
    zombie state.
    """
    mc_home = os.environ.get("KIROCLAW_HOME") or os.path.expanduser("~/.kiroclaw")
...
def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="KiroClaw MCP gateway daemon — pools MCP backends across sessions",
    )
    p.add_argument(
        "--socket",
        dest="socket",
        default=str(_default_cli_socket_path()),
        help="Unix socket path to bind. Default: $XDG_RUNTIME_DIR/kiroclaw/mcp-gateway.sock",
    )
    p.add_argument(
        "--max-backends",
        dest="max_backends",
...
def main() -> None:
    """Sync entry point for ``python -m kiro_claw.mcp_gateway.gatewayd``."""
    try:

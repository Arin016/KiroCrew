...
advertised the ``kiroclaw.caller-identity`` capability (pooled operation),
...
from kiro_claw.mcp_caller import (
    CALLER_CAPABILITY_KEY,
    CALLER_META_KEY,
    CallerContext,
    build_caller_meta,
)
from kiro_claw.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES
...
#    "pool": "example-mcp::kiroclaw::...", "pid": 12345, "ok": true}
#
...
def _strip_caller_meta(msg: dict[str, Any]) -> dict[str, Any]:
    ...
    ``params._meta.kiroclaw.caller`` unconditionally removed.
    ...
    ``_meta.kiroclaw.caller`` in the request. This function is called on
    ...
    # ("kiroclaw.caller") — exactly the shape build_caller_meta writes and
    # CallerContext.from_meta reads. Strip that flat key. (An earlier nested
    # "_meta[kiroclaw][caller]" strip was a no-op against the real wire format,
...
def _inject_caller_meta(msg: dict[str, Any], caller: CallerContext) -> dict[str, Any]:
    """Return a shallow copy of ``msg`` with ``params._meta.kiroclaw.caller``
...
class Backend:
    ...
    async def forward_from_stub(
        ...
        *,
        ...
        3. Caller-identity injection — ``tools/call`` requests get a
           ``params._meta.kiroclaw.caller`` block when the backend
           advertised the capability at initialize time. The block is
           built via :func:`kiro_claw.mcp_caller.build_caller_meta` so
    ...
    async def _handle_initialize(
        ...
        # path — without this a stub could forge _meta.kiroclaw.caller at init.
...
async def spawn_backend(
    ...
    1. ``command`` is taken verbatim from ``MC_MCP_TARGET_<SERVER>`` env
       vars populated at KiroClaw startup by the rewriter from the user's
...
async def send_initialize(
    ...
    *,
    ...
    ``capabilities.experimental.kiroclaw.caller-identity`` in the response.
    ...
    spawn for them (no cross-tenant injection of ``_meta.kiroclaw.caller``).
    ...
    request = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": dict(client_info or {"name": "kiroclaw-gateway", "version": "0"}),
        },
    }
...
async def _pump_stderr(reader: asyncio.StreamReader, label: str) -> None:
    ...
    while True:
        ...
        # stack trace) does not land verbatim in the KiroClaw log.
        logger.debug(
            "backend[%s] stderr: %s",
            label,

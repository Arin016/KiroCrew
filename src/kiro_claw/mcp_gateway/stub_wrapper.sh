...
#
# Shim invoked by kiro-cli in place of calling the KiroClaw MCP stub
# directly.
#
...
# recover session-scoped env vars (currently KIROCLAW_CHANNEL_ID) and
...
# injection.
#
# Generated and placed by kiro_claw.mcp_gateway.rewriter. Usage from
...
channel_id=$(_walk_env KIROCLAW_CHANNEL_ID)

# ── Invocation log ─────────────────────────────────────────────────────────
# Single-line JSON to $KIROCLAW_HOME/logs/stub_wrapper.jsonl capturing the
...
_LOG_DIR="${KIROCLAW_HOME:-$HOME/.kiroclaw}/logs"
...
# dashboard session, which has no KIROCLAW_CHANNEL_ID).
exec "$PY_BIN" -m kiro_claw.mcp_gateway.stub ${EXTRA[@]+"${EXTRA[@]}"} "$@"

"""Supervise the KiroClaw MCP gateway subprocess.

Lifecycle:

1. :meth:`GatewayManager.start` — spawn ``python -m
   kiro_claw.mcp_gateway.gatewayd``, wait until the unix socket appears,
...
Gateway failures are non-fatal for KiroClaw. If the daemon crashes, the
...
from kiro_claw.env import resolve_krb5_ccname
from kiro_claw.mcp_gateway.pool import READ_BUFFER_LIMIT_BYTES
from kiro_claw.sandbox import _SENSITIVE_ENV_PREFIXES as _SANDBOX_SENSITIVE_ENV_PREFIXES
...
_GATEWAYD_MODULE = "kiro_claw.mcp_gateway.gatewayd"
...
# (SLACK_BOT_TOKEN/APP/USER, KIROCLAW_OWNER_ID). config/loader.py seeds those
...
class GatewayManager:
    ...
    async def _spawn_once(self) -> None:
        ...
        # Capture gatewayd stdout/stderr to the canonical KiroClaw log path.

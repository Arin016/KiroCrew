"""KiroClaw MCP stub — shim between kiro-cli and the gateway daemon.
...
record to ``$KIROCLAW_HOME/logs/stub_fallback.jsonl`` and ``execvpe``\u200bs
...
def _default_socket_path() -> str:
    """Resolve the default gateway socket under KIROCLAW_HOME (0700 dir)."""
    home = os.environ.get("KIROCLAW_HOME")
    base = Path(home) if home else Path(os.environ.get("HOME", "")) / ".kiroclaw"
    return str(base / "mc-mcp-gateway.sock")


def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    """Parse the argv shape produced by
    :func:`kiro_claw.mcp_gateway.rewriter.rewrite_agents`. ``--real-stub``
    ...
    p = argparse.ArgumentParser(
        prog="mc-mcp-stub",
        description="KiroClaw MCP shim: proxies kiro-cli stdio to the local gateway",
    )
...
def _resolve_channel_id(cli_value: Optional[str]) -> Optional[str]:
    """``--channel-id`` wins; else ``KIROCLAW_CHANNEL_ID`` env; else None."""
...
def main() -> None:
    """Sync entry point for ``python -m kiro_claw.mcp_gateway.stub``."""
    try:

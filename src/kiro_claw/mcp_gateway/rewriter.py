...
the overlay directory (``~/.kiroclaw/mcp-gateway/agents/``). The host
...
to ``KIROCLAW_SESSION_KEY`` and cannot be safely shared across sessions.
"""
...
# a backend NOT advertising ``kiroclaw.caller-identity`` in its initialize
...
def _build_stub_entry(
    *,
    ...
    wrapped.update({
        ...
        # KIROCLAW_CHANNEL_ID (kiro-cli strips env when spawning MCP
        # subprocesses). Wrapper prepends the recovered value as
        # --channel-id before exec'ing ``python -m
        # kiro_claw.mcp_gateway.stub``. See stub_wrapper.sh.
        "args": stub_args,
    ...
    *,
    ...
    for name, entry in servers.items():
        ...
        if name in UNPOOLABLE_SERVERS:
            # Leave unchanged — these bind to KIROCLAW_SESSION_KEY.
            new_servers[name] = entry
            continue
...
def rewrite_agents(
    *,
    ...
    Args:
        ...
        overlay_dir: Usually ``~/.kiroclaw/mcp-gateway/agents/``. Created
        ...
        sandbox_mode: Value from ``config.agent.sandbox`` — fed through
            so the stub's PoolKey matches KiroClaw's sandbox policy.
    ...
    # Wrapper walks PPID /proc/<pid>/environ to recover KIROCLAW_CHANNEL_ID
    ...
    # ``python -m kiro_claw.mcp_gateway.stub`` exec.
    ...
    try:
        # Bake the interpreter running KiroClaw into the wrapper. kiro-cli
        ...
        # on a python3 on PATH that can import ``kiro_claw``. Substituting
    ...
    # bypasses the gateway unless wrapped (the "kiroclaw-lite bypass" class of
...
def default_overlay_dir() -> Path:
    """Return ``$KIROCLAW_HOME/mcp-gateway/agents`` (follows ``config_dir``)."""
    home = os.environ.get("KIROCLAW_HOME")
    base = Path(home) if home else Path.home() / ".kiroclaw"

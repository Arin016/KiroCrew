...
Runs as ``kiroclaw mcp-cron`` — kiro-cli spawns it as a child process
...
from kiro_claw.cron_script import resolve_script_path
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "script": {
                        ...
                        "'~/.kiroclaw/crons/file.py:function'. Scripts must be under "
                        "~/.kiroclaw/crons/. Function receives a "
...
def _call_tool(name: str, raw_args: dict[str, Any]) -> str:
    ...
    return call_tool_with_logging(
        ...
        session_key="mcp_cron", downstream_service="kiroclaw-cron",
    )
...
def _call_tool_inner(name: str, args: dict[str, Any]) -> str:
    ...
    if name == "cron_add":
        ...
        if channel is None:
            channel = os.environ.get("KIROCLAW_CHANNEL_ID") or None
    ...
    if name == "cron_remove_all":
        ...
        session_key = _resolve_session_key()
        is_cli = os.environ.get("KIROCLAW_CLI", "") == "1"
        if not is_cli:
            if not session_key:
                sel().log_tool_invocation(
...
def run_mcp_server() -> None:
    ...
    run_mcp_stdio_loop("kiroclaw-cron", "1.0.0", _list_tools, _call_tool)

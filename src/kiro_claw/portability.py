"""Portable zip export/import for KiroClaw state (dashboard endpoint).

Creates a zip archive of all KiroClaw settings and memory for download
...
def _mc_dir() -> Path:
    return Path(os.environ.get("KIROCLAW_HOME", config_dir()))
...
def create_export_zip() -> tuple[bytes, dict]:
    """Create a zip archive of KiroClaw state. Returns (zip_bytes, manifest_dict)."""
    mc = _mc_dir()
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    prefix = f"kiroclaw-export-{ts}"

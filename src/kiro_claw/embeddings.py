...
def _sigv4_sign(method: str, url: str, headers: dict, body: bytes | str) -> dict | None:
    ...
    than sending unsigned. Reads AWS_REGION / KIROCLAW_EMBEDDING_SERVICE env vars.
    """
    if not _HAS_BOTOCORE:
        logger.warning("botocore not installed; cannot SigV4-sign embedding requests")
        return None
    try:
        service = os.environ.get("KIROCLAW_EMBEDDING_SERVICE", "execute-api")
    ...
    if host not in ("localhost", "127.0.0.1", "::1"):
        if not allow_remote:
            ...
            raise ValueError(
                ...
                "Set embedding_url in ~/.kiroclaw/config.json to allow a remote server."
            )
...
class OllamaManager:
        ...
        try:
            if system == "Darwin":
                ...
                if proc.returncode == 0:
                    ...
                    # KiroClaw's own Ollama process management. Disable it.
    ...
    def _log_pull_failure(self, detail: str) -> None:
        ...
        msg = (
            ...
            "%r in ~/.kiroclaw/config.json (or run ``ollama pull %s`` manually)."
        )

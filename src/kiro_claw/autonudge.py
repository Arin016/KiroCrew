...
State is persisted to ``~/.kiroclaw/autonudge.json`` (fcntl-locked, atomic
...
Feature-flagged via env ``KIROCLAW_AUTONUDGE`` (on by default; set to ``0`` to disable).
"""
...
def enabled() -> bool:
    """Feature flag — on by default. Set ``KIROCLAW_AUTONUDGE=0`` to disable."""
    return os.environ.get("KIROCLAW_AUTONUDGE", "1").lower() not in ("0", "false", "no")
...
class AutoNudgeService:
    ...
    async def start(self) -> None:
        if not enabled():
            logger.info("AutoNudge disabled (KIROCLAW_AUTONUDGE not set)")
            return
        self._load()

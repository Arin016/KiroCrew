...
from kiro_claw.eval.judge import LLMJudge
from kiro_claw.eval.runner import EvalRunner, format_results, score_by_dimension
...
from kiro_claw.validation import _AGENT_NAME_RE, CHANNEL_ID_RE, CHANNEL_MAX_LEN, WORKSPACE_NAME_RE
...
def _internal_secret() -> str:
    ...
    The gateway writes ``~/.kiroclaw/.local_secret`` (mode 0600) after a
    ...
    browser cookie. Mirrors `kiro_claw.mcp_core._internal_secret`.
...
def _spawn(args: argparse.Namespace) -> None:
    ...
    print("Usage: kiroclaw spawn {run|list}")
...
def _handle_workspace(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        ...
        print("Usage: kiroclaw workspace {list|create|update|delete}")
    ...
    if action == "install":
        ...
        if result.ok:
            ...
            if reg.errors:
                for e in reg.errors:
                    print(f"   ⚠️  {e}")
            print(f"\n   Run: kiroclaw app enable {result.name}")
        else:
        ...
        print(f"   kiroclaw app install {app_dir}")

    else:
        print("Usage: kiroclaw app {install|list|enable|disable|uninstall|info|init}")
    ...
    if action == "list":
        ...
        for name, agent in cfg.agents.items():
            ...
            print(
                ...
                f"{getattr(agent, 'source', 'kiroclaw'):<12}"
            )
        ...
        if args.name in cfg.agents:
            ...
            sys.exit(1)
        cfg.agents[args.name] = KiroClawAgentConfig(
        ...
        print("Usage: kiroclaw agent {list|create|update|delete}")
...
def _learn(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if action == "add":
            ...
            print("Usage: kiroclaw learn {add|list|remove}")
    finally:
        vs.close()
...
def _memory_cmd(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if action == "list":
            ...
            if not import_file:
                print("Usage: kiroclaw memory import <file>")
                return
            ...
            print("Usage: kiroclaw memory {list|search|stats|audit|export|migrate|import}")
    finally:
        store.close()
...
def _artifact(args: argparse.Namespace) -> None:
    ...
    print(
        "Usage: kiroclaw artifact {list|show|save|update|delete|versions}",
        file=sys.stderr,
    )
    sys.exit(2)

"""Sidecar store for KiroClaw's per-agent bookkeeping.
...
"no agent with name X found" line). KiroClaw therefore keeps its private
...
State file (``~/.kiroclaw/agent_model_state.json``, honoring ``KIROCLAW_HOME``)::
...
def _state_path() -> Path:
    """Return the sidecar path (resolved fresh so KIROCLAW_HOME is honored)."""

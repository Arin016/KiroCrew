"""Caller-identity protocol extension for KiroClaw MCP servers.

BACKGROUND
----------
The base MCP protocol (Anthropic spec 2024-11-05) models a 1:1 stdio
transport: one client, one server. When KiroClaw introduces a broker that
...
Previously, the backend read this identity from the ``KIROCLAW_SESSION_KEY``
...
2. **Capability negotiation** — backends that support pooled operation
   advertise the extension in their ``initialize`` response::
       ...
       "capabilities": {
           ...
           "experimental": {
               "kiroclaw.caller-identity": {"schemaVersion": 1}
           }
       }
...
3. **Typed context** — tool handlers receive a ``CallerContext`` dataclass
   ...
   constructed from the ambient ``KIROCLAW_SESSION_KEY`` env var so
...
any future KiroClaw-owned MCP server must go through this module.
"""
...
CALLER_META_KEY = "kiroclaw.caller"

#: Capability key backends advertise in ``initialize`` response under
#: ``capabilities.experimental``. Presence indicates the backend
#: understands the ``kiroclaw.caller`` ``_meta`` block and is safe to pool.
CALLER_CAPABILITY_KEY = "kiroclaw.caller-identity"
...
class CallerContext:
    """Identity of the originating KiroClaw session for a single MCP call.
    ...
    def from_env(cls) -> "CallerContext":
        """Synthesize a context from the ``KIROCLAW_SESSION_KEY`` env var, or
        ...
        Fallback order:
          1. ``KIROCLAW_SESSION_KEY`` env var (primary, per-session mode)

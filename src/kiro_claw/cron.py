...
Jobs are stored in the config directory (``~/.kiroclaw/crons.json`` by default,
overridden by ``KIROCLAW_HOME``) and executed by a background
...
Jobs are created via MCP tools (``cron_add``) or the CLI (``kiroclaw cron add``).
...
def format_schedule(schedule: CronSchedule, tz_name: str = "") -> str:
    ...
    if not tz_name:
        try:
            tz_name = KiroClawConfig.load().timezone
        except Exception:
            pass
...
def get_local_tz() -> tuple[str, ZoneInfo]:
    ...
    try:
        tz_name = KiroClawConfig.load().timezone or "UTC"
...
def _job_tz(job: CronJob) -> ZoneInfo:
    ...
    try:
        tz_name = job.timezone or KiroClawConfig.load().timezone or "UTC"
...
class CronService:
    ...
    def __init__(
        ...
        _cfg = KiroClawConfig.load().cron_history

"""KiroClaw snapshot and restore — portable state management."""
...
def _default_snapshot_dir() -> str:
    """Return snapshot directory from config, falling back to ~/.kiroclaw/snapshots."""
    try:
        from kiro_claw.config.loader import KiroClawConfig
        d = KiroClawConfig.load().snapshot_dir
        if d:
            return str(Path(d).expanduser())
    except Exception:
        pass
    return str(Path.home() / ".kiroclaw" / "snapshots")
...
def _mc_dir() -> Path:
    ...
    # KIROCLAW_HOME override (and the same ~/.kiroclaw default) as every other
    # module. Previously this read an undocumented KIROCLAW_DIR, which made
...
def snapshot_main(argv: list[str] | None = None, *, parsed: argparse.Namespace | None = None) -> int:
    if parsed is None:
        p = argparse.ArgumentParser(prog="kiroclaw-snapshot",
                                    description="Create a portable .tar.gz snapshot of KiroClaw state.")
    ...
    if args.list_snapshots:
        if not out.is_dir():
            ...
            return 0
        snaps = sorted(out.glob("kiroclaw-snapshot-*.tar.gz"),
    ...
    name = f"kiroclaw-snapshot-{ts}"

    # Pre-flight size estimate
    if mc.is_dir():
        total_bytes = sum(f.stat().st_size for f in mc.rglob("*") if f.is_file() and not f.is_symlink())
        total_mb = total_bytes / (1024 * 1024)
        if total_mb > 500:
            print(f"⚠️  ~/.kiroclaw is {total_mb:.0f} MB — snapshot may be large and slow")
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        if (mc / "workspace").is_dir():
            _copytree_safe(mc / "workspace", stage / "workspace",
                           dirs_exist_ok=True,
                           ignore=shutil.ignore_patterns("hygiene_data", "insert_facts*.py"))

        # Plan memory
        if (mc / "plan_memory").is_dir():
            _copytree_safe(mc / "plan_memory", stage / "plan_memory",
                           dirs_exist_ok=True)

        # Skills
        if (mc / "skills").is_dir():
            _copytree_safe(mc / "skills", stage / "skills",
                           dirs_exist_ok=True)
    ...
    remaining = len(list(out.glob("kiroclaw-snapshot-*.tar.gz")))
...
def restore_main(argv: list[str] | None = None, *, parsed: argparse.Namespace | None = None) -> int:
    if parsed is None:
        p = argparse.ArgumentParser(prog="kiroclaw-restore",
                                    description="Restore KiroClaw state from a snapshot.")
    ...
    if not force and _is_gateway_running():
        ...
        print("❌ Gateway is running. Stop it first (kiroclaw stop) or use --force.")
        return 1
    ...
    with tempfile.TemporaryDirectory() as work_str:
        ...
        snap_dirs = [d for d in work.iterdir()
                     if d.is_dir() and d.name.startswith("kiroclaw-snapshot-")]
        if not snap_dirs:
    ...
    print("\n⚠️  Restart kiroclaw gateway to pick up changes: kiroclaw restart")
    return 0

"""Shared helpers for building the KiroClaw website frontend assets.
...
and must be staged into ``<repo-root>/src/kiro_claw/static/dist`` so the
...
``KiroClawWebsite/dist`` clone is honored as a last-resort fallback when
...
# clone. ``KIROCLAW_WEBSITE_REPO`` is retained only so existing tooling/docs
# referencing the public mirror keep a stable name to point at.
_DEFAULT_REPO_URL = "https://github.com/kirodotdev-labs/kiroclaw"
_REPO_URL = os.environ.get("KIROCLAW_WEBSITE_REPO") or _DEFAULT_REPO_URL
...
_DIR_NAME = "website"
_SIBLING_DIR_NAME = "KiroClawWebsite"
...
def _repo_root(kiro_claw_pkg_dir: Path) -> Path:
    """Return the repo root given the ``kiro_claw`` package directory.

    Layout: ``<repo-root>/src/kiro_claw/`` is *kiro_claw_pkg_dir*, so two
    ``.parent`` hops land on the repo root (parent of ``src/``).
    """
    return kiro_claw_pkg_dir.parent.parent


def _resolve_website_dist(kiro_claw_pkg_dir: Path) -> Optional[Path]:
    ...
    2. A sibling checkout — ``<repo-root>/../KiroClawWebsite/dist`` (legacy
       side-by-side dev layout). Last-resort only.

    Returns the resolved dist path on success, ``None`` otherwise.
    """
    repo_root = _repo_root(kiro_claw_pkg_dir)
    ...
    # 2. Sibling KiroClawWebsite/dist (legacy fallback).
...
def ensure_dev_dist_symlink() -> Optional[Path]:
    ...
    The dashboard serves its SPA from ``<kiro_claw>/static/dist/index.html``.
    ...
    3. Missing → resolve the in-tree ``website/dist`` (or a sibling
       ``KiroClawWebsite`` checkout as a last resort) and symlink to it.
    ...
    # local build landing in the source tree / user ran kiroclaw init --ui).
    ...
    candidate = _resolve_website_dist(kiro_claw_pkg_dir)
...
def _stage_dist(
    ...
    """
    static_dist = proj_path / "src" / "kiro_claw" / "static" / "dist"
...
def build_frontend_sync(
    ...
    ``website/dist`` into ``src/kiro_claw/static/dist``. Graceful no-op when
...
async def build_frontend_async(
    ...
    ``src/kiro_claw/static/dist``. Graceful no-op when there is no

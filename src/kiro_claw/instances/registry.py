"""Instances registry — persistent store of remote KiroClaw instances.

Backs the *Instances* feature (multi-instance management). The registry is a
small JSON file at ``~/.kiroclaw/instances.json``. Each record describes how to
reach one remote KiroClaw over SSH; the *local* instance is implicit (the
...
* Writes go through :func:`kiro_claw.atomic_write.atomic_write` (temp file +
...
in-memory copy, so a live gateway and an out-of-band ``kiroclaw`` CLI edit don't
...
# Monkeypatchable in tests via ``monkeypatch.setattr`` alongside KIROCLAW_HOME,
# per the shared-state test-isolation lesson. ``None`` means "derive from
# config_dir() at call time" so KIROCLAW_HOME overrides are always honoured.
...
@dataclass
class Instance:
    """One remote KiroClaw instance reachable over SSH.

...
``kiro_claw.tunnel.manager.TunnelManager`` (which points *outward* to expose the
...
loopback port to the remote KiroClaw's dashboard port.
...
from kiro_claw.instances.validation import (
    SshValidationError,
    validate_remote_bin,
    validate_ssh_host,
)
...
class SshTunnelManager:
    ...
    async def restart_remote(self, instance_id: str) -> dict:
        """Restart the remote KiroClaw gateway over SSH.

        Uses the remote ``kiroclaw restart`` (itself systemd/launchd-aware), run
        ...
        rc, err = await run_remote_kiroclaw(ssh_host, "restart", remote_bin=remote_bin)
        if rc == 0:
    ...
    async def token_validates(self, local_port: int, token: str) -> bool:
        ...
        failed self-heal re-mint, or a remote ``kiroclaw restart`` that

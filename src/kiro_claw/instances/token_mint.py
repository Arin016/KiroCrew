"""Mint a remote KiroClaw dashboard token over SSH.

Build the shell snippet that runs ``kiroclaw token`` on the remote desktop and
...
DEFAULT_REMOTE_BIN = "$HOME/.local/bin/kiroclaw"
...
# ttl accepted by `kiroclaw token --ttl`: a positive integer with an h/m suffix
# (e.g. "20h", "30m"). Validated to keep it out of the remote command unchecked.
_TTL_RE = re.compile(r"^[1-9][0-9]{0,3}[hm]$")

# Extract the JWT from a `kiroclaw token` URL: http://localhost:7777?token=eyJ...
# (also matches https://.../?token=...&foo=bar).
_TOKEN_RE = re.compile(r"[?&]token=([^\s&]+)")

# How long to wait for the remote `kiroclaw token` to return before giving up.
...
def parse_token_from_stdout(stdout: str) -> str:
    """Extract the JWT from a ``kiroclaw token`` URL, or ``""`` if absent."""
...
def _token_subcommand(ttl: str | None, port: int | None = None) -> str:
    """Return the ``kiroclaw token`` subcommand (ttl/port pre-validated).

    ``--port`` is essential when the remote gateway isn't on the default 7777:
    ``kiroclaw token`` calls its own dashboard to mint, so the port must match
...
def build_candidate_command(
    ...
    Generic over the kiroclaw *subcommand* (e.g. ``token``, ``restart``).
    ...
    return " ".join(
        [
            ...
            "  fi;",
            "done;",
            f'echo "kiroclaw binary not found in any of: {", ".join(candidates)}" >&2;',
            "exit 127",
        ]
    )
...
async def mint_remote_token(
    ...
    *,
    ...
    """SSH to *ssh_host*, run ``kiroclaw token``, and return the parsed JWT.

    *remote_port* is passed to ``kiroclaw token --port`` so the remote mint
...
async def run_remote_kiroclaw(
    ...
    *,
    ...
    """Run ``kiroclaw <subcommand>`` on *ssh_host* over SSH.
    ...
    argv = _build_ssh_argv(ssh_host, remote_command)
    logger.info("Running 'kiroclaw %s' on %s", subcommand, ssh_host)
    try:
        proc = await asyncio.create_subprocess_exec(
            *argv,
            stdout=asyncio.subprocess.PIPE,

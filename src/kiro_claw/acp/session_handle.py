...
existing ``from kiro_claw.acp.runtime import AcpSessionHandle`` call sites keep
working.
"""
...
class AcpSessionHandle:
    ...
    def _emit_tool_interrupted_sel(self, site: str) -> None:
        ...
        KiroClaw observes but does not control (kiro-cli denied tool execution).
        Best-effort — a failed audit must not break the turn. This handle has no
        KiroClaw session_key, so the ACP sessionId is recorded in metadata for
        correlation.
        """
        logger.warning(

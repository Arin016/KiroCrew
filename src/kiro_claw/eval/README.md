# KiroClaw Eval Harness

Multi-session evaluation harness for benchmarking KiroClaw's cross-session memory, lesson application, and context accumulation.
...
Scenarios are JSON files in `src/kiro_claw/eval/scenarios/`. Each defines sessions with turns and assertions (YAML is also supported for backward compatibility):
...
```
kiroclaw eval            CLI entry point — scenario selection, output
...
Key design: each session creates a **new provider instance**, simulating a user closing and reopening KiroClaw. Cross-session context must come from persisted memory, not conversation history.

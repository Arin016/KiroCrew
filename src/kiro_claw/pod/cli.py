"""``kiroclaw pod <verb>`` — kubectl-style control of worktree test pods.

Thin verb layer over :mod:`kiro_claw.pod.runtime` / :mod:`kiro_claw.pod.unit`.
Dispatched from :func:`kiro_claw.cli_commands._pod`.
"""
...
def _up(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    if getattr(args, "provision", False):
        ...
        if not prov.has_dist(checkout):
            _die(
                ...
                f"  Or let pod do the full chain: kiroclaw pod up {name} --provision"
            )
    ...
    if code not in (200, 401, 403):
        ...
        if code == -1:
            _die(
                ...
                f"This is the worktree build, not pod — fix it, then `kiroclaw pod up {name}` again."
            )
        _die(
    ...
    if args.json:
        ...
        print(f"  stop     : kiroclaw pod down {name}")
...
def _install(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    print("ready. Next: kiroclaw pod up <worktree>")
...
def _cleanup_internal(cfg: PodConfig, args: argparse.Namespace) -> None:
    ...
    started directly as ``kiroclaw-pod@..`` can't ``rm`` outside the pod root.
    """
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    if not action:
        print("Usage: kiroclaw pod {up|down|ls|status|token|url|logs|install|provision} …")
        sys.exit(2)
    cfg = PodConfig.load()

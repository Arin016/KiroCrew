...
Every value is ``KIROCLAW_POD_*``-overridable so a test harness can stand up a
...
DEFAULT_UNIT_PREFIX = "kiroclaw-pod"
...
class PodConfig:
    ...
    checkout by :func:`kiro_claw.pod.runtime.resolve_checkout` (git-native).
    """
    ...
    def load(cls) -> "PodConfig":
        home = Path.home()
        pod_root = _env_path("KIROCLAW_POD_ROOT", home / ".kiroclaw-pods")
        pods_dir = _env_path("KIROCLAW_POD_ENV_DIR", home / ".kiroclaw" / "pods")
        artifacts_dir = _env_path("KIROCLAW_POD_ARTIFACTS_DIR", pod_root / ".e2e-artifacts")
        ...
        return cls(
            ...
            base_port=_env_int("KIROCLAW_POD_BASE_PORT", DEFAULT_BASE_PORT),
            live_port=_env_int("KIROCLAW_POD_LIVE_PORT", DEFAULT_LIVE_PORT),
            unit_prefix=os.environ.get("KIROCLAW_POD_UNIT_PREFIX", DEFAULT_UNIT_PREFIX),
            gateway_path=os.environ.get("KIROCLAW_POD_PATH", default_path),
            repo_hint=_env_path_opt("KIROCLAW_POD_REPO"),
            worktrees_root=_env_path_opt("KIROCLAW_POD_WORKTREES_ROOT"),
        )

    # ---- derived locations -------------------------------------------------
    def home_dir(self, name: str) -> Path:
        """Isolated ``KIROCLAW_HOME`` for pod *name* (nuked on stop)."""

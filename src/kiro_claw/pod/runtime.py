...
pod's ``.local_secret``) lives here so :mod:`kiro_claw.pod.cli` stays a thin verb
...
def resolve_checkout(
    ...
    (from ``KIROCLAW_POD_REPO`` else *cwd*), matching a worktree's basename, then
    its branch (``name`` or ``feat/<name>``), then an exact path → optional
    ``KIROCLAW_POD_WORKTREES_ROOT/name`` fallback → :class:`PodError`.
    """
    ...
    raise PodError(
        ...
        f"  (run `kiroclaw pod up {name}` from inside a kiroclaw checkout, "
        f"or set KIROCLAW_POD_REPO to point at one)"
    )
...
def sanitized_seed_config(seed_dir: Path) -> dict | None:
    ...
    # (the intended ``--seed ~/.kiroclaw`` workflow) would otherwise boot live
...
def build_pod_env(cfg: PodConfig, home_dir: Path, port: int, checkout: Path) -> dict[str, str]:
    ...
    env = {
        **os.environ,
        ...
        "KIROCLAW_HOME": str(home_dir),
        "KIROCLAW_PORT": str(port),
        "KIROCLAW_PROJECT_DIR": str(checkout),
...
# --------------------------------------------------------------------------- #
# Boot — the ExecStart body. Re-entered as ``kiroclaw pod _run <name>`` by the
...
def boot(cfg: PodConfig, name: str) -> int:
    ...
    if not checkout_str:
        print(
            ...
            f"`kiroclaw pod up {name}` from inside a kiroclaw checkout first"
        )
        return 3
    checkout = Path(checkout_str).expanduser()
    home_dir = cfg.home_dir(name)
    bin_path = checkout / ".venv" / "bin" / "kiroclaw"

    if not (bin_path.exists() and os.access(bin_path, os.X_OK)):
        print(f"FATAL: no kiroclaw venv at {bin_path} (provision {name} first)")
        return 3
    if not (checkout / "src" / "kiro_claw" / "static" / "dist").is_dir():
    ...
    print(f"kiroclaw-pod: name={name} port={port} home={home_dir} checkout={checkout}")

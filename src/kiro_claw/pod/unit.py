...
The unit's ``ExecStart`` re-enters the installed ``kiroclaw`` binary as
``kiroclaw pod _run %i``, so the boot logic lives in Python (see
:func:`kiro_claw.pod.runtime.boot`) and nothing is shipped outside the package.
``ExecStopPost`` re-enters ``kiroclaw pod _cleanup %i`` to delete the pod's
...
cannot contain ``/``; :func:`kiro_claw.pod.runtime.cleanup_home` re-validates the
...
_UNIT_TEMPLATE = """\
[Unit]
Description=KiroClaw pod (%i) — isolated worktree gateway, throwaway
# Multi-active: many run at once, one per worktree, each on its own port + own
# KIROCLAW_HOME. Orthogonal to the live gateway; refuses to bind the live port.
...
# kiroclaw binary; boot logic is kiro_claw.pod.runtime.boot (no shipped shell).
ExecStart={kiroclaw_bin} pod _run %i
...
ExecStopPost={kiroclaw_bin} pod _cleanup %i
...
def _kiroclaw_bin() -> str:
    """Absolute path (or module invocation) to the kiroclaw entry-point the unit
    should boot.

    Resolution order:
      1. ``KIROCLAW_POD_BIN`` — explicit override (used when installing a unit that
         must boot a specific build, e.g. a worktree's own ``.venv``).
      2. the console-script on PATH.
      3. ``<this python> -m kiro_claw`` so it works from a bare checkout.
    """
    override = os.environ.get("KIROCLAW_POD_BIN")
    if override:
        return override
    found = shutil.which("kiroclaw")
    if found:
        return found
    return f"{sys.executable} -m kiro_claw"
...
def _environment_block(cfg: PodConfig) -> str:
    ...
    home = Path.home()
    candidates = [
        ("KIROCLAW_POD_ROOT", str(cfg.pod_root), str(home / ".kiroclaw-pods")),
        ("KIROCLAW_POD_ENV_DIR", str(cfg.pods_dir), str(home / ".kiroclaw" / "pods")),
        (
            "KIROCLAW_POD_ARTIFACTS_DIR",
            str(cfg.artifacts_dir),
            str(cfg.pod_root / ".e2e-artifacts"),
        ),
        ("KIROCLAW_POD_BASE_PORT", str(cfg.base_port), str(DEFAULT_BASE_PORT)),
        ("KIROCLAW_POD_LIVE_PORT", str(cfg.live_port), str(DEFAULT_LIVE_PORT)),
        ("KIROCLAW_POD_UNIT_PREFIX", cfg.unit_prefix, DEFAULT_UNIT_PREFIX),
        ("KIROCLAW_POD_PATH", cfg.gateway_path, None),  # always pin PATH
    ]
    lines = [
    ...
    if cfg.repo_hint is not None:
        lines.append(f"Environment=KIROCLAW_POD_REPO={cfg.repo_hint}\n")
    if cfg.worktrees_root is not None:
        lines.append(f"Environment=KIROCLAW_POD_WORKTREES_ROOT={cfg.worktrees_root}\n")
    return "".join(lines)


def render_unit(cfg: PodConfig) -> str:
    return _UNIT_TEMPLATE.format(
        kiroclaw_bin=_kiroclaw_bin(),

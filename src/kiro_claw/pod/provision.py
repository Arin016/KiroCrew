...
A pod boots the worktree's OWN ``.venv/bin/kiroclaw gateway`` serving its OWN
...
module collapses them into one command (``kiroclaw pod provision`` /
``pod up --provision``).
...
def venv_bin(checkout: Path) -> Path:
    return checkout / ".venv" / "bin" / "kiroclaw"


def dist_dir(checkout: Path) -> Path:
    return checkout / "src" / "kiro_claw" / "static" / "dist"
...
def build_dist(checkout: Path) -> bool:
    ...
    ``src/kiro_claw/static/dist``. Returns True if the dist exists afterward."""
...
def provision(checkout: Path, build: bool = True) -> bool:
    ...
    if not build_dist(checkout):
        return False
    _say(f"[provision] {checkout.name} is ready — `kiroclaw pod up {checkout.name}`")
    return True

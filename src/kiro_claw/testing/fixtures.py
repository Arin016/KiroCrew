"""Test helpers for consumers that need a seeded ``$KIROCLAW_HOME``.
...
1. ``seeded_home(fixture_name)`` — plain context manager. Use from any
   ...
   the previous ``KIROCLAW_HOME`` env var on exit.
2. ``seeded_home_fixture`` — pytest fixture wrapping (1). Parameterised
   ...
   ``kiro_claw.testing.fixtures`` to their ``conftest.py`` via
...
# ``pytest`` is an OPTIONAL runtime dependency. ``kiro_claw.testing`` ships in
# the runtime wheel (not ``dev_requirements``) so downstream consumers can do
# ``pip install kiroclaw`` and use ``seeded_home`` from any test harness
...
from kiro_claw.seed import seed
...
def seeded_home(fixture_name: str) -> Iterator[Path]:
    """Context manager yielding a populated ``$KIROCLAW_HOME`` tempdir.

    On enter: creates a fresh temp directory, points ``KIROCLAW_HOME`` at
    it, and runs ``seed(fixture_name)`` to populate the tree. On exit:
    restores the previous ``KIROCLAW_HOME`` (or unsets if it wasn't set)
    ...
    Example:
        with seeded_home("minimal") as home:
            ...
            # agent code under test reads $KIROCLAW_HOME here
    """
    previous = os.environ.get("KIROCLAW_HOME")
    tmpdir = Path(tempfile.mkdtemp(prefix="kiroclaw-seeded-home-"))
    try:
        ...
        os.environ["KIROCLAW_HOME"] = str(tmpdir)
        seed(fixture_name)
        yield tmpdir
    finally:
        ...
        if previous is None:
            ...
            os.environ["KIROCLAW_HOME"] = previous
        if tmpdir.exists():
...
if pytest is not None:
    ...
    def seeded_home_fixture(request: "pytest.FixtureRequest") -> Iterator[Path]:
        ...
        Defined only when pytest is importable — ``kiro_claw.testing`` is in

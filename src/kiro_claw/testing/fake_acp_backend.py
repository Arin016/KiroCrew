...
newline-delimited stdio) that ``kiro_claw.acp.client.AcpClient`` drives:
...
the ``KIROCLAW_KIRO_BIN`` override (the provider stays ``acp``); it is never
selectable from a real gateway. Run standalone as
``python -m kiro_claw.testing.fake_acp_backend`` (the pytest harness and the
live-test harness both point ``KIROCLAW_KIRO_BIN`` at a launcher that runs it).

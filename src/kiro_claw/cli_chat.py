...
def _tui(args: argparse.Namespace) -> None:
    ...
    if not tui_js:
        ...
        for _ in range(15):
            p = p.parent
            bundle = p / "src" / "KiroClaw" / "tui" / "dist" / "bundle.mjs"
            if bundle.is_file():
                tui_js = bundle
                break
            idx = p / "src" / "KiroClaw" / "tui" / "dist" / "index.js"
            if idx.is_file() and (p / "src" / "KiroClaw" / "tui" / "node_modules").is_dir():
                tui_js = idx
                break

    if not tui_js:
        print("TUI not built. Run: cd tui && npm install && npm run build")
        print("  (or use: kiroclaw chat  /  kiroclaw gateway)")
        sys.exit(1)
    ...
    home_override = getattr(args, "home", None) or os.environ.get("KIROCLAW_HOME", "")
    if home_override:
...
async def _chat(message: str | None, model: str | None, agent: str | None = None) -> None:
    ...
    if message:
        ...
        if not notice_file.exists():
            print("💡 Try `kiroclaw tui` for a richer experience.", file=sys.stderr)
            notice_file.touch()
...
async def _interactive(provider: LLMProvider, cfg: KiroClawConfig) -> None:
    ...
    notice_file = Path(config_dir()) / ".tui_notice_shown"
    if not notice_file.exists():
        print("💡 Try `kiroclaw tui` for a full-featured terminal UI with")
        print("   syntax highlighting, tool approval, sessions, and more.")
        print("   `kiroclaw chat` will be merged into `kiroclaw tui` in a future version.\n")
        notice_file.touch()
...
def _ensure_default_agent_in_config() -> None:
    """Ensure config.json includes a default KiroClaw agent for fresh installs."""
    p = config_path()
    try:
        data = json.loads(p.read_text(encoding="utf-8")) if p.exists() else {}
    except Exception:

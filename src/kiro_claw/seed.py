"""``kiroclaw seed`` — copy a hand-authored fixture into ``$KIROCLAW_HOME``.

Phase 1.A (walking skeleton): one subcommand, one fixture (``empty``), one guardrail
(``KIROCLAW_HOME`` must be set). Safety guardrails and ``--seed-replace`` land in 1.B.

Fixtures ship as package data at ``src/kiro_claw/tests_fixtures/<name>/``.
Each fixture is a valid ``KIROCLAW_HOME`` tree with a ``fixture.yaml``
declaring ``schema-version``. Shipping inside the package is what lets
``kiroclaw seed`` work after ``pip`` installs the wheel into
``site-packages/kiro_claw/`` (a source-tree-relative path walk would
...
except ImportError:  # pragma: no cover — KiroClaw targets py3.10.
    _resource_files = None  # type: ignore[assignment]

# ``kiro_claw.sel`` is a first-party module shipped in the same package, so
# we import it at top level like every other caller (``subagent.py``,
# ``taskrunner.py``, ``dashboard/stt_stream.py``, ...). An ImportError here
# would mean the KiroClaw install itself is broken — there's no scenario
...
def _fixtures_root() -> Path:
    ...
    workspaces (``src/kiro_claw/tests_fixtures/``) and in the pip-installed
    layout where the package lives under
    ``site-packages/kiro_claw/``. ``files()`` returns a ``Traversable``
    which, for the regular filesystem loader KiroClaw uses, is a real
    ...
    return Path(str(_resource_files("kiro_claw") / _FIXTURES_PKG))
...
def _resolve_fixture(name: str) -> Path:
    ...
    - Empty or ``.``-only names resolve to the fixtures root itself — would
      copy the whole ``tests_fixtures/`` tree into ``$KIROCLAW_HOME``.
    ...
    if not candidate.is_dir():
        ...
        # ``src/kiro_claw/tests_fixtures/`` or the PRD. Sorted for stable
...
def _main_home() -> Path:
    """Return the resolved path to the main gateway home (``~/.kiroclaw``).

    ``Path.home() / ".kiroclaw"``, then ``expanduser().resolve()`` to
    ...
    ``$HOME`` and exercise the guardrail on synthetic ``~/.kiroclaw`` paths.
    """
    return (Path.home() / ".kiroclaw").expanduser().resolve()


def _resolve_target(*, for_main_home_check: bool = False) -> Path:
    """Return ``$KIROCLAW_HOME`` as a ``Path``, or raise ``SeedError``.

    ``expanduser()`` is applied so ``KIROCLAW_HOME=~/dev`` works. Pass
    ``for_main_home_check=True`` to additionally ``resolve()`` the path so
    a symlinked ``KIROCLAW_HOME`` pointing at ``~/.kiroclaw`` is caught by
    ...
    """
    raw = os.environ.get("KIROCLAW_HOME")
    if not raw:
        raise SeedError(
            "$KIROCLAW_HOME is not set. Point it at a dev directory "
            "(e.g. KIROCLAW_HOME=~/.kiroclaw-dev kiroclaw gateway --seed empty).",
    ...
    if for_main_home_check:
        ...
        # catches ``$KIROCLAW_HOME -> ~/.kiroclaw`` even when the symlink
        ...
        # ``~/.kiroclaw`` that ``resolve()`` couldn't evaluate, the
        ...
        try:
            ...
            raise SeedError(
                f"cannot resolve $KIROCLAW_HOME ({raw!r}) for main-home "
...
def seed(fixture_name: str, *, replace: bool = False) -> None:
    """Copy fixture ``fixture_name`` into ``$KIROCLAW_HOME``.

    Raises ``SeedError`` on guardrail violations. Enforces, in order:

    1. **Main-home guardrail** — refuses when ``$KIROCLAW_HOME`` resolves to
       ``~/.kiroclaw`` (the dev's live gateway home). This guardrail is
    ...
    # ``$KIROCLAW_HOME`` into its target and we'd copy/rmtree the wrong
    # location; conversely, the raw path can't be trusted for the main-home
    # guardrail because ``KIROCLAW_HOME=~/my-link`` where ``my-link -> ~/.kiroclaw``
    ...
    if dst_resolved == _main_home():
        raise SeedError(
            ...
            "Point $KIROCLAW_HOME at a separate dev directory "
            "(e.g. ~/.kiroclaw-dev).",
    ...
    if dst.is_symlink() and not dst.exists():
        raise SeedError(
            f"$KIROCLAW_HOME is a dangling symlink: {dst}. "
    ...
    # ``$KIROCLAW_HOME`` pointing at a regular file (or a symlink to one) would
    ...
    if dst.exists() and not dst.is_dir():
        raise SeedError(
            f"$KIROCLAW_HOME is not a directory: {dst}. "
    ...
    if dst.exists() and any(dst.iterdir()):
        ...
        if dst.is_symlink():
            raise SeedError(
                f"refusing to seed into a symlinked $KIROCLAW_HOME: {dst}. "
                "Point it at a real directory.",
                guardrail=SeedError.GUARDRAIL_SYMLINK_REPLACE,
            )
        if not replace:
            raise SeedError(
                f"$KIROCLAW_HOME is not empty: {dst}. "
...
def seed_cmd(args) -> int:  # noqa: ANN001 — argparse.Namespace at call site
    ...
    directory tree into ``$KIROCLAW_HOME`` and 1.B's ``--seed-replace`` will
    ...
    # this flag is always ``True`` on the success path (if ``KIROCLAW_HOME``
    ...
    target_set = "KIROCLAW_HOME" in os.environ
    try:

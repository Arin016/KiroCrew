"""Connect to a launched KiroClaw instance — SSM tunnel + token + open browser.
...
2. Mint a short-lived dashboard token on the instance (``kiroclaw token``) over
...
from kiro_claw.cloud import ssm
...
# `kiroclaw token` prints a URL like http://localhost:5476/?token=<jwt>;
...
def mint_token(instance_id: str, profile: str = "", region: str = "", *, ttl: str = "6h") -> str:
    ...
    Runs ``kiroclaw token --ttl <ttl>`` on the box and parses the token out of
    ...
    res = ssm.run_command(
        ...
        f"kiroclaw token --ttl {_safe_ttl(ttl)} 2>/dev/null | grep -m1 'token=' || true",
        profile,
        region,
        total_wait=90,
    )
    ...
    if not ready:
        ...
        try:
            ...
            error = (
                ...
                f"({exc}) — check `kiroclaw cloud status` / your IAM permissions, "
                "then retry."
            )
            _terminate(proc)
            return Connection(
        ...
        if not token:
            # The tunnel is up but we couldn't mint a token (e.g. `kiroclaw token`
            ...
            error = (
                ...
                "instance — check `kiroclaw cloud status` / the box's gateway, then retry."
            )
            _terminate(proc)
            ready = False
        else:
...
def _safe_ttl(ttl: str) -> str:
    """Constrain a TTL string to the ``<n>[hm]`` shape kiroclaw token accepts."""
...
def _browser_open_supported() -> bool:
    ...
    if os.environ.get("KIROCLAW_NO_BROWSER"):
        return False

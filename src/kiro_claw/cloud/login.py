...
+ code, and open that URL in the user's *local* browser. KiroClaw stores **no**
...
_LOGIN_LOG_PATH = "/tmp/kiroclaw-kiro-login.log"
_LOGIN_PID_PATH = "/tmp/kiroclaw-kiro-login.pid"
_LOGIN_FIFO_PATH = "/tmp/kiroclaw-kiro-login.stdin"
...
def social_login_hint(prompt: Optional[LoginPrompt]) -> str:
    ...
    return (
        ...
        f"KiroClaw could not automate the SSM port-forward for {ports}; "
        "run `kiroclaw cloud connect` and retry sign-in from the instance."
    )
...
def _open_browser(url: str) -> bool:
    ...
    try:
        ...
        from kiro_claw.cloud.connect import redact_token

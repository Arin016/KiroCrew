...
Every AWS interaction in :mod:`kiro_claw.cloud` goes through :func:`run_aws`.
...
Credentials are resolved by the ``aws`` CLI's own provider chain — KiroClaw
...
(:func:`kiro_claw.sandbox.wrap_argv`, ``mode="standard"``) exactly like the
...
class CloudActionDenied(Exception):
    ...
    ``deniedCommands`` block the ``aws`` CLI verbs and the ``kiroclaw cloud``
    wrappers, but an agent could still import ``kiro_claw.cloud`` and call a
...
def _in_agent_session() -> bool:
    ...
    return bool(os.environ.get("KIROCLAW_SESSION_KEY"))


def assert_human_action(action: str) -> None:
    """Refuse a destructive/sensitive cloud mutation from an agent session.

    The gateway injects ``KIROCLAW_SESSION_KEY`` into every agent/ACP subprocess
    (and MCP tools key off it). A human running ``kiroclaw cloud …`` in their own
    ...
    ``KIROCLAW_SESSION_KEY`` is an ordinary env var the agent's own code can
    ...
    credentials (see :mod:`kiro_claw.cloud.iam`); to truly wall the agent off
    ...
    if _in_agent_session():
        raise CloudActionDenied(
            ...
            "from an agent session — run `kiroclaw cloud …` yourself in a terminal."
        )
...
def assert_chokepoint_allowed(args: list[str]) -> None:
    ...
    in-process ``import kiro_claw.cloud`` path. All three are bypassable by a
    determined, code-executing agent (obfuscated shell string; ``del
    os.environ['KIROCLAW_SESSION_KEY']``; raw ``aws``/boto3 when ``~/.aws`` is
    ...
    credentials (:mod:`kiro_claw.cloud.iam` — tag/ARN-scoped, no IAM writes), plus
    ...
    if _in_agent_session() and (tuple(args[:2]) not in _AGENT_READ_ALLOWLIST):
        ...
        raise CloudActionDenied(
            ...
            "`kiroclaw cloud …` yourself in a terminal."
        )
    ...
    *,
    ...
    if rc != 0:
        ...
        if missing:
            ...
            hint = " — add the missing permission (see `kiroclaw cloud iam-policy`) and retry"
        raise AWSError(

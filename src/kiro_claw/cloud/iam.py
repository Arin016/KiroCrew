...
Same philosophy as ``deploy_web/iam.py``: **KiroClaw never performs an IAM
...
# kiroclaw:managed). Scopes the tag-conditioned statements below.
MANAGED_TAG_KEY = "kiroclaw:managed"

# The IAM role name pattern the template uses; PassRole is scoped to it.
ROLE_NAME_PREFIX = "kiroclaw-ec2-"
...
# permissive. iam:CreateRole is gated on this name so a kiroclaw-ec2-* role MUST
...
BOUNDARY_NAME = "kiroclaw-ec2-boundary"
...
def boundary_policy_document(account: str = "*") -> dict[str, Any]:
    ...
    launcher bucket prefix (``kiroclaw-src-<account>-*/*``) instead of the
    ...
    resource is region-agnostic (``kiroclaw-src-<account>-*``) — a byte-stable
    ...
    return {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Effect": "Allow",
                "Action": ["s3:GetObject"],
                "Resource": f"arn:aws:s3:::kiroclaw-src-{account}-*/*",
            },
        ],
    }
...
def policy_document() -> dict[str, Any]:
    ...
    statements: list[dict[str, Any]] = [
        {
            # Stack-mutating verbs scoped to kiroclaw-* stacks so the applied
        ...
        },
        {
            ...
            # BOTH the changeSet and stack ARN forms, still kiroclaw-*-scoped.
        ...
        {
            # `kiroclaw cloud list` discovers instances via the tagging API.
            "Sid": "TagDiscovery",
            "Effect": "Allow",
        ...
        },
        {
            ...
            # is evaluated per-resource, so requiring kiroclaw:managed=true on the
        ...
        },
        {
            ...
            # CreateSecurityGroup creates the SG (tagged kiroclaw:managed=true at
        ...
        {
            # SG rule mutation is gated to KiroClaw-tagged security groups: the
            # stack's SG carries kiroclaw:managed=true from creation
        ...
        },
        {
            ...
            # kiroclaw:managed=true and thereby bring it under the tag-gated
        ...
        {
            # Destructive verbs, gated to KiroClaw-tagged resources so a leaked
            # launcher credential can't delete/retag security groups it never
            # created. The stack's SG carries kiroclaw:managed=true from creation.
        ...
        },
        {
            ...
            # iam:CreateRole is the enforcement point: a kiroclaw-ec2-* role can
            # ONLY be created WITH our permissions boundary (iam:PermissionsBoundary
            # must ArnLike-match arn:...:policy/kiroclaw-ec2-boundary). Because the
        ...
        {
            # PutRolePolicy is scoped to the kiroclaw-ec2-* role ARN AND gated on
            # the role carrying our creation tag (aws:ResourceTag/
            # kiroclaw:managed=true). The name-prefix scope alone let a leaked
            # launcher credential target a PRE-EXISTING kiroclaw-ec2-* role that a
            ...
            # tagged kiroclaw:managed=true, and CreateRole is boundary-gated
        ...
        },
        {
            ...
            # unbounded kiroclaw-ec2-* role as kiroclaw:managed=true, then inline
        ...
        },
        {
            ...
            # 403 and the launch can't tag the role kiroclaw:managed=true (which the
            ...
            # kiroclaw-ec2-* role kiroclaw:managed=true, then inline admin + pass it
            # to EC2 (the tag is only non-spoofable if the same policy can't apply
            # it to an arbitrary role).
            #
            # Gate: aws:ResourceTag/kiroclaw:managed=true — the role must ALREADY be
            ...
            #     kiroclaw:managed=true tag is already "present" in context → match.
            #   * (b) A STANDALONE tag-role on a pre-existing unbounded victim
            #     (which does NOT yet carry kiroclaw:managed) is DENIED — the key is
        ...
        },
        {
            ...
            # admin (kiroclaw cloud iam-boundary) and drop this statement — the
        ...
        },
        {
            ...
            # a kiroclaw-ec2-* role and pass it to EC2 — a full escalation. The
        ...
        {
            # PassRole is scoped to the kiroclaw-ec2-* role ARN, gated on the EC2
            # service (iam:PassedToService) AND on our creation tag
            # (aws:ResourceTag/kiroclaw:managed=true) — the same non-spoofable
            # constraint as PutRolePolicy. Without the tag, a leaked launcher
            # credential could pass a PRE-EXISTING, unbounded kiroclaw-ec2-* role
        ...
        },
        {
            ...
            # and SendCommand (effectively RCE) — are gated to KiroClaw-tagged

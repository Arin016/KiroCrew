"""Ship the local KiroClaw source to the instance via S3 (private-repo safe).

The public KiroClaw repo may be private, so the EC2 box can't ``git clone`` it
...
is created once per account/region (``kiroclaw-src-<account12>-<region>``) and
reused; each launch uploads to ``<tag>/kiroclaw-src.tar.gz``.
"""
...
SOURCE_KEY_NAME = "kiroclaw-src.tar.gz"
_BUCKET_PREFIX = "kiroclaw-src-"
...
# keep out of the git-archive path — notably the KiroClaw dev-mode data dir
...
def repo_root() -> Path:
    """The KiroClaw source root to package (the installed package's repo).
    ...
    for parent in here.parents:
        if (parent / "install.sh").exists() and (parent / "setup.cfg").exists():
            return parent
    raise aws.AWSError(
        "could not locate the KiroClaw source root (no install.sh + setup.cfg "
        "above this module) — source shipping needs an editable/git checkout. "
        "Run `kiroclaw cloud launch` from a clone, or launch without S3 source "
...
def _use_git_archive(root: Path) -> Optional[Path]:
    ...
    try:
        out = tempfile.NamedTemporaryFile(  # noqa: SIM115 - handed to caller
            prefix="kiroclaw-src-", suffix=".tar.gz", delete=False
        )
        out.close()
...
def _custom_home_rel_parts(root: Path) -> Optional[tuple]:
    """``KIROCLAW_HOME``'s path parts relative to the repo root, if it's under it.

    Dev mode (AGENTS.md) allows a custom-named data dir (e.g. ``.kiroclaw-dev`` or
    ...
    actual (possibly nested) path, not just the hardcoded ``.kiroclaw*`` entries.
    Returns e.g. ``("data", "kc-home")`` for ``root/data/kc-home``; ``None`` when
    ``KIROCLAW_HOME`` is unset or resolves outside the repo (an absolute
    ``~/.kiroclaw`` isn't in the tarball anyway).
    """
...
def _tracked_tree_is_dirty(root: Path) -> bool:
    ...
    tracked file without committing, ``kiroclaw cloud launch`` would silently
...
def _tar_fallback(root: Path) -> Path:
    ...
    a custom ``KIROCLAW_HOME`` under the repo is dropped. If the tracked-file
    ...
    if tracked is None:
        raise aws.AWSError(
            ...
            ".gitignore) — run `kiroclaw cloud launch` from a git checkout, or "
...
def _account_from_bucket(bucket: str) -> str:
    """Extract the 12-digit account id embedded in a ``kiroclaw-src-<account>-<region>`` name.
    ...
    shape (e.g. the ``kiroclaw-src-unknown-*`` fallback), so the caller fails
...
def ensure_bucket(profile: str, region: str) -> str:
    ...
    if not account:
        ...
        # closed rather than mint an unprotected `kiroclaw-src-unknown-*` bucket.
        raise aws.AWSError(
    ...
    # A pre-existing kiroclaw-src-* bucket (older KiroClaw version, or one whose
...
def ensure_instance_boundary(profile: str = "", region: str = "") -> str:
    ...
    ``kiroclaw-ec2-boundary`` (see :mod:`cloud.iam`) — the launcher CODE creates
    ...
    **fetch its default version and compare it to the expected content-fixed
    ...
    (``kiroclaw cloud iam-boundary``) and drop the ``iam:CreatePolicy`` grant.
    """
    from kiro_claw.cloud import iam
    ...
    create = [
        ...
        iam.BOUNDARY_NAME,
        "--description",
        "KiroClaw EC2 instance permissions ceiling (SSM core + launcher source read).",
        "--policy-document",
...
def _verify_instance_boundary_content(arn: str, account: str, profile: str, region: str) -> None:
    ...
    An existing ``kiroclaw-ec2-boundary`` must ONLY be reused if its content is
    ...
    if _canonical(actual) != _canonical(expected):
        raise aws.AWSError(
            ...
            "or re-run `kiroclaw cloud iam-boundary`.",
            action="iam:GetPolicyVersion",
        )
...
def delete_instance_boundary(profile: str = "", region: str = "") -> dict:
    ...
    call this only after every kiroclaw-ec2-* role is gone.
    """
    from kiro_claw.cloud import iam
...
def delete_source(tag: str, profile: str = "", region: str = "") -> dict:
    ...
    # back to `kiroclaw-src-unknown-*` because the account didn't resolve), fail

...
def run_command(
    ...
    *,
    ...
    it runs as the KiroClaw user with a login shell (PATH, home). Blocks until
...
def session_manager_plugin_install_hint() -> str:
    ...
    return (
        ...
        f"Session Manager plugin, then rerun `kiroclaw cloud connect`: "
...
def install_session_manager_plugin() -> PluginInstallResult:
    ...
    with tempfile.TemporaryDirectory(prefix="kiroclaw-ssm-plugin-") as tmp:
        tmpdir = Path(tmp)

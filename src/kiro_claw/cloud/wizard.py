"""The interactive ``kiroclaw cloud launch`` wizard.

One command → answer at most a question or two → KiroClaw running on a
...
from kiro_claw.cloud import ec2, iam, login, sizes, ssm, ui
from kiro_claw.cloud.aws import AWSError
from kiro_claw.cloud.config import DEFAULT_REGION, CloudConfig
...
def _deploy_with_progress(
    *,
    ...
    exists, the tail of ``/var/log/kiroclaw-setup.log`` over SSM. This replaces
...
def _ensure_running_and_ssm_ready(instance_id: str, profile: str, region: str) -> bool:
    ...
    if state in ("stopped", "stopping"):
        ...
        if state == "terminated":
            ui.fail("the saved instance is terminated — run `kiroclaw cloud launch --new`.")
            return False
    ...
    ui.fail(
        ...
        "`kiroclaw cloud status`; the agent may still be starting — retry shortly."
    )
    return False
...
def aws_start_instance(instance_id: str, profile: str, region: str) -> None:
    ...
    from kiro_claw.cloud import aws as _aws
...
def _verify_operational(
    ...
    try:
        ...
        ui.detail(f"{exc} — check later with: kiroclaw cloud login")
        return False
...
def _fetch_bootstrap_log(
    ...
    try:
        res = ssm.run_command(
            instance_id,
            "tail -n %d /var/log/kiroclaw-setup.log 2>/dev/null "
            "| tr -d '\\r' "
            "| grep -avE 'Installing (npm|kiroclaw and) depend|building React app' || true" % lines,
            profile,
            region,
            run_as="root",
            total_wait=30,
...
def launch(
    ...
    *,
    ...
    on it — used when the wizard is embedded in a larger flow (``kiroclaw
    ...
    if not profile:
        ...
        ui.detail("KiroClaw never stores AWS credentials — the aws CLI resolves them.")
    ...
    ui.detail("  kiroclaw cloud iam-policy   → prints the least-privilege policy")
    ...
    try:
        ...
        if exc.missing_action:
            ui.detail(f"Grant `{exc.missing_action}` (see `kiroclaw cloud iam-policy`) and retry.")
        return 1
    ...
    if result is None:
        ...
        ui.info("Provisioning EC2 + installing KiroClaw (this takes a few minutes)…")
        try:
            ...
            if exc.missing_action:
                ui.detail(
                    f"Grant `{exc.missing_action}` (see `kiroclaw cloud iam-policy`) and retry."
                )
            else:
                ...
                if not keep_on_failure:
                    ui.detail(
                        ...
                        "inspection (`kiroclaw cloud launch --keep-on-failure`)."
                    )
            return 1
        ...
        cfg.profile, cfg.region, cfg.last_tag = profile, region, tag
        cfg.save()
        ui.ok(f"Instance {result.instance_id} is up and KiroClaw is healthy.")
    elif not result.instance_id:
        ui.warn("Previous cloud stack exists but the instance is not ready yet.")
        ui.detail("Check progress with: kiroclaw cloud status")
        return 1
    else:
        ui.ok(f"Resuming instance {result.instance_id}.")
        # A resumed stack may be STOPPED (after `kiroclaw cloud stop`). All the
    ...
    if login.is_logged_in(instance_id, profile, region):
        ...
        if prompt.already_logged_in:
            ...
            if signed:
                ...
                ui.detail("Re-run: kiroclaw cloud connect (then sign in from the dashboard/SSM).")
        else:
            prompt.close()
    ...
    if not _verify_operational(instance_id, profile, region, assume_yes=assume_yes):
        ui.warn(
            ...
            "sign in. Run: kiroclaw cloud login"
        )
    ...
    connect_mod.register_instance(instance_id, name=f"KiroClaw Cloud ({result.tag})")

    # ── Done ──────────────────────────────────────────────────────────────
    print()
    dashboard_ready = bool(conn and conn.ready and conn.url)
    if dashboard_ready:
        ui.note(f"{ui.GREEN}{ui.BOLD}KiroClaw is live on AWS.{ui.RESET}")
    else:
        ui.warn("KiroClaw is running on AWS, but the dashboard tunnel is not open.")
        ui.detail("Fix the local SSM tunnel issue, then run: kiroclaw cloud connect")
    print()
    ui.note(f"{ui.BOLD}Manage it:{ui.RESET}")
    ui.detail("kiroclaw cloud status            # state + cost estimate")
    ui.detail("kiroclaw cloud connect           # reopen the dashboard")
    ui.detail("kiroclaw cloud stop | start      # pause / resume (saves cost)")
    ui.detail("kiroclaw cloud destroy           # remove EVERYTHING from AWS")
    print()
    ...
    # When embedded in `kiroclaw setup` (hold_tunnel=False) we must return so
    ...
    if conn and conn.ready and conn.process is not None and conn.process.poll() is None:
        if not hold_tunnel:
            conn.close()
            ui.detail("Dashboard tunnel closed — reopen anytime: kiroclaw cloud connect")
        else:
            ui.detail(
                "Keeping the dashboard tunnel open — press Ctrl+C to exit "
                "(reopen later with `kiroclaw cloud connect`)."
            )
            try:
                conn.process.wait()
            except KeyboardInterrupt:
                conn.close()
                ui.info("Tunnel closed. KiroClaw keeps running on AWS.")
...
def _ensure_session_manager_plugin(*, assume_yes: bool = False) -> bool:
    ...
    ui.detail("KiroClaw can install AWS's official Session Manager plugin locally.")
    if not assume_yes and not ui.confirm("Install session-manager-plugin now?", default=True):
        ui.detail("Install it later with: kiroclaw cloud doctor")
        return False
...
def _choose_existing_launch(
    ...
    if assume_yes:
        if len(launches) == 1 or cfg.last_tag:
            ...
            ui.info(
                f"Existing KiroClaw cloud stack found: {chosen.stack_name}. "
                "Keeping it by default."
            )
            ui.detail("Use `kiroclaw cloud launch --new` to create a separate instance.")
            return chosen
        tags = ", ".join(launch.tag for launch in launches)
        raise AWSError(
            "multiple existing KiroClaw cloud stacks found; rerun interactively "
            f"to choose one ({tags}), or pass `kiroclaw cloud launch --new` "
    ...
    if len(launches) == 1:
        launch = launches[0]
        idx = ui.choose(
            "Existing KiroClaw cloud deployment",
            [
    ...
    idx = ui.choose("Existing KiroClaw cloud deployments", options, default_index=0)
    if idx < len(launches):
        return launches[idx]

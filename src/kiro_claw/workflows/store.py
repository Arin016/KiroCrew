...
try:
    ...
    KiroClawConfig = None  # type: ignore[assignment,misc]
...
def default_workflows_dir() -> Path:
    ...
    ``<config_dir>/workflows`` (e.g. ``~/.kiroclaw/workflows``, or the
    ``KIROCLAW_HOME`` override used by the dev instance).
    """
    # KiroClawConfig is an optional dependency resolved at module top (gate F1);
    # when present, honor workflows.dir, else fall back to config_dir()/workflows.
    if KiroClawConfig is not None:
        try:

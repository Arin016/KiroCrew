...
def ensure_utf8_console() -> None:
    """Make stdout/stderr UTF-8 on Windows so KiroClaw's emoji output can't crash it.

    KiroClaw prints non-ASCII glyphs throughout its CLI/gateway output. On
    ...
    KiroClawHub client) Python encodes prints as cp1252 — so the FIRST non-ASCII
    ...
    for name in ("stdout", "stderr"):
        ...
        try:
            ...
            # observed in the 3-layer Windows gateway spawn: kiroclaw.exe launcher
...
def find_listening_pids(port: int) -> list[int]:
    ...
    # silently drops IPv6 listeners entirely, so `kiroclaw stop` /
    # `kiroclaw restart` no-op on a dual-stack or `[::]`-bound gateway
...
def process_command_line(pid: int) -> str:
    ...
    Used to confirm a listener PID is actually a KiroClaw gateway when the image
    name alone is ambiguous (the venv ``kiroclaw.exe`` re-execs ``python.exe``).
    """
    try:
        if sys.platform == "linux":
...
# POSIX execute bit). A hook is a small script KiroClaw shells out to; on
...
def is_executable_file(path: str | os.PathLike) -> bool:
    ...
    determined when KiroClaw actually invokes it, not by a meaningless bit.
    """
    try:
        if IS_POSIX:
...
def find_python_interpreter(reject: Optional[Callable[[str], bool]] = None) -> str | None:
    ...
    need *an* interpreter to re-exec KiroClaw itself should prefer

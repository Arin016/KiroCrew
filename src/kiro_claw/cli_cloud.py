"""CLI ``kiroclaw cloud`` command group — thin dispatchers into :mod:`cloud`.
...
from kiro_claw.cloud import ec2, iam
from kiro_claw.cloud import login as login_mod
from kiro_claw.cloud import sizes, ssm, ui, wizard
from kiro_claw.cloud.aws import AWSError, CloudActionDenied
...
def _resolve_tag(args: argparse.Namespace) -> str:
    ...
    if not cfg.last_tag:
        ...
        ui.detail("Pass --tag <tag>, or run `kiroclaw cloud list` to see instances.")
        sys.exit(1)
    return cfg.last_tag
...
def _cloud_list(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    if not rows:
        ui.info("No KiroClaw cloud instances found.")
        ui.detail("Launch one with: kiroclaw cloud launch")
        return 0
    ui.note(f"{ui.BOLD}KiroClaw cloud instances ({region}):{ui.RESET}")
    for r in rows:
...
def _cloud_connect(args: argparse.Namespace) -> int:
    ...
    if conn.ready and conn.url:
        if not conn.token:
            ...
            ui.detail("The page will ask for a token. Retry: kiroclaw cloud connect")
...
def _cloud_login(args: argparse.Namespace) -> int:
    ...
    if signed:
        ui.ok(
            ...
            "was already open: kiroclaw cloud connect"
        )
        return 0
    ui.warn(
...
def _cloud_stop(args: argparse.Namespace) -> int:
    ...
    ui.detail("Resume with: kiroclaw cloud start")
    return 0
...
def _cloud_start(args: argparse.Namespace) -> int:
    ...
    ui.detail("Reopen the dashboard with: kiroclaw cloud connect")
    return 0
...
def _cloud_destroy(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "yes", False):
        if not ui.confirm(f"Remove KiroClaw instance '{tag}' from AWS?", default=False):
    ...
    if not res.get("destroyed"):
        ...
        ui.detail("Check `kiroclaw cloud status` (and the AWS console); re-run destroy if needed.")
        return 1
...
def _cloud_doctor(args: argparse.Namespace) -> int:
    ...
    profile, region = _resolve(args)
    ui.note(f"{ui.BOLD}KiroClaw cloud — diagnostics{ui.RESET}")
...
def handle_cloud(args: argparse.Namespace) -> int:
    """Entry point for ``kiroclaw cloud <action>``."""
    action = getattr(args, "cloud_action", None)
    if not action:
        ui.note(f"{ui.BOLD}kiroclaw cloud{ui.RESET} — run KiroClaw on your own AWS EC2")
        print()
        ui.detail("launch      Provision + configure an instance (interactive)")
        ui.detail("list        List your KiroClaw cloud instances")

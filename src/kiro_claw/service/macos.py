...
The plist lives at ``~/Library/LaunchAgents/dev.kiroclaw.gateway.plist``
...
LOG_DIR = Path.home() / "Library" / "Logs" / "KiroClaw"
...
def render_plist() -> str:
    ...
    bin_path = _xml_escape(kiroclaw_bin())
...
def status() -> str:
    ...
    res = _launchctl("list", LAUNCHD_LABEL)
    if res.returncode != 0:
        return f"kiroclaw service is not loaded ({res.stderr.strip() or 'no entry'})\n"
    return res.stdout

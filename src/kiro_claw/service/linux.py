...
The unit lives at ``/etc/systemd/system/kiroclaw.service`` and is
...
from kiro_claw.service.common import (
    SERVICE_NAME,
    kiroclaw_bin,
    service_path,
)
...
def render_unit() -> str:
    ...
    has access to ``$HOME/.kiroclaw``, the user's config, etc. The PATH
    is set explicitly so subprocess invocations of git, node, etc.
    resolve the same way they would from an interactive shell.
    """
    bin_path = kiroclaw_bin()
    user = _current_user()
    group = _current_group(user) if user else ""
    home = str(Path.home())
    return (
        "[Unit]\n"
        "Description=KiroClaw gateway (dashboard + Slack + cron)\n"
        "Documentation=https://github.com/kiroclaw/kiroclaw\n"
...
def _write_unit_via_sudo(contents: str) -> subprocess.CompletedProcess[str]:
    ...
    ``daemon-reload``.
    """
    fd, tmp_path = tempfile.mkstemp(prefix="kiroclaw-unit-", suffix=".service")
    try:
...
def install() -> None:
    ...
    No kiroclaw / LLM / agent code runs under sudo: only ``tee`` and
...
def status() -> str:
    ...
    ``kiroclaw service status`` doesn't prompt for a password just to

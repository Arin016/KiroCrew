...
importing :mod:`kiro_claw.service.linux` or :mod:`kiro_claw.service.macos`
...
from kiro_claw.service import linux, macos
from kiro_claw.service.common import Platform, current_platform


def _unsupported_message() -> None:
    print(
        "❌ kiroclaw service management is only supported on Linux (systemd)\n"
        "   and macOS (launchd). On other platforms run `kiroclaw gateway`\n"
...
def install_service() -> int:
    ...
    ``/etc/systemd/system/kiroclaw.service`` and to run
    ``systemctl daemon-reload / enable / restart``. The gateway itself
    runs as ``User=$USER`` once started — kiroclaw code is never
    ...
    if plat == Platform.SYSTEMD:
        try:
            ...
            return 1
        print("✅ kiroclaw service installed and started.")
        print(f"   unit: {linux.UNIT_PATH}")
        print()
        print("   Status: kiroclaw service status")
        print("   Logs:   kiroclaw logs -f")
        print("   Remove: kiroclaw service uninstall")
        return 0
...
def uninstall_service() -> int:
    ...
    if plat == Platform.SYSTEMD:
        linux.uninstall()
        print("✅ kiroclaw service stopped and removed.")
        return 0
    if plat == Platform.LAUNCHD:
        macos.uninstall()
        print("✅ kiroclaw service stopped and removed.")
...
def is_service_active() -> bool:
    """Return True if a kiroclaw service is installed and currently running."""
    plat = current_platform()
    if plat == Platform.SYSTEMD:

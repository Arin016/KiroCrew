...
SERVICE_NAME = "kiroclaw"  # systemd unit name (without .service)
LAUNCHD_LABEL = "dev.kiroclaw.gateway"  # launchd Label


def kiroclaw_bin() -> str:
    """Return the resolved kiroclaw executable path, or fall back to sys.argv[0].
    ...
    installs where ``kiroclaw`` isn't on the global PATH.
    """
...
def service_path(home: str) -> str:
    ...
    interactive shell that ran ``kiroclaw service install``. Common

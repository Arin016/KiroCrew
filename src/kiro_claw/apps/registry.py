"""App registry — curated list of available KiroClaw apps.
...
own ``app.json`` — they never need to update the KiroClaw registry JSON
...
from kiro_claw.apps.manager import list_apps as list_installed_apps
from kiro_claw.apps.manager import (
    set_app_source,
    update_app,
)
...
async def _fetch_app_manifest(
    ...
    Tries, in order:
      1. The persistent clone under ``~/.kiroclaw/app-sources/{app_name}/``
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="kiroclaw-manifest-")
        clone_cmd = [
            "git",
            "clone",
            "--depth",
...
def _enrich_with_install_status(
    ...
    *detected* is a set of app names that were found via ``detectInstalled``
    shell commands (installed outside KiroClaw's app manager).
    """
...
async def _fetch_external_registry_index(
    ...
    *repo* is a git-cloneable URL (https/ssh/git/scp-style).  The repo is
    ...
    tmp_root: str | None = None
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="kiroclaw-registry-")
        clone_cmd = [
            "git",
            "clone",
            "--depth",
...
async def _load_external_registries() -> list[dict[str, Any]]:
    ...
    config = await asyncio.to_thread(KiroClawConfig.load)
...
async def list_registry() -> list[dict[str, Any]]:
    ...
    5. Enrich with install status from KiroClaw's app manager
    """
...
def get_server_platform() -> dict[str, str]:
    ...
    from kiro_claw.apps.manifest import PlatformConfig
...
def app_source_dir(name: str) -> Path:
    """Return ~/.kiroclaw/app-sources/{name}/ — persistent clone directory."""
...
async def _clone_build_app(
    ...
    Source is cloned to ``~/.kiroclaw/app-sources/{app_name}/`` (persistent;
...
async def install_from_registry(
    ...
    Source code is cloned to ``~/.kiroclaw/app-sources/{name}/`` (persistent,
    survives reboots, used by app update scripts).

    For self-managed apps (``managed: "self"`` in registry), only the clone +
    install script is run — KiroClaw does NOT copy files to ``~/.kiroclaw/apps/``
    or register resources via bridges.  The app registers itself at runtime.

    For kiroclaw-managed apps, files are copied to ``~/.kiroclaw/apps/{name}/``
    ...
    2. Clone the repo to ~/.kiroclaw/app-sources/{name}/ (timeout: 60s)
    3. Build it (npm/pip, auto-detected) then run the install script from
       app.json if any (timeout: 300s)
    4. For kiroclaw-managed: call install_app() or update_app()
    ...
    # Validate minKiroClawVersion if declared
    min_version = (manifest or {}).get("minKiroClawVersion", "")
    if min_version:
        ...
        try:
            ...
            if proc.returncode == 0:
                return {
                    ...
                    f"Launch it to register with KiroClaw automatically.",
                }
        except (asyncio.TimeoutError, OSError):
            pass
    ...
    try:
        ...
        if deps_data and isinstance(deps_data, dict):
            ...
            from kiro_claw.apps.manifest import Dependencies as _Deps
        ...
        # Kiroclaw-managed: copy to ~/.kiroclaw/apps/ and register resources

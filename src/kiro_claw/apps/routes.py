"""App management REST API endpoints for the KiroClaw dashboard.
...
from kiro_claw.apps.hooks_integration import on_app_disable, on_app_enable
...
from kiro_claw.apps.manifest import Dependencies
...
def _check_min_version(manifest_data: dict[str, Any]) -> str | None:
    """Check if the app requires a newer KiroClaw version.

    Returns an error message if the current version is too old, or None if OK.
    """
    return _check_min_version_str(manifest_data.get("minKiroClawVersion", ""))
...
async def handle_install_app(request: web.Request) -> web.Response:
    ...
    # Check minKiroClawVersion before installing
...
async def handle_register_external(request: web.Request) -> web.Response:
    ...
    KiroClaw only tracks metadata so the dashboard can display them.
...
async def handle_uninstall_preview(request: web.Request) -> web.Response:
    ...
    from kiro_claw.apps.dependency_ledger import classify_for_uninstall
...
async def handle_uninstall_app(request: web.Request) -> web.Response:
    ...
    if not keep_dependencies:
        ...
        from kiro_claw.apps.dependency_ledger import classify_and_clean_for_uninstall
        ...
        if removable:
            from kiro_claw.apps.dependencies import clean_dependencies
    ...
    if is_registry_source(info.get("source", "")):
        ...
        if app_reg_name:
            from kiro_claw.apps.registry import app_source_dir
...
async def handle_disable_app(request: web.Request) -> web.Response:
    ...
    if name in BUILTIN_NAMES:
        try:
            mod = importlib.import_module(f"kiro_claw.apps.builtins.{name}")
...
async def handle_open_app(request: web.Request) -> web.Response:
    ...
    if not is_local:
        return web.json_response({
            ...
            "remote": True,
            "command": open_cmd,
            "message": f"KiroClaw is running remotely. Run this on your local machine: {open_cmd}",
        })
...
async def handle_app_config(request: web.Request) -> web.Response:
    ...
    Reads/writes ``~/.kiroclaw/apps/{name}/data/config.json``.
    ...
    from kiro_claw.apps.manager import app_data_dir
    from kiro_claw.atomic_write import atomic_write
...
async def _fetch_git_blob(repo: str, ref: str, file_path: str, cache_path: Path) -> bool:
    ...
    (mirroring how :mod:`kiro_claw.apps.registry` already clones), read the
    ...
    try:
        tmp_root = await asyncio.to_thread(tempfile.mkdtemp, prefix="kiroclaw-blob-")
        clone_cmd = [
            "git",
            "clone",
            "--depth",
...
async def handle_blob_proxy(request: web.Request) -> web.Response:
    ...
    Caches fetched blobs to ~/.kiroclaw/cache/blobs/{repo}/{ref}/{path}.
...
# Strip sensitive auth headers — app backends use X-KiroClaw-Proxy HMAC, not user cookies
    ...
    for _server_name, server_cfg in manifest.mcpServers.items():
        if isinstance(server_cfg, dict):
            ...
            if url.startswith("http"):
                ...
                gateway_port = int(os.environ.get("KIROCLAW_PORT", "7777"))

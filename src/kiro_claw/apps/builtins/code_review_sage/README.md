...
A self-evolving deep code reviewer packaged as a built-in KiroClaw app. Reviews
...
App-local runtime data lives under `~/.kiroclaw/apps/code-review-sage/data/`
(honors `KIROCLAW_HOME`; created on first use, never committed).

## Enable

Code Review Sage is a built-in app (listed in `kiro_claw.apps.builtins`). Enable
it from the dashboard App Store, or:

```bash
kiroclaw app enable code-review-sage
```

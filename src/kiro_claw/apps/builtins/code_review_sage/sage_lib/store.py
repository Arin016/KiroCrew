...
on-disk layout under ``~/.kiroclaw/apps/code-review-sage/data/`` and is safe to
...
def app_root() -> Path:
    """Resolve the installed app root under the KiroClaw home dir.

    Honors ``KIROCLAW_HOME`` (the canonical override used by
    ``kiro_claw.config.paths``); defaults to ``~/.kiroclaw``. Kept free of a
    hard ``kiro_claw`` import so the store stays importable standalone."""

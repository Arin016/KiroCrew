...
# Optional KiroClaw redaction — used to scrub LLM-generated text before it is
# posted to an external surface. Imported at module top (absent only when run
# fully standalone outside the KiroClaw runtime).
try:
...
def resolve_rule_pack(pack_name: str) -> str | None:
    """Find a rule-pack SKILL.md by skill name under the KiroClaw skills dir
    (``~/.kiroclaw/skills/``, honoring ``KIROCLAW_HOME`` — this is where the app
    ...
    skills = (Path(home) if home else Path.home() / ".kiroclaw") / "skills"

...
# Optional KiroClaw runtime dep (absent when running standalone / in tests).
# Kept at module top per the imports guideline; guarded at each use site.
try:
    from kiro_claw.security import redact_credentials, redact_exfiltration_urls  # type: ignore
...
def _redact(text: str) -> str:
    ...
    KiroClaw redaction lib isn't importable (standalone)."""
...
def _candidate_ports() -> list[int]:
    """Ports to try for the live gateway: KIROCLAW_PORT, config.json dashboard.url,
    ...
    _add(os.environ.get("KIROCLAW_PORT"))
    try:
        home = os.environ.get("KIROCLAW_HOME") or os.path.expanduser("~/.kiroclaw")
...
def _probe(base: str, secret: str) -> bool:
    """True if a KiroClaw gateway is listening at base (any HTTP response, incl.

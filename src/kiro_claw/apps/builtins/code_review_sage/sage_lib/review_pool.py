...
# The reusable pool engine (provider-agnostic; see kiro_claw/acp/worker_pool.py).
...
from kiro_claw.acp.worker_pool import PoolWorker as Worker  # noqa: E402,F401  (back-compat alias)
from kiro_claw.acp.worker_pool import WorkerFactory, WorkerPool  # noqa: E402,F401
...
_FALLBACK_AGENT = "kiroclaw"     # default agent when the reviewer agent isn't installed
...
try:
    from kiro_claw.config.loader import DEFAULT_MODEL as _SYSTEM_DEFAULT_MODEL
...
# Valid concrete effort levels — sourced from kiro_claw.effort (single source of
# truth), not a hardcoded list. "" (inherit default) is handled separately.
try:
    from kiro_claw.effort import EFFORT_LEVELS as VALID_EFFORTS
...
def _resolve_review_agent(preferred: str = REVIEW_AGENT) -> str:
    ...
    `kiroclaw` agent. GitHub posting runs the `gh` CLI, so the chosen agent needs
...
def _review_work_dir() -> Optional[str]:
    ...
    default cwd (~/.kiroclaw/workspace) sends the result record to the wrong dir
    ...
    try:
        ...
        try:
            base = os.environ.get("KIROCLAW_HOME") or str(Path.home() / ".kiroclaw")
...
def _write_effort_overlay(work_dir: str, model: str, effort: str = REVIEW_EFFORT) -> None:
    ...
    ``kiro_claw/providers/acp.py:_write_cli_overlay``)::
...
class AcpReviewWorker:
    ...
    async def start(self) -> None:
        if AcpClient is None:
            raise RuntimeError("AcpClient unavailable (kiro_claw.acp.client not importable)")
...
class ReviewPool(WorkerPool):
    ...
    :class:`kiro_claw.acp.worker_pool.WorkerPool` engine. The engine owns the

...
class TestReviewAgentResolution(unittest.TestCase):
    """The pool prefers the dedicated reviewer agent but degrades to kiroclaw
    when it isn't installed (older builds)."""

    def test_fallback_to_kiroclaw_when_dedicated_missing(self):
        # A name with no ~/.kiro/agents/<name>.json on disk -> fall back.
        self.assertEqual(
            _resolve_review_agent("definitely-not-installed-xyz"), "kiroclaw")

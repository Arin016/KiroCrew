...
the existing bucket + distribution by the ``kiroclaw:site`` tag and reuses them
(idempotent), so the public URL stays stable. The physical bucket name is an
opaque ``kiroclaw-web-<random>`` (no account id); idempotency never relies on
re-deriving it.
"""
...
BUCKET_PREFIX = "kiroclaw-web-"
TAG_MANAGED = "kiroclaw:managed"
TAG_SITE = "kiroclaw:site"
...
def run_aws(args: list[str], profile: str, timeout: int = 30) -> tuple[int, str, str]:
    ...
    resolved by the CLI's own provider chain; KiroClaw passes only the profile name.
...
def find_site_by_tag(site_id: str, profile: str, region: str = DEFAULT_REGION) -> Optional[dict[str, str]]:
    """Resolve an existing site's bucket + distribution by the kiroclaw:site tag.
...
def list_sites(profile: str, region: str = DEFAULT_REGION) -> list[dict[str, Any]]:
    ...
    Groups managed resources by the kiroclaw:site tag; status/domain read live

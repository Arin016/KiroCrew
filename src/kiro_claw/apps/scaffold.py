"""App scaffolding — generate a new app skeleton with `kiroclaw app init`.
...
_BACKEND_TEMPLATE = '''"""Minimal backend for {name} — a KiroClaw app.

Run with: python backend/server.py
Or let KiroClaw manage it via the app manifest backend section.
"""
import json
import os
...
APP_NAME = os.environ.get("KIROCLAW_APP_NAME", "{name}")
  ...
  build: {{
    ...
    rollupOptions: {{
      external: [
        ...
        '@kiroclaw/app-sdk', '@kiroclaw/app-sdk/ui', 'lucide-react',
      ],
    }},
  }},
}})
...
import {{ useAppApi }} from '@kiroclaw/app-sdk'
...
```bash
kiroclaw app install /path/to/{name}
kiroclaw app enable {name}
```
...
def scaffold_app(
    ...
    *,
    ...
    if not description:
        description = f"A KiroClaw app: {display_name}"
    if not author:
        import os

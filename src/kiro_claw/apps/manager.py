"""App Manager — install, uninstall, enable, disable lifecycle for KiroClaw apps.

Apps are installed to ``~/.kiroclaw/apps/{name}/``.  Each installed app has an
...
def app_data_dir(name: str) -> Path:
    """Return the app-scoped data directory: ``~/.kiroclaw/apps/{name}/data/``."""
    d = app_dir(name) / "data"
...
class InstalledApp:
    ...
    ``origin`` — where the app came from (read-only, set at install time):
      - ``"builtin"``: baked into the KiroClaw dashboard
    ...
    ``resources`` — who manages agent/skill/cron registration:
      - ``"gateway"``: KiroClaw manages via bridges.py symlinks
      - ``"app"``: the app manages its own resource registration

    ``lifecycle`` — who manages updates and uninstall:
      - ``"gateway"``: KiroClaw handles updates and uninstall
...
def register_external_app(
    ...
    *,
    ...
    """Register a self-managed app with KiroClaw's app system.

    Self-managed apps (``resources="app"``) handle their own agent/skill/MCP
    registration.  KiroClaw only tracks metadata so the dashboard can display them.
...
# Built-in apps are features baked into the KiroClaw dashboard that we
    ...
    for app_data in all_builtins:
        ...
        if app_data.get("backend", {}).get("entryPoint"):
            ...
            try:
                ...
                # kiro_claw.apps.routes imports from kiro_claw.apps.manager

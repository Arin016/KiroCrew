...
leaf). It does NOT import ``kiro_claw.slack`` or ``kiro_claw.dashboard``.
...
``Renderer`` (``kiro_claw.slack.renderer``); the native loop is rewired onto
...
from kiro_claw.security import StreamRedactor, redact_credentials, redact_exfiltration_urls

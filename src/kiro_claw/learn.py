...
Lessons are saved via the ``kiroclaw learn`` CLI (called by the LLM via bash)
and loaded into every session's context alongside memory and skills.

Storage: ``~/.kiroclaw/lessons.jsonl`` (append-only JSONL).
"""
...
try:
    from kiro_claw.config.loader import config_dir as _config_dir
except ImportError:

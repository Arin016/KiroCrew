...
Usage:
    kiroclaw browse setup              # Generate Playwright MCP config (OSS)
    kiroclaw browse auth health        # not available in OSS
    kiroclaw browse auth inject        # not available in OSS
    kiroclaw browse auth refresh       # not available in OSS
    kiroclaw browse auth federate <url># not available in OSS
"""
...
from kiro_claw.browser import setup as _setup
...
def _print_help() -> None:
    print("""kiroclaw browse — Playwright MCP browser setup
...
def _cmd_setup() -> None:
    ...
    print("Done. Wrote Playwright MCP config to ~/.kiroclaw/playwright-config.json.")
...
def _cmd_extension(action: str) -> None:
    ...
    if action == "on":
        ...
        print("Restart gateway to apply: kiroclaw stop && kiroclaw gateway")
    elif action == "off":
        ...
        print("Usage: kiroclaw browse extension <on|off>", file=sys.stderr)
        sys.exit(1)

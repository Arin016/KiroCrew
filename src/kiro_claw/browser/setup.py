...
def has_playwright_extension() -> bool:
    ...
    """
    flag_file = Path.home() / ".kiroclaw" / "playwright-extension-mode"
    return flag_file.exists()


def get_extension_token() -> str | None:
    """Read the stored Playwright extension token."""
    token_file = Path.home() / ".kiroclaw" / "playwright-extension-token"
...
def generate_playwright_config() -> Path:
    """Generate ~/.kiroclaw/playwright-config.json with correct absolute paths.

    The open-source build ships a generic Chromium config with no
    Amazon-internal auth-server allowlist.
    """
    config_path = Path.home() / ".kiroclaw" / "playwright-config.json"
    config_path.parent.mkdir(parents=True, exist_ok=True)

    storage_state = str(Path.home() / ".kiroclaw" / "playwright-storage-state.json")
...
def refresh_storage_state() -> dict[str, Any]:
    ...
    storage_state_path = Path.home() / ".kiroclaw" / "playwright-storage-state.json"
...
def _kiroclaw_bin() -> str:
    """Resolve path to the kiroclaw binary."""
...
def patch_mcp_headless() -> None:
    ...
    try:
        data = json.loads(mcp_json.read_text(encoding="utf-8"))
        servers = data.setdefault("mcpServers", {})
        config_path = str(Path.home() / ".kiroclaw" / "playwright-config.json")
        entry = {
            "command": _kiroclaw_bin(),

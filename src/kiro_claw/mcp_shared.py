...
def _resolve_excluded_tools() -> set[str]:
    ...
    if (
        ...
        sel().log_api_access(
            caller=os.environ.get("KIROCLAW_SESSION_KEY", "mcp"),
...
def respond(req_id: Any, result: Any, error: dict | None = None) -> None:
    ...
    if error:
        ...
        resp["result"] = result
    from kiro_claw.validation import ValidationError, validate_jsonrpc_response
...
def run_mcp_stdio_loop(
    ...
    from kiro_claw.validation import ValidationError, build_tool_response, validate_jsonrpc_request
    ...
    while True:
        ...
        if method == "initialize":
            ...
            if tool_name in excluded:
                sel().log_tool_invocation(
                    session_key=os.environ.get("KIROCLAW_SESSION_KEY", "mcp"),
                    source="mcp",
                    tool_name=tool_name,

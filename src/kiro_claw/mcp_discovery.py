...
# Configurable via dashboard.mcp_probe_timeout_secs in ~/.kiroclaw/config.json.
...
# the dashboard badges (kiroclaw / kiroGlobal / ccGlobal) and are the
# source of truth for the ``presence`` field on each server.
SCOPE_KIROCLAW = "kiroclaw"
...
# order in :func:`_load_mcp_json` (kiroclaw > cc global > kiro global),
# not by this tuple's order.
_MCP_SOURCES: tuple[tuple[Path, str], ...] = (
    (Path.home() / ".kiroclaw" / "mcp.json", SCOPE_KIROCLAW),
...
def _load_agent_config() -> dict[str, Any]:
    ...
    AND the installed kiroclaw.json — because defaults.json may not have
    mcpServers (they're added dynamically at install time by ``kiroclaw setup``).
    """
    ...
    from kiro_claw.agent import AGENT_FILENAME  # circular import: agent imports mcp_discovery
...
def _load_mcp_json_by_source() -> dict[str, dict[str, Any]]:
    ...
    unknown path falls back to :data:`SCOPE_KIROCLAW`.
    """
    ...
    for p in _MCP_JSON_PATHS:
        scope = path_to_scope.get(p, SCOPE_KIROCLAW)
        if not p.is_file():
            continue
        try:
...
def _load_mcp_json() -> dict[str, Any]:
    ...
    # kiroclaw-specific file > CC global > kiro global. Matches
    # rebuild_agent_config's merge order in agent.py.
    for scope in (SCOPE_KIROCLAW, SCOPE_CC_GLOBAL, SCOPE_KIRO_GLOBAL):
...
def list_servers() -> list[McpServerInfo]:
    ...
    #    don't overwrite it.  Order = kiroclaw-specific > CC global >
    ...
    #
    #    MC presence = "will this load in KiroClaw sessions after the next
    ...
    #    KiroClaw has an explicit ``disabled: true`` override.
    ...
    try:
        init_body = {
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "kiroclaw-probe", "version": "1.0.0"},
            },
        }
        hdrs = {
            **server.headers,
...
def sync_to_agent_config(servers: list[McpServerInfo]) -> bool:
    ...
    merge function that reads all source files (``~/.kiroclaw/mcp.json``,
    ...
    """
    from kiro_claw.agent import AGENT_FILENAME, install_agent  # circular import
    ...
    try:
        from kiro_claw.sel import sel  # circular import

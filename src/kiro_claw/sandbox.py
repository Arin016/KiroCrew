...
The parent KiroClaw process is completely unaffected — isolation applies
only to the spawned child.  Falls back gracefully to no sandbox when the
OS mechanism is unavailable (logged as warning).

Config: ``"sandbox": "auto" | "off"`` in ``~/.kiroclaw/config.json``.
...
def _probe_sandbox_exec() -> bool:
    ...
    fd, profile_path = tempfile.mkstemp(suffix=".sb", prefix="kiroclaw_probe_")
    try:
...
def _build_launcher_script(
    ...
    *,
    ...
    if sandbox_level in ("cc", "strict"):
        ...
        # (the file-level bind-mount of ~/.kiroclaw/.env hides them on disk;
...
def main():
    ...
    if pid > 0:
        ...
        # binds (e.g. /tmp on ext4 over ~/.kiroclaw/.env on ext4) can corrupt the
        ...
        for _candidate in (f"/run/user/{{REAL_UID}}", "/dev/shm"):
            try:
                ...
                _probe = tempfile.mkdtemp(dir=_candidate, prefix="kiroclaw_sb_")
                os.rmdir(_probe)

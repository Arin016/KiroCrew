...
`~/.kiroclaw/config.json` — main configuration file. Created automatically
on first `kiroclaw gateway` run.

### Managing Config

```bash
kiroclaw config get                    # print full config
kiroclaw config get agent.model        # print a specific value
kiroclaw config set agent.model auto   # set a value (auto type detection)
...
KiroClaw supports tiered sandbox levels for agent-backend process isolation:
...
Set via `kiroclaw config set sandbox.mode auto`.
...
| `agent.completion_keep_chars` | Maximum characters retained in the completion event after applying `completion_keep`. The full transcript stays in `~/.kiroclaw/subagents/<id>/result.txt` until cleanup; use the `spawn_status` MCP tool to read it before delivery. `0` disables truncation entirely. | `3000` |
| `agent.enforce_denied_commands` | Scope for deniedCommands: `"all"` or `"kiroclaw"` | `"all"` |
...
| `slack.allowed_users` | Users who can interact with KiroClaw | `[]` |
...
`~/.kiroclaw/.env` — Slack tokens and owner ID:

```
SLACK_APP_TOKEN=xapp-...
SLACK_BOT_TOKEN=xoxb-...
KIROCLAW_OWNER_ID=UXXXXXXXX
```
...
| Path | Purpose |
|------|---------|
| `~/.kiroclaw/config.json` | Main config |
| `~/.kiroclaw/.env` | Slack credentials |
| `~/.kiroclaw/skills/` | User skills |
| `~/.kiroclaw/crons.json` | Scheduled jobs |
| `~/.kiroclaw/lessons.jsonl` | Learned corrections |
| `~/.kiroclaw/history/` | Chat history (JSONL) |
| `~/.kiroclaw/workspace/memory/` | Memory files |
| `~/.kiroclaw/session_map.json` | Session resume mapping |
| `~/.kiro/agents/kiroclaw.json` | Installed agent config |

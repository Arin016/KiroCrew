...
KiroClaw connects to Slack via Socket Mode. You interact with it through DMs
or in channels where the bot is present.

## Activation Modes

> **Security note:** Slack interaction is restricted to the owner only
> (`KIROCLAW_OWNER_ID`). Multi-user access is disabled for security —
...
| Command | Description |
|---------|-------------|
| `/kiroclaw dashboard` | Same as `!dashboard` |
| `/kiroclaw @user` | ~~Same as `!allowlist @user`~~ (disabled) |
| `/kiroclaw #channel` | ~~Same as `!allowlist #channel`~~ (disabled) |

## Tool Approval Flow

When KiroClaw needs to run a tool (file write, bash command, etc.):
...
When KiroClaw is executing and you need to halt it immediately, type `!stop`
...
When KiroClaw presents choices, they render as interactive Block Kit buttons.
Click a button to send that choice back to the conversation. You can select
multiple options before submitting.

## Sharing Access

> ⚠️ **Multi-user Slack access is currently disabled for security.** KiroClaw
> is restricted to the bot owner only. The `!allowlist` command and
> `/kiroclaw @user` are disabled. Allowed users in config have no effect.
>
...
When `slack.tracking_channels` is configured, KiroClaw watches for new members
...
KiroClaw connects to Slack as a Socket Mode app that you create and install in
your own workspace.
...
3. **Set credentials** in `~/.kiroclaw/.env` (`SLACK_APP_TOKEN`,
   `SLACK_BOT_TOKEN`, `KIROCLAW_OWNER_ID`).
4. **Slash command** (optional) — the command name is configurable via
   `slack.command` in config.json (default: `kiroclaw`). Each app instance

...
Talk to your KiroClaw agent from WeChat — through a WeCom (企业微信) AI bot. Create
the bot in your WeCom console, drop in two values, and you're chatting. Replies
stream back live.

> **WeChat vs. WeCom.** KiroClaw connects through **WeCom (企业微信)**, the work
> edition, using its AI-bot API. It does **not** sign in to a personal WeChat
> account — people message the bot from inside WeCom.

Like Telegram, the connection is outbound-only: KiroClaw opens a secure
WebSocket to WeCom, so there's no callback URL or open port to manage.

## The easy way: just ask KiroClaw

You don't have to edit anything by hand. In any KiroClaw session — the
dashboard, Slack, or the CLI — say something like *"set up the WeChat channel."*
KiroClaw tells you where to create the WeCom AI bot, then writes your Bot ID and
Secret into `~/.kiroclaw/.env` and `config.json` and restarts the gateway for
...
You'll need a running gateway (`kiroclaw gateway`) and admin access to your
WeCom console.
...
3. **Save the credentials** to `~/.kiroclaw/.env`:
   ```
   WECOM_BOT_ID=your-bot-id
   WECOM_SECRET=your-bot-secret
   ```
4. **Turn it on** in `~/.kiroclaw/config.json`:
   ```json
   "wechat": {
     "enabled": true,
...
Those two values — **Bot ID** and **Secret** — are all KiroClaw needs. There's
...
> **KiroClaw runs on your machine, with your files and credentials.** So it only
> talks to the owner and the userids you name.

- Authorized senders: the **owner** (`KIROCLAW_OWNER_ID`) plus anyone listed in
...
Credentials go in `~/.kiroclaw/.env`: `WECOM_BOT_ID` and `WECOM_SECRET`.

...
KiroClaw can run tasks on a schedule — recurring checks, daily briefings,
...
KiroClaw uses the `cron_add` MCP tool to create the job.
...
```bash
kiroclaw cron add "pipeline-check" "check pipeline health" --every 1800
kiroclaw cron add "weekday-9am" "check tickets" --cron "0 9 * * MON-FRI" --approval-mode auto
kiroclaw cron update <job-id> --name "new name" --message "new prompt" --approval-mode auto
kiroclaw cron list
kiroclaw cron remove <id>
```
...
2. KiroClaw creates a fresh LLM session for the job
...
```bash
kiroclaw cron add "auto-check" "check pipeline" --every 300 --approval-mode auto
```
...
instant, KiroClaw adds a small random delay before executing scheduled jobs:
...
```bash
kiroclaw cron add "standup-prep" "prepare standup notes" --cron "0 9 * * MON-FRI" --strict-schedule
kiroclaw cron update <job-id> --strict-schedule
```
...
|--------|-----------|-------|-----|
| List | Overview → Cron tab | `cron list` | `kiroclaw cron list` |
| Pause | Pause button | `cron pause <id>` | — |
| Resume | Resume button | `cron resume <id>` | — |
| Delete | Delete button | `cron remove <id>` | `kiroclaw cron remove <id>` |

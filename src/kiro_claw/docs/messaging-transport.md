...
Channel-neutral contracts that let KiroClaw talk to Slack today and to other
...
the reverse. `kiro_claw.messaging` imports nothing from `kiro_claw.slack`.
...
`kiro_claw/messaging/transport.py`. One implementation per channel.
...
`kiro_claw/messaging/driver.py`. Shared by every channel — you do **not**
...
`kiro_claw/messaging/renderer.py`. One implementation per channel. The
...
`kiro_claw/messaging/link.py`. Session keys are **namespaced by channel** so two
...
| Path | Role |
|---|---|
| `src/kiro_claw/messaging/transport.py` | `MessagingTransport`, `TransportCapabilities`, `InboundMessage` |
| `src/kiro_claw/messaging/driver.py` | `TurnDriver` + approval ladder + redaction |
| `src/kiro_claw/messaging/renderer.py` | `Renderer` ABC, `OutputEvent`, `chunk_text` |
| `src/kiro_claw/messaging/link.py` | `ChannelLink`, `session_key`, `canonical_key` |
| `src/kiro_claw/slack/transport.py` | Slack `MessagingTransport` |
| `src/kiro_claw/slack/renderer.py` | Slack `Renderer` |
| `src/kiro_claw/slack/transport_dispatch.py` | Slack dispatch glue |
| `src/kiro_claw/config/loader.py` | `MessagingConfig` (`use_transport`) |

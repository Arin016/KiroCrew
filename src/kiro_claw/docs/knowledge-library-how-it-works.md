...
Each chunk is sent independently to the `kiroclaw-knowledge` agent (Claude Haiku 4.5, cheapest model at 0.4x rate). The LLM returns structured JSON:
...
|---------|---------------------|-----------|
| **KiroClaw** | Shared entity names | 0.4x per chunk (Haiku) |
...
Embeddings are configured in `~/.kiroclaw/config.json`:

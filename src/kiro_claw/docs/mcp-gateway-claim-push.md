...
gatewayd stamps a caller identity (`_meta.kiroclaw.caller`) on every MCP call
...
- `src/kiro_claw/mcp_gateway/claim.py` — frame builder + sender (stdlib-only)
- `src/kiro_claw/mcp_gateway/gatewayd.py` — `_StubConn`, `_CONN_INDEX`,
  `_apply_claim`, claim first-frame dispatch, per-frame caller pickup
- `src/kiro_claw/mcp_gateway/stub.py` — `parent_pid` on register; unbounded
  backoff recaller
- `src/kiro_claw/acp/client.py`, `src/kiro_claw/acp/session_provider.py` —
  `rekey()` claim hooks

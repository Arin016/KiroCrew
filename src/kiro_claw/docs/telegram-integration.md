...
Chat with your KiroClaw agent right from Telegram — on your phone, your laptop,
...
works from behind a firewall — KiroClaw reaches out to Telegram, so there's
nothing to expose.
...
channel."* KiroClaw walks you through creating the bot, then writes the token
and your user ID into `~/.kiroclaw/.env` and `config.json` and restarts the
...
You'll need a running gateway (`kiroclaw gateway`) and a Telegram account.
...
3. **Save the token** to `~/.kiroclaw/.env`:
   ```

...
Publish a KiroClaw artifact (HTML / Markdown / widget) to a public HTTPS URL on
**your own AWS account** — private S3 + CloudFront + Origin Access Control (OAC).
KiroClaw stores only your AWS **profile name**; it never sees, stores, or manages
your credentials.

Web Deploy is a built-in app, **opt-in / disabled by default**. Enable it from
Settings → Apps (or `kiroclaw app enable deploy-web`).
...
profile must therefore be configured on the **machine running the KiroClaw
...
Pick one (run in the gateway host's terminal — KiroClaw never sees your keys):
...
3. Click **Get IAM policy**, then apply the least-privilege policy **yourself**
   ...
   **KiroClaw never edits your IAM.** The first real deploy is the true test: on
...
Web Deploy is designed to keep KiroClaw **out of credential and account
management** entirely, and to serve content from a bucket that is never itself
public.

### 3.1 Credentials never touch KiroClaw
- KiroClaw stores **only the profile name** (in
  `~/.kiroclaw/apps/deploy-web/data/config.json`).
...
- KiroClaw **never** writes IAM and **never** creates/manages accounts, users,
...
- The bucket name is random/opaque (`kiroclaw-web-<random hex>`) and is hidden
...
- Before upload, content runs through KiroClaw's secret/credential regexes plus

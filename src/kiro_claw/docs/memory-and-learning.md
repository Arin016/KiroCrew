...
KiroClaw has persistent memory that survives across sessions. It remembers your
...
Corrections and rules you teach KiroClaw. Two ways to create:
1. **Explicit**: say "remember to always use pytest" → saved immediately
2. **Implicit**: correct KiroClaw during conversation → extracted during consolidation
...
KiroClaw saves these via the `learn_add` MCP tool. View them with `learn_list`
...
KiroClaw automatically consolidates conversations into memory:
...
- **CLI**: `kiroclaw memory show` / `kiroclaw memory edit`
- **Chat**: ask KiroClaw to update its memory files directly

...
Skills are markdown files that give KiroClaw specialized knowledge for specific
workflows. They live in `~/.kiroclaw/skills/` as `SKILL.md` files.
...
Ask KiroClaw: "Create a skill called X that does Y"

### Manually

Create `~/.kiroclaw/skills/my-skill/SKILL.md` with frontmatter and content.

## Built-in Skills

KiroClaw ships with built-in skills that are synced from the project's
`skills/` directory on startup. These cover common workflows like URL
shortening, code search, and writing assistance.

## Skill Sources (Priority Order)

1. `$KIROCLAW_PROJECT_DIR/skills/` — project-level (edit without rebuilding)
2. Built-in skills bundled in the Python package

Both are synced into `~/.kiroclaw/skills/` on startup. Newer source files
overwrite older ones (mtime-based). User-created skills in
`~/.kiroclaw/skills/` persist as long as they don't share a name with a

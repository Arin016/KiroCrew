...
Populated `$KIROCLAW_HOME` used by tests that need a non-empty
...
Running `kiroclaw gateway --seed minimal` against a fresh
`$KIROCLAW_HOME` lands you on a clickable dashboard: memory tab shows
the workspace content, cron tab shows the two jobs, sessions tab shows
the pinned **Welcome to KiroClaw** starter. Useful as the default
...
`$KIROCLAW_HOME` but are intentionally **not** included here. Text and

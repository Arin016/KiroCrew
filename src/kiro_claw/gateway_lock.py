"""Single-writer guard for a ``KIROCLAW_HOME`` (Mesh-2386).

Two ``kiroclaw gateway`` processes bound to the same home each open the same
...
Isolated homes (``--test-mode``/``--seed`` with a distinct ``KIROCLAW_HOME``)
...
class GatewayLockError(RuntimeError):
    """Raised when another live gateway already owns this ``KIROCLAW_HOME``."""
...
class GatewayLock:
    """Process-lifetime exclusive lock on a single ``KIROCLAW_HOME``.
    ...
    matching the single-host scope of ``KIROCLAW_HOME``.
    """

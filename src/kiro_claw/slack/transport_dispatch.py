...
from kiro_claw.slack.renderer import SlackApprovalDecider, SlackRenderer
...
#: Canonical kiro-cli agent name for KiroClaw. Used as the final agent
#: fallback so the transport session loads the same MCP surface (incl. the
#: kiroclaw-core server that provides ``spawn_run``) as the dashboard/native
#: path. Without it, an empty ``agent.default_agent`` config makes kiro-cli
#: launch under its bare built-in default (no kiroclaw-core -> no spawn_run).
#: Mirrors the native path's fallback (``handler.py``: ``_get_default_agent()
#: or "kiroclaw"``) and the many ``agent="kiroclaw"`` call sites.
...
async def handle_message_transport(
    ...
    *,
    ...
    # /kiroclaw link-to-dashboard routes into its dashboard slot (with the same
    ...
    try:
        ...
        # default win; otherwise fall back to the canonical "kiroclaw" agent so
        # the session loads kiroclaw-core (spawn_run) rather than kiro-cli's bare

...
from kiro_claw.task import Task
...
# Mapping of bang commands to their /kiroclaw slash equivalents.
_BANG_TO_SLASH: dict[str, str] = {
    "!yolo": "/kiroclaw yolo",
    "!stop": "/kiroclaw stop",
    "!voice": "/kiroclaw voice",
    "!agent": "/kiroclaw agent",
...
try:
    _overrides = KiroClawConfig.load().slack.reactions
except Exception:
...
# Falls back to single KIROCLAW_OWNER_ID for backward compatibility.
...
_orch_cfg: KiroClawConfig | None = None
...
def _get_default_agent() -> str:
    ...
    if _cached_default_agent is None:
        _cached_default_agent = KiroClawConfig.load().agent.default_agent
...
def set_orch_cfg(cfg: KiroClawConfig) -> None:
...
def is_allowed_user(user_id: str) -> bool:
    ...
    (KIROCLAW_OWNER_ID) is authorized to interact via Slack.
    """
    if not user_id:
        return False
...
async def _handle_slash_command(
    ...
    if cmd == "!stop":
        ...
        from kiro_claw.slack.blocks import build_stopping_blocks
    ...
    if cmd == "!voice":
        from kiro_claw.voice_reply import VALID_ENGINES, _validate_pitch, _validate_rate
    ...
    if cmd == "!agent":
        parts = cmd_text.split()
        if len(parts) == 1:
            name = _get_default_agent() or "kiroclaw"
            await slack.post_message(
                channel,
    ...
    if cmd == "!dashboard":
        ...
        from kiro_claw.slack.allowlist import send_dashboard_link
    ...
    if cmd == "!channel":
        if not is_owner(user_id):
            ...
            return ""
        from kiro_claw.config.loader import _VALID_ACTIVATIONS
...
def _append_footer_actions(
    ...
    if options:
        from kiro_claw.slack.format import build_options_blocks

        footer_blocks.extend(build_options_blocks(options))
    if thread_ts and not linked_session_key and dashboard_state:
        from kiro_claw.slack.format import build_link_dashboard_button
...
async def handle_message(
    ...
    *channel_agent* overrides the default agent for this channel (set via
    ...
    # @mention: "<@UBOT|kiroclaw> !agent foo"   → "!agent foo"      (strip prefix)
    ...
    if channel_activation == ACTIVATION_REVIEW:
        from kiro_claw.slack.blocks import review_draft_blocks
...
async def _handle_sessions_command(
    ...
    *,
    ...
    Delegates to :func:`kiro_claw.slack.sessions_view._collect_recent_sessions`
    and :func:`kiro_claw.slack.sessions_view._build_sessions_blocks` so the

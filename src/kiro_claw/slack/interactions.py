...
from kiro_claw.config.loader import ACTIVATION_REVIEW
from kiro_claw.security import redact_and_truncate, redact_credentials, redact_exfiltration_urls
...
async def dispatch(payload: dict) -> None:
    ...
    # ── Cron acknowledge ──
    from kiro_claw.slack.format import CRON_ACK_ACTION_PREFIX
    ...
    from kiro_claw.slack.format import SUBAGENT_ACK_ACTION_PREFIX
...
async def _refresh_channels_modal(view_id: str) -> None:
    ...
    if not _orch or not _orch.slack:
        return
    from kiro_claw.slack.blocks import channels_modal
...
async def _handle_ch_activation(payload: dict, action: dict) -> None:
    ...
    from kiro_claw.slack.handler import _persist_channel_config

    _persist_channel_config(cid, activation=new_mode)
    if _orch:
        from kiro_claw.config.loader import KiroClawConfig

        _orch._cfg = KiroClawConfig.load()
...
async def _handle_ch_remove(payload: dict, action: dict) -> None:
    ...
    from kiro_claw.slack.allowlist import persist_tracking_channel
...
async def _handle_options_submit(payload: dict, channel: str, msg_ts: str) -> None:
    ...
    if not is_allowed_user(user_id):
        sel().log_tool_invocation(
            session_key=thread_ts, agent="kiroclaw", source="slack",
        ...
        if _orch.slack:
            try:
                ...
                await _orch.slack.post_message(
                    ...
                    "⚠️ *Do not enter sensitive or confidential data into KiroClaw.*"
...
async def _handle_users_select(
    ...
    from kiro_claw.config.loader import config_path
    from kiro_claw.slack.handler import is_owner, set_allowed_users
...
async def _handle_channels_select(
    ...
    from kiro_claw.config.loader import config_path
    from kiro_claw.slack.handler import is_owner, set_tracking_channels
...
async def _handle_stop_confirm(payload: dict, channel: str, msg_ts: str, user_id: str) -> None:
    ...
    if has_session or active_task:
        ...
        async def _on_soft() -> None:
            from kiro_claw.slack.blocks import build_stopped_blocks
        ...
        async def _on_hard() -> None:
            from kiro_claw.slack.blocks import build_stop_failed_blocks
...
async def _handle_allowlist_remove(
    ...
    from kiro_claw.slack.blocks import allowlist_list_block
...
async def _handle_channel_remove(
    ...
    from kiro_claw.slack.blocks import channel_list_block
...
async def _handle_review_approve(payload: dict, action: dict) -> None:
    ...
    from kiro_claw.slack.handler import _review_drafts_get, _review_drafts_pop
...
async def _handle_review_edit(payload: dict, action: dict) -> None:
    ...
    from kiro_claw.slack.blocks import review_edit_modal
    from kiro_claw.slack.handler import _review_drafts_get
...
async def _handle_review_revise(payload: dict, action: dict) -> None:
    ...
    from kiro_claw.slack.blocks import review_revise_modal
    from kiro_claw.slack.handler import _review_drafts_get

...
``kiro_claw.slack.blocks.session_task_card``; it does not import
...
from kiro_claw.slack.blocks import session_task_card
...
_SESSIONS_DIR = Path.home() / ".kiroclaw" / "sessions"
...
def _collect_recent_sessions(
    ...
    *,
    ...
    """Read JSONLs under ``~/.kiroclaw/sessions/`` and return a sorted list.
...
def _build_sessions_blocks(
    ...
    shared :func:`kiro_claw.slack.blocks.session_task_card` builder so the

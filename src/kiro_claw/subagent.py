...
def _validate_agent(requested: str) -> tuple[str, str]:
    ...
    if requested in known:
        return requested, ""
    available = sorted(known - {"kiroclaw", "kiroclaw-conductor"})
    logger.warning(
        "Agent %r not found, falling back to kiroclaw. Available: %s", requested, available
    )
    return "", ""
...
class SubagentManager:
    ...
    def __init__(
        ...
        try:
            self._global_approval_mode = KiroClawConfig.load().agent.approval_mode
        except Exception:
            logger.warning(
                "Failed to load KiroClawConfig for approval_mode; defaulting to interactive",
                exc_info=True,
            )
    ...
    def spawn(
        ...
        Args:
            ...
            agent (str): Agent name override (default: "kiroclaw").
        ...
        try:
            min_mem = KiroClawConfig.load().agent.spawn_min_memory_gb
        except Exception:
            min_mem = 4.0
        ...
        if cwd:
            try:
                allowed_roots = KiroClawConfig.load().agent.subagent_cwd_allowed_roots
            except Exception:
    ...
    async def _run_inner(self, info: SubagentInfo, session_key: str) -> None:
        ...
        # Mirrors kiro_claw.dashboard.chat_runner._pending_tools.

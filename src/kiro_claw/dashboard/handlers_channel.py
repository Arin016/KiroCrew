...
async def api_channel_presets(request: web.Request) -> web.Response:
    ...
    Reads ``~/.kiroclaw/config.json`` fresh on each call so users can edit the
...
async def api_channel_update_agent(request: web.Request) -> web.Response:
    ...
    if "approval" in body:
        from kiro_claw.channel import ApprovalPolicy
        ...
        try:
            ...
            pass
    if "listen" in body:
        from kiro_claw.channel import ListenMode

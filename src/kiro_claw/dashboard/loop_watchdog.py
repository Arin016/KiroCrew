...
KiroClaw's gateway runs the dashboard HTTP server, every agent turn, and all
...
logger = logging.getLogger("kiro_claw.dashboard.loop_watchdog")
    ...
    * the daemon-thread :meth:`check` that dumps **without** exiting at
    ...
    def __init__(
        ...
        *,
        ...
        # armed timer so a dashboard-only process (e.g. `kiroclaw chat`, where

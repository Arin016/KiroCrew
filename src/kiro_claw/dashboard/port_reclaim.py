"""Reclaim the dashboard bind port from a *stale* KiroClaw gateway.

Background
----------
The gateway binds its dashboard port once at startup (:func:`_start_site` in
:mod:`kiro_claw.dashboard.server`). If the port is already taken it retries for
...
dump-then-exit from :mod:`kiro_claw.dashboard.loop_watchdog`, or a ``kill -9``)
...
* the holder is a *KiroClaw gateway* process — reusing the same structural
  command-line check that gates ``kiroclaw stop`` (:func:`_is_kiroclaw_process`)
...
* the holder does **not** answer an HTTP liveness probe — a *responsive*
  ...
  ``KIROCLAW_HOME`` sharing the port) and is never killed.

Complements — does not replace — :class:`kiro_claw.gateway_lock.GatewayLock`,
...
:class:`kiro_claw.dashboard.loop_watchdog.LoopStallWatchdog`.
"""
...
logger = logging.getLogger("kiro_claw.dashboard.port_reclaim")
...
FOREIGN_HOLDER = "foreign_holder"  # held by a non-KiroClaw process — never touched
...
async def _terminate_pids(
    ...
    *,
    ...
    silently looping).
    """
    from kiro_claw.cli_server import _pid_exited
...
async def reclaim_stale_gateway_port(
    ...
    *,
    ...
    is_kiroclaw: Callable[[int], bool] | None = None,
    ...
    """Reclaim *port* iff it is held by a stale/unresponsive KiroClaw gateway.
    ...
    # as `kiroclaw stop`). Bound to an explicit non-Optional local so the call
    # sites below type-check cleanly.
    checker: Callable[[int], bool]
    if is_kiroclaw is not None:
        checker = is_kiroclaw
    else:
        try:
    ...
    # ``identity_timeout``: the shared ``_is_kiroclaw_process`` shells out to
    ...
    if not kiro_pids:
        log.error(
            "Port %d is held by a non-KiroClaw process (pids %s) — refusing to "
    ...
    if await probe_healthy(port, probe_timeout):
        log.error(
            "Port %d is owned by a live, responsive KiroClaw gateway (pid %s) — "
            "not reclaiming. Stop it with `kiroclaw stop` or use a different port.",
            port,
            kiro_pids,
        )
        return HEALTHY_PEER
    ...
    log.warning(
        "Port %d is held by an unresponsive KiroClaw gateway (pid %s) left by an "
    ...
    log.error(
        ...
        "kill it manually (e.g. `sudo kiroclaw stop`).",
        kiro_pids,
        port,
    )

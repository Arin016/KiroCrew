...
- Persistence: ``~/.kiroclaw/refresh_chains.json`` (mode ``0600``).
"""
...
# will be forced through the regular ``kiroclaw token`` URL flow again
...
class RefreshStateManager:
    ...
    def clear_all(self) -> None:
        """Wipe all state. Used by tests and ``kiroclaw logout``."""
        with self._lock:
...
def generate_refresh_token(
    ...
    *,
    ...
    Omit it to start a fresh chain (initial mint after ``kiroclaw token``
    URL is consumed).
    """
    now = time.time()

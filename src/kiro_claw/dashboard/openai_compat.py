...
Translates OpenAI API format into KiroClaw's slot-based chat system,
allowing any OpenAI SDK client to talk to KiroClaw agents by setting
`model` to the agent name (e.g. "router", "lite").

Limitations:
- ``usage`` fields are hardcoded to zero; KiroClaw does not track token
...
async def _blocking_response(
    ...
    Note: ``usage`` is hardcoded to zero — KiroClaw does not expose token

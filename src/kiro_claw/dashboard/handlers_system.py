...
def _get_telemetry_salt() -> bytes:
    ...
    try:
        salt_file = Path.home() / ".kiroclaw" / "telemetry_salt"
        if salt_file.exists():
...
async def api_status(request: web.Request) -> web.Response:
    ...
    from kiro_claw.dashboard.handlers import updates as _updates_mod
...
def _collect_system_metrics() -> dict[str, object]:
    ...
    # matches both "kiroclaw_sandbox" and "kiro-cli"); per-category _counts can
    ...
    #   Linux:  The namespace launcher (python3 /tmp/kiroclaw_sandbox_*.py ...)
    #           forks — the parent stays alive with "kiroclaw_sandbox" in its
    ...
    #           "kiroclaw_sandbox" string (only in the -f path arg) is lost.

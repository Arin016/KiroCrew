...
under ``~/.kiroclaw/metrics/metrics-YYYY-MM-DD-<pid>.jsonl`` (see
``kiro_claw.metrics.local_exporter``). Each line is one export cycle serialized
...
and generically surfaces every other ``kiroclaw.*`` metric so newly-added emit
...
_STARTUP_METRIC = "kiroclaw.session.startup.duration"
_TURN_METRIC = "kiroclaw.turn.duration"
_WINDOW_DAYS = 14
...
def _telemetry_cfg() -> tuple[bool, Path]:
    ...
    try:
        cfg = KiroClawConfig.load().telemetry
...
def _aggregate(shard_paths: list[Path]) -> dict[str, Any]:
    ...
    # generic surface for every other kiroclaw.* metric
    other_hist: dict[str, _Hist] = {}
    other_ctr: dict[str, dict[str, Any]] = {}  # name -> {total, by_attr}
    turn = _Hist()
    turn_outcome: dict[str, int] = {}  # kiroclaw.turn.duration count by outcome
                    ...
                    for rm in obj.get("resource_metrics", []) or []:
                        for sm in rm.get("scope_metrics", []) or []:
                            for m in sm.get("metrics", []) or []:
                                name = m.get("name") or ""
                                if not name.startswith("kiroclaw."):
                                    continue
                                data = m.get("data") or {}
...
async def api_telemetry_startup(request: web.Request) -> web.Response:
    """GET /api/telemetry/startup — session-startup latency + all kiroclaw.* metrics.
    ...
    a generic ``other`` list surfacing every other emitted kiroclaw.* metric.
    """

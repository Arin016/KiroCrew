...
if TYPE_CHECKING:
    from kiro_claw.providers.base import LLMProvider  # noqa: F811
...
async def _remove_slot_for_history_key(state: DashboardState, key: str) -> None:
    ...
    """
    from kiro_claw.dashboard.chat import _history_key_for  # circular import  # noqa: F811
...
async def api_sessions_clear(request: web.Request) -> web.Response:
    ...
    from kiro_claw.dashboard.chat import _history_key_for  # noqa: F811
...
async def api_session_tool_policy(request: web.Request) -> web.Response:
    ...
    Used by managed MCP servers (kiroclaw-core, kiroclaw-cron) to filter
...
async def api_sessions_restart(request: web.Request) -> web.Response:
    ...
    Also syncs MCP servers from mcp.json → kiroclaw.json so newly
...
async def api_session_archive_list(request: web.Request) -> web.Response:
    ...
    from kiro_claw.history import _archive_dir, _safe_key

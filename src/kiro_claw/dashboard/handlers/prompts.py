...
def _list_aim_prompts():
    ...
    import kiro_claw.dashboard.handlers as _pkg
...
async def api_skills(request: web.Request) -> web.Response:
    ...
    Sources:
    - ``kiroclaw``: ``~/.kiroclaw/skills/`` (managed by SkillsLoader; editable)
    ...
    still be loaded via KiroClaw text-injection or an external MCP server).
    """

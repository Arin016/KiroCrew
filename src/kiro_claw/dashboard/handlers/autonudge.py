...
from kiro_claw.autonudge import get_instance as _autonudge_get
from kiro_claw.config.loader import workspace_dir_for
...
async def api_autonudge_start(request: web.Request) -> web.Response:
    ...
    if svc is None:
        return web.json_response(
            {"error": "auto-nudge disabled (KIROCLAW_AUTONUDGE not set)"}, status=503
        )

...
def _get_hook_store(state: DashboardState):
    ...
    if state._hook_store is None:
        from kiro_claw.hooks import (  # noqa: F811  # circular import
            ScriptHookStore,
...
async def api_hook_test(request: web.Request) -> web.Response:
    ...
    from kiro_claw.hooks import run_script_hook  # noqa: F811
...
_HOOK_STORE_PATH = Path.home() / ".kiroclaw" / "hooks.json"
...
def _verify_hook_token(request: web.Request) -> bool:
    ...
    return hmac.compare_digest(request.headers.get("x-kiroclaw-token", ""), token)
...
async def api_hooks_agent(request: web.Request) -> web.Response:
    ...
    Payload:
        ...
        agent (str): agent name for routing (default: kiroclaw)
...
async def _run_hook_inner(
    ...
    from kiro_claw.providers.base import EVENT_COMPLETE, EVENT_TEXT_CHUNK  # noqa: F811

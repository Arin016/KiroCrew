"""Portability API handlers — export/import KiroClaw state as zip."""
...
from kiro_claw.portability import apply_import_zip, create_export_zip, validate_import_zip
from kiro_claw.sel import sel as _sel_fn  # circular import — sel imports lazily
...
async def api_portability_export(request: web.Request) -> web.Response:
    """GET /api/portability/export — download KiroClaw state as zip."""
    ...
    filename = f"kiroclaw-export-{ts}.zip"
...
async def api_portability_import(request: web.Request) -> web.Response:
    """POST /api/portability/import — upload and apply a KiroClaw export zip."""
    if "user" not in request or not request["user"]:
        return web.json_response({"error": "authentication required"}, status=401)
    caller = request["user"]

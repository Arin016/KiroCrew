...
def _write_mcp_json(data: dict) -> None:
    ...
    from kiro_claw.agent import (  # noqa: F811  # circular import: agent imports handlers
        _atomic_json_write,
    )
...
def _sync_mcp_to_agent(name: str, enabled: bool, *, remove: bool = False) -> None:
    """Sync MCP server state to kiroclaw.json mcpServers (not tools/allowedTools)."""
    from kiro_claw.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
...
def _sync_mcp_to_agent_batch(names: list[str], enabled: bool) -> None:
    """Batch sync multiple MCP servers to kiroclaw.json in a single read-modify-write."""
    from kiro_claw.dashboard.handlers.agents import (  # noqa: F811 circular: agents imports mcp
        _installed_agent_config,
    )
    ...
    changed = False
    if enabled:
        # Ensure all servers exist in kiroclaw.json mcpServers
        mcp_servers = cfg.setdefault("mcpServers", {})
        try:
            gdata = json.loads(_GLOBAL_MCP_JSON.read_text(encoding="utf-8"))
        except (FileNotFoundError, json.JSONDecodeError):
...
async def api_mcp_active(request: web.Request) -> web.Response:
    ...
    For non-kiroclaw agents, reads ``mcpServers`` from the agent's config
    in ``~/.kiro/agents/`` — these are the only servers kiro-cli loads
    when ``--agent <name>`` is passed.  For kiroclaw (or no agent),
    ...
    # Resolve KiroClaw agent name → kiro agent name so "default" → "kiroclaw"
    if agent:
        try:
            from kiro_claw.config.loader import KiroClawConfig, resolve_agent_bindings  # noqa: F811
    ...
    # Non-kiroclaw agent: read from agent config
    ...
    # Kiroclaw / default: read from global mcp.json
    from kiro_claw.mcp_discovery import list_servers  # noqa: F811
    ...
    # Also include kiroclaw-cron and kiroclaw-core (always enabled)
    names = {r["name"] for r in result}
    for builtin in ("kiroclaw-cron", "kiroclaw-core"):
...
async def api_mcp_sync(request: web.Request) -> web.Response:
    ...
    2. Adds them to both kiroclaw agent config AND global mcp.json
       (kiro-cli ACP only reads the global config).
    3. Resets all sessions so changes take effect.
    """
    from kiro_claw.mcp_discovery import (  # noqa: F811
    ...
    # toggled enable/disable which writes to kiroclaw.json but requires
    # a session restart for kiro-cli to pick up the change.
    from kiro_claw.dashboard.handlers.sessions import _reset_all_sessions  # noqa: F811
...
async def api_mcp_toggle(request: web.Request) -> web.Response:
    ...
    2. Syncs ``tools``/``allowedTools`` in ``kiroclaw.json`` (non-ACP mode).
    """
    try:
    ...
    async with _get_mcp_lock():
        ...
        if name not in servers:
            ...
            from kiro_claw.mcp_discovery import (
                list_servers as _ls,  # circular import: mcp_discovery defers imports of kiro_claw.agent which shares state with this module
            )
        ...
        # 2. Sync to kiroclaw.json tools/allowedTools (lock prevents lost updates vs agents.py)
...
async def api_mcp_toggle_all(request: web.Request) -> web.Response:
    ...
    async with _get_mcp_lock():
        ...
        # Batch sync: single read-modify-write of kiroclaw.json
        from kiro_claw.dashboard.handlers.agents import _get_config_lock  # noqa: F811
...
async def api_mcp_remove(request: web.Request) -> web.Response:
    ...
    kiroclaw.json.  If the optional ``aim`` package manager happens to be
...
async def api_mcp_server_detail(request: web.Request) -> web.Response:
    ...
    # Sync to kiroclaw.json (enable by default)
...
_KIROCLAW_MCP_JSON = Path.home() / ".kiroclaw" / "mcp.json"
...
def _atomic_write(path: Path, data: dict) -> None:
    ...
    from kiro_claw.agent import (  # noqa: F811  # circular: agent imports dashboard handlers
        _atomic_json_write,
    )
...
def _find_server_spec_anywhere(name: str) -> dict | None:
    ...
    Search order matches the KiroClaw merge: agent config → ~/.kiroclaw/mcp.json
    ...
    """
    candidates = [
        Path.home() / ".kiro" / "agents" / "kiroclaw.json",
        _KIROCLAW_MCP_JSON,
        _GLOBAL_MCP_JSON,
        _CC_GLOBAL_JSON,
    ]
...
def _set_kiroclaw_entry(name: str, *, enabled: bool, spec: dict | None = None) -> str:
    """Set the server's ``disabled`` state in ``~/.kiroclaw/mcp.json``.
    ...
    ``"disabled"``, or ``"noop"``.
    """
    data = _load_json_or_empty(_KIROCLAW_MCP_JSON)
    servers = data.setdefault("mcpServers", {})
    ...
    _atomic_write(_KIROCLAW_MCP_JSON, data)
    return action


def _remove_kiroclaw_entry(name: str) -> bool:
    """Delete the server from ``~/.kiroclaw/mcp.json`` entirely.  Returns True on change."""
    data = _load_json_or_empty(_KIROCLAW_MCP_JSON)
    servers = data.get("mcpServers", {})
    if name not in servers:
        return False
...
async def api_mcp_apply(request: web.Request) -> web.Response:
    ...
    Request body::

        {
          "changes": [
            {
              "name": "slack-mcp",
              "kiroclaw": true,     // desired MC visibility
    ...
    ``~/.kiroclaw/mcp.json`` before the removal so MC keeps its config.

    After all changes are written, ``rebuild_agent_config`` is called once
    so the provider-native agent files (``~/.kiro/agents/kiroclaw.json`` and
    ``~/.claude/agents/kiroclaw.md`` + ``kiroclaw.mcp.json``) reflect the
    ...
    async with _get_mcp_lock():
        for change in changes:
            ...
            if change.get("uninstall"):
                outcome["actions"]["kiroclaw"] = (
                    "removed" if _remove_kiroclaw_entry(name) else "noop"
                )
                ...
                _remove_from_agent_file(
                    Path.home() / ".kiro" / "agents" / "kiroclaw.json", name
                )
                _remove_from_agent_file(
                    Path.home() / ".claude" / "agents" / "kiroclaw.mcp.json", name
                )
            ...
            desired_mc = bool(change.get("kiroclaw", True))
            ...
            # ~/.kiroclaw/mcp.json so MC keeps its config via the merge.
            preserved_spec: dict | None = None
            if desired_mc and not desired_kiro and not desired_cc:
                has_mc = _scope_has_entry(name, _KIROCLAW_MCP_JSON)
                if not has_mc:
            ...
            outcome["actions"]["kiroclaw"] = _set_kiroclaw_entry(
                name,
                enabled=desired_mc,
                spec=preserved_spec,
            )
            ...
            # ── Per-tool overrides (disabledTools in ~/.kiroclaw/mcp.json) ──
            ...
            if isinstance(tool_overrides, dict) and tool_overrides:
                ...
                # persisted to ~/.kiroclaw/mcp.json and later consumed by
            ...
            # reachable from KiroClaw sessions — a permission-shaping
    ...
    try:
        # circular import: kiro_claw.agent imports dashboard handlers, so
        # this is delayed to runtime to break the cycle at module load.
        from kiro_claw.agent import rebuild_agent_config  # noqa: F811
...
async def api_mcp_gateway_status(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {
            "enabled": KiroClawConfig.load().mcp_gateway.enabled,
...
async def api_mcp_gateway_enable(request: web.Request) -> web.Response:
    ...
    """
    from kiro_claw.agent import _atomic_json_write  # circular import
    from kiro_claw.config.loader import config_path  # circular import
    from kiro_claw.dashboard.handlers.agents import _get_config_lock  # circular import
...
async def api_mcp_gateway_servers(request: web.Request) -> web.Response:
    ...
    from kiro_claw.config.loader import KiroClawConfig  # noqa: F811
    from kiro_claw.mcp_gateway.rewriter import UNPOOLABLE_SERVERS

    allowlist = set(KiroClawConfig.load().mcp_gateway.poolable_servers)
...
async def api_mcp_gateway_set_poolable(request: web.Request) -> web.Response:
    ...
    from kiro_claw.dashboard.handlers.agents import _get_config_lock  # circular: agents imports mcp

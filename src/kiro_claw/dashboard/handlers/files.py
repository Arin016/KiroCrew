...
async def api_slack_upload_file(request: web.Request) -> web.Response:
    ...
    from kiro_claw.config.loader import outbox_dir, workspace_root  # noqa: F811
    ...
    if not (resolved.is_relative_to(allowed_outbox) or resolved.is_relative_to(allowed_workspace)):
        ...
        return web.json_response({"error": "file_path must be under ~/.kiroclaw/"}, status=403)
    try:
        raw = safe_read_file_bytes(str(resolved))
    except FileTooLargeError as e:
        _sel().log_tool_invocation(
    ...
    if target_channel:
        ...
        try:
            creds = KiroClawConfig.load().load_credentials()
            owner_id = creds.get("KIROCLAW_OWNER_ID", "")
            if owner_id:
...
_SCREENSHOT_DIR = Path.home() / ".kiroclaw" / "screenshots"

_UPLOAD_DIR = Path.home() / ".kiroclaw" / "uploads"
...
async def api_upload_file(request: web.Request) -> web.Response:
    ...
    Saves files to ~/.kiroclaw/uploads/ and returns server-side paths
...
async def api_workspaces_create(request: web.Request) -> web.Response:
    ...
    from kiro_claw.config.loader import config_dir  # noqa: F811
    from kiro_claw.validation import WORKSPACE_NAME_RE  # noqa: F811
    ...
    if copy_from:
        ...
        if src_path.is_dir():
            ...
            from kiro_claw.security import is_sensitive_path  # noqa: F811
    ...
    from kiro_claw.security import is_sensitive_path as _isp  # noqa: F811
...
async def api_file_read(request: web.Request) -> web.Response:
    ...
    from kiro_claw.validation import (  # noqa: F811
        FILE_READ_SCHEMA,
        ValidationError,
        validate_tool_args,
    )
...
async def api_file_search(request: web.Request) -> web.Response:
    ...
    if project:
        ...
        from kiro_claw.config.loader import workspace_dir_for  # noqa: F811
    ...
    if not search_roots:
        # Fallback: project dir, kiroclaw workspace, home
        proj = os.environ.get("KIROCLAW_PROJECT_DIR", "")
        if proj and os.path.isdir(proj):
            search_roots.append(proj)
        mc_workspace = os.path.expanduser("~/.kiroclaw/workspace")
    ...
    # Dot-prefixed dirs (.kiroclaw, .kiro, .aim) excluded by startswith(".") guard below.
    skip_dirs = {
...
async def api_browse_dirs(request: web.Request) -> web.Response:
    ...
    if is_sensitive_path(base):
        ...
        return web.json_response({"error": "Access denied"}, status=403)
    skip = {".git", "node_modules", "__pycache__", ".cache", ".venv", "venv", "env", ".kiroclaw", ".kiro", ".aim"}
    dirs: list[dict] = []
    try:
...
async def api_browse_files(request: web.Request) -> web.Response:
    ...
    if is_sensitive_path(base):
        ...
        return web.json_response({"error": "Access denied"}, status=403)
    skip = {".git", "node_modules", "__pycache__", ".cache", ".venv", "venv", "env", ".kiroclaw", ".kiro", ".aim", "build", "dist", ".next"}
    dirs: list[dict] = []
    files: list[dict] = []
    try:

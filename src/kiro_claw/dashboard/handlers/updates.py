...
from kiro_claw import __version__ as _local_version
from kiro_claw import shutdown_event
from kiro_claw.config.loader import KiroClawConfig, config_path
...
async def _build_frontend(proj: str, state: DashboardState) -> None:
    ...
    Delegates to the shared :func:`kiro_claw.frontend.build_frontend_async`
    helper so the build/stage logic is not duplicated across the three update
    paths (CLI ``kiroclaw update``, this dashboard endpoint, and the gateway
    auto-update). That helper runs ``npm ci`` (fallback ``npm install``) then
    ``npm run build`` in ``<proj>/website`` and copies ``website/dist`` into
    the served ``src/kiro_claw/static/dist`` — without which the dashboard
...
async def _restart_gateway(state: DashboardState) -> None:
    ...
    if not os.path.isfile(exe) or not os.access(exe, os.X_OK):
        ...
        return
    # circular import: kiro_claw.dashboard.chat imports from
    ...
    os.execv(exe, [exe, "-m", "kiro_claw"] + sys.argv[1:])
...
async def api_log_level(request: web.Request) -> web.Response:
    """POST /api/logs/level — change the kiro_claw logger level at runtime.
    ...
    root = logging.getLogger("kiro_claw")
...
async def api_log_level_get(request: web.Request) -> web.Response:
    """GET /api/logs/level — current kiro_claw logger level."""
    root = logging.getLogger("kiro_claw")
...
def install_log_ring_handler() -> _RingLogHandler | None:
    ...
    logging.getLogger("kiro_claw").addHandler(handler)

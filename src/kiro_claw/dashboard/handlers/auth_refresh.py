...
async def api_auth_refresh(request: web.Request) -> web.Response:
    ...
    if state.is_consumed(jti):
        ...
        # the event loop. Per kiroclaw-cr-reviewer rule 4d5.
    ...
    # (atomic-rename write to ~/.kiroclaw/refresh_chains.json) — must
    # not block the event loop. Per kiroclaw-cr-reviewer rule 4d5.
    await asyncio.to_thread(
        state.mark_consumed,
        jti,
        chain_id=chain_id,

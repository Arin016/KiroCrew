...
from kiro_claw.instances.registry import (
    ...
    return web.json_response(
        {
            ...
            "instances": items,
            "warm_set_cap": KiroClawConfig.load().instances.warm_set_cap,
        }
    )
...
async def api_instances_connect(request: web.Request) -> web.Response:
    ...
    if status.state.value == "connected":
        ...
        # (a failed self-heal re-mint, or a remote `kiroclaw restart` that

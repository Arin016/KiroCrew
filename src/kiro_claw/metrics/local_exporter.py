...
to ``<dir>/metrics-YYYY-MM-DD-<pid>.jsonl``. This is KiroClaw's default metrics
sink: data stays on the local disk (default ``~/.kiroclaw/metrics``) and never
...
the ``~/.kiroclaw`` file-permission convention -- so no other local user on a
    ...
    def export(
        ...
        **kwargs: object,
        ...
        try:
            ...
            # ~/.kiroclaw convention: telemetry stays private (dir 0o700, file

...
Consent + local-first:
  ...
  * When on, a ``PeriodicExportingMetricReader`` drains aggregated metrics to the
    local JSONL exporter under ``~/.kiroclaw/metrics``. Nothing egresses the host.
...
_SERVICE_NAME = "kiroclaw"
_SCOPE = "kiro_claw"

# Explicit histogram bucket boundaries (milliseconds), applied to EVERY kiroclaw

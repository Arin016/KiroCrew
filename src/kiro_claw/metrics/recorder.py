...
Every core or app metric flows through this facade so KiroClaw's namespace and
privacy guardrails (``validate_name`` / ``validate_attrs`` / ``redact`` from
``kiro_claw.metrics.schema``) run *before* the value reaches an OTEL instrument.
...
the first-party ``kiro_claw.metrics.schema`` guardrails. No Amazon-internal
imports.
"""
...
from kiro_claw.metrics.schema import validate_attrs, validate_name

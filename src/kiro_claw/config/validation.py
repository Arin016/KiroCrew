"""Config validation + the validated-data cache for KiroClaw.
...
- ``_config_fingerprint()`` — it reads ``config_path()`` / ``config_local_path()``,
  which the test suite patches via ``kiro_claw.config.loader.config_path``; keeping
...
``kiro_claw.config.loader`` logger — not this module's ``__name__`` logger — to
...
# name. Exposed so any lingering `kiro_claw.config.loader._CONFIG_CACHE_LOCK`
...
def validate_config_data(data: dict) -> dict:
    ...
    # circular import: schema.py imports KiroClawConfig from config.loader, which

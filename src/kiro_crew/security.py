...
_CREW_SECRET_LEAVES: list[str] = [
    ...
    ".vault",
    # Staging directory for atomic writes onto fenced FILES.
    #
    # `atomic_write`-style publication is `mkstemp` in the destination's directory,
    # then `os.replace`. For a fenced file in the data-home ROOT that puts the payload
    # in an UNFENCED sibling (`tmp*.tmp`) for the whole write: the fence names
    # `autonudge.json`, and the temp file is not that name. An agent that wins the
    # window can rename its own file onto the temp path, and the writer's `os.replace`
    # then publishes it under the fenced name -- forging the very
    # `operator_authored` flag the fence exists to protect.
    #
    # Directory-shaped keystone leaves (`variables/`, `profiles/`, `.vault`) never had
    # this exposure, because a prefix fence covers everything inside them. This gives
    # the file-shaped ones somewhere equally covered to stage.
    ".staging",
]

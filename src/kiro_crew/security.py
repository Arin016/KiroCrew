...
def _build_container_regex() -> "re.Pattern[str]":
    ...
    # A bare `~` OR a POSIX named-user tilde (`~runner`, `~kyle`): bash and
    # Python's `os.path.expanduser` both expand `~name` to that user's home, so
    # `~<owner>/.kiro/crew` names the real container. Anchoring on bare `~` alone
    # let an interpreter payload -- `os.rename(os.path.expanduser("~runner/.kiro/crew"))`
    # -- slip the raw scan, the ONE layer that sees inside a `-c` payload, and
    # relocate the fenced container (GPT review). The login-name run excludes both
    # path separators (`/` and the Windows `\\`, since `win_home_alts` reuses this)
    # plus whitespace and quotes, so it stays anchored to the single segment before
    # the separator; over-matching a longer name is fail-CLOSED.
    tilde = r"~[^/\\\s'\"]*"

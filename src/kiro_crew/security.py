...
_WRITE_PROTECTED_HOME_PATHS += [
    # Downloaded MODEL WEIGHTS. WRITE-protected as a whole directory, not read+write
    # sensitive: the weights hold no secret, and the settings surface and
    # `kirocrew doctor` both read the directory to report what is installed.
    #
    # They are an INPUT TO A TRUST DECISION. `embeddings.models_dir()` is this
    # directory, and it verifies its file against a pinned sha256 and then hands the
    # PATH to a native loader, so a writable directory leaves a window between the
    # digest and the open in which the bytes can be swapped -- and no amount of
    # re-hashing closes it, because the loader re-opens by name. Removing the
    # writability removes the window instead: the agent cannot modify the file at all,
    # so the verified bytes are the loaded bytes. A poisoned model is persistent and
    # invisible.
    #

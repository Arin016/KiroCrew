    ...
    try:
        async with lock:
            ...
            async def _restore_binding() -> None:
                """Put the binding back the way the user left it, as ONE mutation.

                All three fields together — link, inbound flag, mute — because a
                half-restore (link back, mute not) reconnects a conversation the user
                had muted.

                Composed from a read and two writes, this had a window: the in-channel
                `!unlink` handlers take no conversation lock, so a clear landing between
                the "is it still ours" check and the writes made the rollback RESURRECT
                a binding the user had just deleted. `restore_mirror_owner` samples the
                current binding and applies all three fields under one lock hold, so the
                skip and the restore can no longer be separated. Nothing was displaced
                here, so the empty occupant list keeps its clear narrowed to this
                session rather than the whole conversation.
                """
                try:
                    await asyncio.to_thread(
                    ...
                    logger.debug(
                        "could not restore the %s binding after a failed reconnect",
                        link.channel_type,
                        exc_info=True,
                    )
...
async def _claim_and_seed(
    ...
    try:
        # The full question at the actual send boundary: still bound, not muted, still
        # permitted. Governance alone was not enough — the claim above is several awaits
        # of disk I/O, and a disconnect landing in there left this send addressing a
        # conversation the user had detached from.
    ...
    if occupants:
        try:
            ...
            if governed is None:
                logger.info(
                    "mirror-link eviction notice suppressed: %s is detached, muted or "
                    "no longer permitted",
                    link.channel_type,
                )
            else:
...
async def _deliver_catch_up(
    ...
    for unit in units:
        try:
            ...
            # The MUTE is asked too. The rebind above already lifted it, so a mute
            # observed here is a fresh disconnect arriving mid-replay, and the rest of
            # this private transcript must not follow the user out of the conversation.

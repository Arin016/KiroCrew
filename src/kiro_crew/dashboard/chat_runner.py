...
def _expand_dollar_skills(
    ...
    Announces the chip itself, which :func:`_resolve_dollar_skills` deliberately does
    not: that one runs under ``asyncio.to_thread`` in the turn pipeline, and
    ``slot.append`` ends in ``asyncio.Event.set()``. This wrapper is synchronous and
    is never offloaded, so it is a safe place to do it.
    """
    authored, blocks, names = _resolve_dollar_skills(message, state, slot, session_key)
    if names:
        slot.append(
            "system",
        ...
        state.push_slots_update()
    return _join_skill_parts(authored, blocks), len(names)
...
def _resolve_dollar_skills(
    ...
    Returns ``(message, skill_blocks, resolved_names)``. The names come back rather
    than being announced from inside, because this runs on a worker thread -- see the
    comment at the return.
    ...
    # The chip is RETURNED, not appended here: this function runs under
    # `asyncio.to_thread`, and `slot.append` ends in `asyncio.Event.set()`, which is
    # not thread-safe and corrupts the loop's ready queue when called off it. The
    # caller appends on the loop thread once the offload completes -- the same shape
    # the variables stage in this pipeline already uses, and for the same reason.
    return message, blocks, names
    ...
    *message* has ALREADY been rewritten before arriving here — an auto-nudge body
    ...
    try:
        ...
        if "$" in message and not is_slash and not prompt_expanded and _prompt_depth < 1:
            ...
            message, skill_blocks, _skill_names = await asyncio.to_thread(
                _resolve_dollar_skills, message, state, slot, session_key
            )
            _n_skills = len(_skill_names)
            if _n_skills:
                # ON THE LOOP THREAD, after the offload. `slot.append` ends in
                # `asyncio.Event.set()`; doing it inside the worker is what broke the
                # variables stage here before.
                slot.append(
                    "system",
                    f"📎 Loaded skill(s) via `$`: **{', '.join(_skill_names)}**",
                    "msg msg-info",
                )
                state.push_slots_update()
                sel().log_tool_invocation(

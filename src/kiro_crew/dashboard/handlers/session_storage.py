...
async def api_session_storage_empty(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # missing total only costs the progress bar its denominator. It also costs the
        # identity check, since there is no snapshot to check AGAINST - stated rather
        # than hidden. The exposure is bounded by this path only ever deleting ids the
        # caller named, never "everything currently staged", which is the case above

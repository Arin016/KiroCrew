...
def _is_apple_silicon() -> bool:
    ...
    Rosetta 2 — which Kiro Crew's bundled Python does — so it cannot be used to
    ...
    if platform.machine() == "arm64":
        return True
    try:
        # The child prints a single ASCII flag (``1``/``0``), so the decode is
        # pinned rather than left to ``locale.getpreferredencoding``.
        out = subprocess.run(
...
#: Machine-readable reasons on the STT transcribe endpoint's non-2xx bodies. The
#: dashboard renders localised text and cannot key off an English sentence, so the
#: prose is advisory and these are the contract.
...
def _deny_app_token(request: web.Request, operation: str) -> web.Response | None:
    """Refuse an app token on the operator-only STT endpoints. 403 or None.

    ``request["user"]`` is truthy for an app token too, so a cookie check alone does
    not separate a browser from an app that declared this path in its manifest's
    ``permissions.api``. ``POST /api/stt/install`` runs a package-install script on
    the host — brew/pipx/pip against the operator's machine — which is operator setup
    rather than something an app earns by naming a path. The live transcription
    surfaces (``/api/ws/stt``, ``POST /api/stt/transcribe``) are deliberately NOT gated
    this way: shipped apps reach them on an app token.

    An absent ``app`` key is refused along with a non-empty one, so an unauthenticated
    route can only ever fail closed here.
    """
    if request.get("app") == "":
        return None
    # Best-effort: an unwrapped SEL failure here would replace the intended 403 with a
    # 500, which is the one outcome a refusal must never turn into.
    try:
        _sel().log_api_access(
...
async def _reap_stt_install_child(proc: asyncio.subprocess.Process | None) -> None:
    """Kill and reap the install child, tolerating one that never spawned or already exited.

    ``proc`` is ``None`` when the spawn itself was interrupted, and ``kill`` races a
    child that exited on its own — neither is an error worth surfacing, since the
    only goal is that no unparented pip tree outlives the request.
    """
    if proc is None:
        return
    try:
...
async def api_stt_install(request: web.Request) -> web.Response:
    ...
    # Before the busy check, so an app token cannot even reserve the slot.
    denied = _deny_app_token(request, "stt.install")
    ...
    # Bound before the try so the cancellation arm can reap a child that spawned,
    # and skip one that did not.
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            return web.json_response(
                {"ok": False, "code": "stt_install_failed", "error": output[-500:]}, status=500
            )
        ...
        # The handler is cancelled when the caller goes away mid-install (tab closed,
        # navigation) or the gateway shuts down. Reset FIRST and only then reap: the
        # busy check at the top of this handler reads that global, so a non-terminal
        # step left behind latches the 409 for the rest of the process lifetime and
        # the Install button never works again — and the reap can itself be
        # interrupted by a second cancellation.
        _stt_install_status = {"step": "error", "detail": "", "error": "install interrupted"}
        await _reap_stt_install_child(proc)
        _sel().log_api_access(
            caller=caller,
            operation="stt.install",
            outcome="failed",
        ...
        return web.json_response(
            {"ok": False, "code": "stt_install_timeout", "error": "Install timed out"}, status=500
        )
    except FileNotFoundError:
        ...
        return web.json_response(
            {"ok": False, "code": "stt_install_no_bash", "error": "bash not found"}, status=500
        )
    except Exception:
        # Catch-all for the same reason the cancellation arm exists: the busy check at
        # the top reads the status global, so ANY non-terminal step left behind latches
        # the 409 for the rest of the process lifetime and the Install button never
        # works again. The enumerated arms above cannot be exhaustive — a child line
        # longer than the StreamReader limit with no newline raises ValueError out of
        # ``readline`` — so the terminal state and the child's reaping must not depend
        # on having named the exception.
        logger.exception("STT install failed")
        _stt_install_status = {"step": "error", "detail": "", "error": "install failed"}
        await _reap_stt_install_child(proc)
        _sel().log_api_access(
            caller=caller,
            operation="stt.install",
        ...
        return web.json_response(
            {"ok": False, "code": "stt_install_failed", "error": "Install failed"}, status=500
        )
...
async def api_stt_transcribe(request: web.Request) -> web.Response:
    ...
    # Off the loop: with no argument the probe loads the config from disk, and every
    # provider branch then walks the filesystem — ensure_ffmpeg_in_path, a
    # shutil.which PATH walk, a find_spec import-path walk. Called inline it stalls
    # every other gateway task behind the first upload. Same reasoning, and the same
    # threading, as the probe bundle in api_stt_config.
    if not await asyncio.to_thread(is_available):
        return web.json_response(
            {"error": "STT not available", "code": _CODE_STT_UNAVAILABLE}, status=503
        )

...
def _read_repo_metadata_file(path: str, max_bytes: int) -> bytes | None:
    ...
    try:
        ...
        if not stat.S_ISREG(st.st_mode):
            raise PermissionError(f"repository metadata is not a regular file: {path}")
        # A hard link IS a regular file, so S_ISREG above cannot see it, and it has
        # no target for O_NOFOLLOW to refuse -- yet its bytes are whatever inode it
        # shares. A repository-controlled metadata entry hard-linked to a protected
        # file (a credential store) would otherwise be read out through this path,
        # which is the same vector _is_multilink_regular guards on the tracked-diff
        # side. Checked on the FSTAT, not an lstat pre-check, so the link count
        # belongs to the inode actually opened and there is no check-to-use window.
        # Git never hard-links the metadata read here, so refusing costs nothing.
        if st.st_nlink > 1:
            raise PermissionError(f"repository metadata is a hard link: {path}")

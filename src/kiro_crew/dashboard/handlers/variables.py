...
async def api_variables(request: web.Request) -> web.Response:
    ...
    if request.method != "PUT":
        payload = await asyncio.to_thread(_view_blocking)
        # Audited on SUCCESS, not only on refusal. A GET returns every operator
        # variable at every scope, so a read is a disclosure of the whole store -- the
        # denied branch above and each PUT path already log, which left the one
        # outcome that actually hands the data over as the only silent one.
        sel().log_api_access(
            caller=caller,
            operation="variables.access",
            outcome="allowed",

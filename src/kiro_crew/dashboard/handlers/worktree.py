...
async def _resolve_repo_root(
    ...
    try:
        ...
        # Audited like the sandbox refusal directly above. This barrier runs for
        # EVERY endpoint, so an unrecorded failure here is the one gap that hides
        # all three at once -- and `operation` already names which caller hit it.
        sel().log_api_access(
            caller=caller,
            operation=operation,
            outcome="error",
...
def _sync_result_response(payload: dict, status: int) -> web.Response:
    ...
    value rather than a literal; ``list``'s audited failure exits reach here for
    the same reason. Funnelling them through here keeps that single
...
async def api_worktree_create(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # The last unaudited git-failure exit in this module. Its siblings here
        # (timeout, missing sandbox) already record, and `list` records all three
        # of its own, so leaving this one meant the audit trail answered "did a
        # create fail?" only when the failure was a timeout or a missing backend
        # -- a plain git error was invisible. `error` rather than `denied`: this is
        # a failure, not a policy refusal.
        sel().log_api_access(
            caller=caller,
            operation="worktree_create",
            outcome="error",
...
def _list_worktrees_sync(root: str, state: object, caller: str = "dashboard") -> list[dict] | None:
    ...
    for index, rec in enumerate(records):
        ...
        # `head`, `bare` and `locked` are parsed from the porcelain above but are
        # deliberately NOT in the response: nothing renders them, and a field the
        # wire carries is a field the spec and the client type have to keep in
        # sync forever. `bare` still gates the dirty probe just above. Re-add any
        # of them together with the UI that reads it.
        out.append(
            {
                "path": path,
                "branch": rec["branch"],
...
async def api_worktree_list(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return _audited_list_failure(caller, root, _SANDBOX_REFUSAL, "sandbox_unavailable", 503)
    except (OSError, subprocess.SubprocessError) as exc:
        logger.warning("worktree_list failed: %s", exc)
        return _audited_list_failure(
            caller, root, "worktree listing failed", "worktree_list_failed", 500
        )
    if worktrees is None:
        return _audited_list_failure(
            caller,
            root,
            "git could not list this repository's worktrees",
...
def _audited_list_failure(
    caller: str, root: str, message: str, code: str, status: int
) -> web.Response:
    """A `worktree_list` failure that is RECORDED, not just returned.
    ...
    readable: an auditor could see which listings succeeded and which paths were
    withheld, but a repository whose listing keeps failing -- or a host whose
    sandbox is gone, which is how every git probe in this module stops working --
    left no record at all. These are genuine failures rather than policy
    refusals, so they carry `error`, matching the vocabulary `worktree_remove`
    ...
    # Rendered through `_sync_result_response`, not a second `web.json_response`
    # with a variable status: the error-code contract holds this module to exactly
    # ONE dynamic-status site, and adding another is what its ratchet counts. The
    # payload carries its own literal `code`, which is the part that has to be
    # machine-readable.
    return _sync_result_response({"error": message, "code": code}, status)
...
def _remove_worktree_sync(root: str, target: str, force: bool, state: object) -> tuple[dict, int]:
    ...
    # A NUL (or any embedded null) makes `realpath` raise ValueError, not OSError,
    # so an unvalidated request string crashed this thread and surfaced as a 500.
    # Screened HERE, at the one place a request-supplied path first becomes a
    # filesystem operation, rather than guarded at each `realpath` downstream.
    if "\x00" in target or "\x00" in root:
        return ({"error": "invalid path", "code": "worktree_invalid_arguments"}, 400)
...
async def api_worktree_remove(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # Audited like the `TimeoutExpired` sibling directly above. Missing here
        # because the first version of the audit ratchet only checked the
        # git-failure clause, so this one sat a single `except` away from a rule
        # written around the instance that prompted it.
        sel().log_api_access(
            caller=caller,
            operation="worktree_remove",
            outcome="error",
            resources=f"root={root} path={path}",
            error="sandbox backend unavailable",
        )
        return web.json_response(
            {"error": _SANDBOX_REFUSAL, "code": "sandbox_unavailable"}, status=503
        ...
        # This exit returns BEFORE the outcome mapping below, so the denied/error
        # split there never saw it -- a delete that failed on a git error left no
        # record at all, which is the one outcome an auditor most wants.
        sel().log_api_access(
            caller=caller,
            operation="worktree_remove",
            outcome="error",
    ...
    # A REFUSAL is not an ERROR. 403 (sensitive path) and 409 (main worktree /
    # in use by a session / uncommitted changes) are this endpoint refusing a
    # delete on policy grounds -- exactly the events a reviewer greps for -- while
    # 4xx/5xx from a failed git call, a timeout, or a missing sandbox are genuine
    # errors. Collapsing both into "error" hides every security denial from a SEL
    # query filtering `outcome="denied"`, which is the query the list path's own
    # denial (and `_resolve_repo_root`'s) is recorded for. Same vocabulary here or
    # the audit trail is only selectively searchable.
    if status == 200:
        outcome = "allowed"

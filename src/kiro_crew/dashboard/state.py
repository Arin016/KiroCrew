...
class DashboardState:
    ...
    def link_slack(
        ...
        *,
        ...
        ``persist=False`` is for a caller that read *thread_ts*/*channel_id* out of the
        stored row in the first place, so writing them back is a filesystem round-trip
        on the event loop that changes nothing. It suppresses only THIS slot's own
        write: the old-owner clear below still persists, because that row belongs to a
        different session and losing the eviction would leave two slots claiming one

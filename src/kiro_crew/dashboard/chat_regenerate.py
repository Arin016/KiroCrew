...
async def api_chat_slot_edit_resend(request: web.Request) -> web.Response:
    ...
    async with slot._lock:
        ...
        # Rows the commit ends up carrying because they arrived mid-boundary.
        # Populated by ``_commit_live_state``; read after it to decide whether the
        # merged window needs a durability pass of its own.
        ...
        try:
            ...
            def _commit_live_state() -> None:
                ...
                carried_rows.extend(arrived_rows)
            ...
            # A row carried by the commit is live but NOT yet durable: the
            # rewrite above persisted the prospective window only, and it dropped
            # that row's own durable copy on the way (a rewrite skips the
            # cross-process-append scan). Between here and the next periodic
            # flush tick it exists in memory alone, so a crash there loses a
            # workflow result the user was already shown. Make the MERGED window
            # durable now instead of waiting. Only when something actually
            # arrived, which is the rare case -- the normal path adds no await.
            #
            # AFTER ``dispatch_commit`` deliberately, mirroring rewind's
            # post-commit cleanup await: a cancellation landing on this await
            # must not strand a committed edit whose turn never runs.
            # ``best_effort`` (the default) is right here and is not the
            # swallow-a-destructive-failure shape: the boundary has already
            # committed, this write is additive, and a failure re-arms
            # ``_dirty`` so the periodic flush retries -- the same durability
            # this replaces, just later.
            if carried_rows:
                await save_slot_off_loop(state, slot, expected_history_key=expected_history_key)
        finally:

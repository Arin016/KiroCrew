...
async def api_chat_slot_slack_link(request: web.Request) -> web.Response:
    ...
    if existing_ts and existing_chan:
        ...
        if slack_mirror_is_paused(state, session_key):
            ...
            # ASKED AGAIN, after the last await. Both awaits above are real windows —
            # a policy read and a session-map write on worker threads — and an
            # `/slack-unlink` landing in either one CLEARS the stored row. The rebind
            # below deliberately does not write, so it would rebuild the slot flags
            # and the routing index around coordinates that no longer name anything:
            # a connection living only in memory, gone on the next restart, and unusable
            # by the reply path, which reads the stored row. Reported rather than
            # repaired — re-creating the row would overrule a user who unlinked on
            # purpose, exactly what the reconnect rollback stopped doing.
            live_ts, live_chan = state.sessions.get_slack_link(session_key)
            if (live_ts or "") != (existing_ts or "") or (live_chan or "") != (
                existing_chan or ""
            ):
                return web.json_response(
                    {
                        "error": "the Slack link changed while reconnecting",
                        "code": "slack_link_changed",
                    },
                    status=409,
                )
            # In-memory only. These coordinates came OUT of the stored row, and the
            # unpause above already persisted, so writing them back would be a
            # session-map write on the event loop that changes nothing — the gateway
            # stalls for the length of that write on slow storage. The call still
            # earns its place: it repairs the slot's link flags and the inbound
            # routing map, which a rehydrated slot can be missing, and pushes the
            # slot update the UI needs to show the reconnect.
            state.link_slack(slot.key, existing_ts, existing_chan, persist=False)

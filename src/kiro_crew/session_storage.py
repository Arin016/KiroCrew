...
def _delete_listed_files(
    ...
    # The approved directory, or nothing. `batch` is a NAME, and the mutation lock was
    # released between the snapshot that showed the user what would go and this open -
    # so a directory moved into that name would be deleted on the strength of consent
    # given for a different one. Checked on the DESCRIPTOR rather than by another stat
    # of the path: the fd is the object every removal below actually addresses, so a
    # further swap after this point does not reach the data. Nothing is deleted yet, so
    # refusing here costs only the batch.
    if expect_identity is not None:
        try:
            actual = os.fstat(batch_fd)
        except OSError as exc:  # pragma: no cover - defensive
            logger.warning("refusing to empty %s: %s", batch.name, exc)
            os.close(batch_fd)
            os.close(parent_fd)
            return 0, SKIP_UNREADABLE
        if (actual.st_dev, actual.st_ino) != expect_identity:
            logger.warning(
                "refusing to empty %s: it is not the directory that was selected",
                batch.name,
            )
            os.close(batch_fd)
            os.close(parent_fd)
    ...
    try:
        for rel in rels:
            ...
            # name it cannot hand to the kernel at all, so a single bad entry used to
            # abort the batch instead of costing itself. One unusable name must skip its
            # own file and leave the batch reported incomplete and restorable.
            except (OSError, ValueError):
                continue
...
def staged_targets(
...
) -> tuple[list[str], int, dict[str, tuple[int, int]]]:
    ...
    with _mutation_lock():
        ...
        for batch in chosen:
            try:
                info = os.stat(_batch_dir(batch.batch_id), follow_symlinks=False)
            except OSError:
                continue
            identities[batch.batch_id] = (info.st_dev, info.st_ino)
...
def empty_trash(
    ...
    under the lock. A batch whose opened descriptor does not match is kept, because
    the id is only a name and a directory swapped in during the handoff would
    otherwise be deleted on the strength of the user having approved a different one.
    Omitted means unchecked, which is what a direct caller that never took a snapshot
    gets.
    """
    with _mutation_lock():
        try:

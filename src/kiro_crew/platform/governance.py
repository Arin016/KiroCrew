...
def _absence_could_be_self_inflicted(path: Path) -> bool:
    ...
    * **Sticky** (``S_ISVTX``) **and root-owned** -> ``False``. Sticky means an entry may
      be unlinked only by its own owner or the directory's owner, so on a root-owned
      sticky directory group members can add files and still cannot remove root's managed
      document. Not academic: ``/Library`` is ``drwxrwxr-t root admin`` and the default
      single-Mac account is in group ``admin``, so asking "can I write here" answered True
      and aborted boot on EVERY standalone Mac that was never MDM-provisioned. The
      exemption is scoped to root-owned because a sticky directory owned by someone else
      protects THAT owner's entries, and its owner can still remove anything in it.
    * **Group- or world-writable** -> ``True``. The branch that answers for a root caller
      as well, where "can I write it" is true of everything and so says nothing.
    * **The managed directory EXISTS and is not root-owned** -> ``True``. A provisioned
      artifact is judged directly: its owner can remove its entries.
    * **The managed directory does NOT exist** -> only OUR OWN ownership of the nearest
      ancestor counts. Nothing was provisioned at this path, so an ancestor's owner is a
      fact about the OS image, not about this tier -- and on an id-mapped filesystem a
      system directory can be owned by an arbitrary uid (``/etc`` reads as uid 65534
      ``nobody`` on such hosts), where treating that as a route refuses boot on every one.
    ...
    managed_dir = path.parent
    try:
        # Whether the MANAGED DIRECTORY ITSELF exists is the discriminator below: an
        # existing one is a provisioned artifact and can be judged directly, while an
        # absent one means nothing was ever provisioned at this path and only its
        # neighbourhood is left to ask about.
        provisioned = managed_dir.exists()
        holder = managed_dir
    ...
    # STICKY exempts only a ROOT-OWNED directory. Sticky means an entry may be unlinked
    # only by its own owner or the directory's owner, so on a root-owned sticky directory
    # a group member can add files and still cannot remove root's managed document --
    # which is what makes macOS ``/Library`` (``drwxrwxr-t root admin``) safe. A sticky
    # directory owned by someone ELSE protects that owner's entries, not root's, and the
    # owner can still remove anything in it, so the exemption must not extend to it.
    if st.st_mode & stat.S_ISVTX and st.st_uid == 0:
        return False
    # Group or world write, without a root-owned sticky bit, IS a way in -- and this is
    # the branch that answers for a ROOT caller too, where "can I write it" is true of
    # every directory and so says nothing.
    if st.st_mode & _GROUP_WORLD_WRITE:
        return True
    if provisioned:
        # The managed directory EXISTS, so it is a provisioned artifact and is held to
        # the strict standard: any non-root owner can rewrite or remove its entries, so a
        # non-root-owned managed directory means the absence cannot be trusted. This is
        # the case where a distant ancestor's ownership was never the right question --
        # the artifact itself is right here to be judged.
        return st.st_uid != 0
    # The managed directory does NOT exist, so no managed document was ever provisioned
    # at this path and the ownership of some ancestor is a fact about the OS image rather
    # than about this tier. Only OUR OWN ownership counts here: on an id-mapped filesystem
    # (NFS, a container userns) a system directory can be owned by an arbitrary uid --
    # ``/etc`` reads as uid 65534 ``nobody`` on such hosts -- and treating that as a route
    # refused boot on every one of them, which is a self-inflicted abort rather than a
    # governance signal.
    getuid = getattr(os, "getuid", None)
    if getuid is None or getuid() == 0:
        # Root: already answered by the mode bits above. Asking "can root write it" would
        # be true everywhere and abort every root-run standalone install.
        return False

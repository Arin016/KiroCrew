...
def _copy_app_tree(source: Path, dest: Path) -> None:
    ...
    def _ignore(dir_path: str, names: list[str]) -> set[str]:
        # Prefix match, not exact: staging dirs carry unique per-transaction
        # suffixes (.kirocrew-deps-staging-<pid>-<nonce>), and an interrupted
        # install's leftover must neither be copied on update nor survive.
        skip = {n for n in names if n in _COPY_IGNORE or n.startswith(".kirocrew-deps-staging")}
        for n in names:
            if n in skip:
                continue
    ...
    try:
        if keep_data:
            ...
            if data.is_dir():
                ...
                _gen_names.extend(
                    p.name for p in data.glob(".kirocrew-deps-staging*")
                )
                ...
                _survivors = [
                    ...
                    if not (p.name == ".kirocrew-deps.lock" and p.is_file())
                ]
                if _survivors:
                    raise OSError(

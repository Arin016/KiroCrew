...
def app_enabled_state(name: str) -> bool | None:
    ...
    try:
        try:
            ...
            # A dangling symlink is a path that EXISTS and whose target cannot be
            # seen, which is not the same as nothing being there.
            if meta_path.is_symlink():
                logger.warning("Metadata path %s is a dangling symlink", meta_path)
                return None

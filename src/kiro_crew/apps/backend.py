            ...
            try:
                ...
                if pin.fd is not None:
                    _stale_names = [
                        ...
                        if e.startswith(_DEPS_STAGING_NAME) and e != staging.name
                    ]
                else:
                    _stale_names = [
                        p.name
                ...
                with tempfile.TemporaryFile() as _pipbuf:
                    try:
                        ...
                        if not getattr(_pip_exc, "stderr", None):
                            ...
                            _sz = _pipbuf.tell()
                            _pipbuf.seek(max(0, _sz - _DEPS_PIP_STDERR_TAIL))
                            _pip_exc.stderr = _pipbuf.read()
                        raise
...
def stop_recorded_app_backend(app_name: str) -> bool:
    ...
    (written at spawn, cross-process), signals its tree, and WAITS for
    death, PID-reuse-guarded by the start-time token. Returns True only
    ...
    """
    # Read the persisted record BEFORE the in-process stop:
    # stop_app_backend unconditionally forgets the pidfile entry, so reading
    # after it would always find nothing in a CLI process (empty tracking)
    # and report a live backend as stopped.
    rec = _read_pidfile().get(app_name)
    stop_app_backend(app_name)
    if not rec:
        return True
    pid = rec.get("pid")

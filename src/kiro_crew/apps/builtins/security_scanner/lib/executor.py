...
def run_poc(poc: PoC, target: Target, limits: ExecLimits | None = None) -> ExploitEvidence:
    ...
    # Adversarial PoC code MUST run under the OS-level sandbox. Route the spawn
    # through the shared fail-closed chokepoint (the one the subprocess-spawn
    # audit requires) so the child gets filesystem isolation + a scrubbed env.
    # If the host offers no sandbox backend, REFUSE to run it unsandboxed and
    # return an inconclusive ERROR rather than executing adversarial code
    # same-UID (SECURITY_NOTES.md #1).
    try:
        spawn_argv, spawn_env, sb_cleanup = sandbox.sandboxed_spawn_argv(
            [sys.executable, script_path], mode="strict", env=env,
        )
        ...
        return ExploitEvidence(
            ...
            output=scrub_secrets(
                f"OS sandbox unavailable — refused to run PoC unsandboxed ({exc}).",
                extra=[getattr(target, "token", "")],
            ),

...
#
#    The JSONL goes under $DATA, not /tmp: $DATA is resolved portably (PRPM_DATA_DIR,
#    then KIROCREW_HOME, then the default) and exists on Windows, where there is no
#    /tmp at all.
python3 -m $M batch --repo <repo> --repo-path <repo_path> \
    --limit 20 --detect-moves --out $DATA/scan.jsonl

# 2. load them as per-PR report files
python3 -m $M import-reports --jsonl $DATA/scan.jsonl
...
python3 -m $M bundles --repo <repo> --repo-path <repo_path> \
    --jsonl $DATA/scan.jsonl --out-dir $DATA/bundles \
...
```bash
python3 -m $M check-analysis --dir $DATA/analysis --bundle-dir $DATA/bundles
```

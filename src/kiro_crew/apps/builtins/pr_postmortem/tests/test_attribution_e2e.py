...
def _git(args: list[str], cwd: str) -> None:
    """Run git against the synthetic repo, fully isolated from the host's config.

    Without the isolation below these commits execute the DEVELOPER's global hooks:
    a global `core.hooksPath`, or an `init.templateDir` that seeds `.git/hooks`,
    both run arbitrary local code inside `git commit`. Pointing global and system
    config at paths that do not exist inside the test's own tmp tree removes every
    such setting at once, and an empty `core.hooksPath` covers a hooks directory
    seeded some other way. Deliberately not `/dev/null` -- that is not a valid
    empty directory on Windows. Found by review on PR #2669.
    """
    isolated = os.path.join(cwd, ".git-isolation")
    hooks = os.path.join(isolated, "empty-hooks")
    os.makedirs(hooks, exist_ok=True)
    subprocess.run(
        ["git", "-c", f"core.hooksPath={hooks}", "-c", "core.autocrlf=false", *args],
        cwd=cwd,
        check=True,
        capture_output=True,
        text=True,
        env={
            **os.environ,
            "GIT_CONFIG_GLOBAL": os.path.join(isolated, "no-global-config"),
            "GIT_CONFIG_SYSTEM": os.path.join(isolated, "no-system-config"),
...
class TestGitInvocationIgnoresHostConfig(unittest.TestCase):
    """The synthetic commits must not execute the developer's global git hooks."""

    def test_a_poisoned_global_hooks_path_does_not_run(self):
        tmp = tempfile.mkdtemp()
        try:
            # A hostile global config: core.hooksPath at a dir whose pre-commit
            # hook writes a sentinel. If `_git` leaks the host config, committing
            # in the synthetic repo runs it.
            hooks = os.path.join(tmp, "evil-hooks")
            os.makedirs(hooks, exist_ok=True)
            sentinel = os.path.join(tmp, "hook-ran")
            ...
            try:
                ...
                with open(os.path.join(repo, "a.txt"), "w", encoding="utf-8") as fh:
                    fh.write("x\n")
                _git(["add", "a.txt"], repo)
                _git(["commit", "-q", "-m", "feat: thing"], repo)
            finally:
                if prior is None:
                    os.environ.pop("GIT_CONFIG_GLOBAL", None)
                else:
                    os.environ["GIT_CONFIG_GLOBAL"] = prior

            self.assertFalse(
                os.path.exists(sentinel),
                "the host's global pre-commit hook RAN inside the test -- _git is "
                "not isolating global git config",
            )
        finally:

...
class TestAnalysisContract(unittest.TestCase):
    ...
    def test_the_prompt_schema_does_not_contradict_the_validator(self):
        """A model following the schema literally must produce a VALID rejection.

        The validator refuses a rejected link that carries a `candidate_rule`, so a
        schema that shows the key unconditionally would instruct the analyst to
        produce something the validator then rejects.
        """
        prompt = analysis.build_prompt("o/r", "/b.json", "/a.json", 10, 4)
        self.assertIn("omit this whole key when rejected", prompt)
        self.assertIn("OMIT the `candidate_rule` key entirely", prompt)
        # And the identity requirement is stated, since it is now enforced.
        self.assertIn("identify\nthe pair you were asked about", prompt)

    def test_a_rejection_built_exactly_as_the_prompt_describes_validates(self):
        # Empty analysis fields, no candidate_rule key -- what the prompt now says.
        rejected = {
            "fix_pr": 10,
            "culprit_pr": 4,
            "culprit_link_verdict": "rejected",
            "culprit_link_reason": "blame landed on a reformatting commit",
            "failure_mode": "",
            "root_cause": "",
            "why_review_missed": "",
            "why_tests_missed": "",
...
class TestAnalysisIsBoundToItsPair(_DataDir):
    ...
    def test_a_mismatched_CULPRIT_is_rejected_when_the_caller_knows_it(self):
        # The sibling of the fix_pr check: the bundle names the culprit the prompt
        # was built for, so a different value means a different pair.
        path = os.path.join(self.data, "analysis-10.json")
        store.write_json(path, dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        _obj, errs = analysis.load_and_validate(path, expected_culprit=7)
        self.assertTrue(any("culprit_pr" in e and "different pair" in e for e in errs),
                        errs)

    def test_a_matching_culprit_is_accepted(self):
        path = os.path.join(self.data, "analysis-10.json")
        store.write_json(path, dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        _obj, errs = analysis.load_and_validate(path, expected_culprit=4)
        self.assertEqual([], errs)

    def test_an_expectation_of_None_is_still_checked(self):
        # None is a REAL expectation (a culprit commit predating the PR workflow),
        # not "no expectation" -- so a numbered culprit against it must fail.
        path = os.path.join(self.data, "analysis-10.json")
        store.write_json(path, dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        _obj, errs = analysis.load_and_validate(path, expected_culprit=None)
        self.assertTrue(any("culprit_pr" in e for e in errs), errs)

    def test_omitting_the_expectation_skips_the_culprit_check(self):
        path = os.path.join(self.data, "analysis-10.json")
        store.write_json(path, dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        _obj, errs = analysis.load_and_validate(path)
        self.assertEqual([], errs)

    def test_is_about_compares_BOTH_halves_of_the_pair(self):
        path = os.path.join(self.data, "analysis-10.json")
        store.write_json(path, dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        self.assertTrue(analysis.is_about(path, 10, 4))
        self.assertFalse(analysis.is_about(path, 10, 7), "culprit differs")
        self.assertFalse(analysis.is_about(path, 11, 4), "fix differs")

    def test_check_analysis_cross_checks_against_the_bundle(self):
        d = os.path.join(self.data, "analysis")
        b = os.path.join(self.data, "bundles")
        os.makedirs(d)
        os.makedirs(b)
        store.write_json(os.path.join(b, "bundle-10.json"),
                         {"fix_pr": 10, "culprit_pr": 7})
        store.write_json(os.path.join(d, "analysis-10.json"),
                         dict(_VALID_ANALYSIS, fix_pr=10, culprit_pr=4))
        # Without the bundle dir the schema alone is satisfied...
        self.assertEqual(0, cli.main(["check-analysis", "--dir", d]))
        # ...with it, the wrong culprit is caught.
        self.assertEqual(
            1, cli.main(["check-analysis", "--dir", d, "--bundle-dir", b])
        )

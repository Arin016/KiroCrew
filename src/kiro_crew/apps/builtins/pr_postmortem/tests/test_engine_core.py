...
class TestTrustedCliDirsCoverEveryPlatform(unittest.TestCase):
    """The CLI list must not silently exclude a platform the system list trusts.

    `_TRUSTED_SYSTEM_BIN_DIRS` already enumerates the POSIX roots this project
    supports, including the NixOS system profile. A general-purpose bin directory
    present there but missing here means `trusted_cli_bin` fails closed on that whole
    platform -- which is how NixOS was excluded. Asserted as an invariant rather than
    as one hardcoded path, so the next addition to the system list cannot be
    forgotten here.
    """

    def test_every_general_purpose_system_root_is_also_a_cli_root(self):
        # A "sbin" directory holds system administration binaries and no developer
        # CLI installs there, so those entries are legitimately absent from the CLI
        # list. Identified by basename rather than by a hardcoded path, which also
        # keeps this file free of the POSIX path literals the portability gate
        # rejects.
        general = [d for d in platform_compat._TRUSTED_SYSTEM_BIN_DIRS
                   if os.path.basename(d) != "sbin"]
        missing = [d for d in general
                   if d not in platform_compat._CLI_BIN_DIRS_POSIX]
        self.assertTrue(general, "the system list must not be empty")
        self.assertEqual(
            [], missing,
            f"these roots are trusted for system binaries but not for CLIs, so "
            f"git/gh would be unresolvable on those platforms: {missing}",
        )

    def test_the_cli_roots_are_absolute(self):
        for path in platform_compat._CLI_BIN_DIRS_POSIX:
            self.assertTrue(path.startswith("/"), path)
        for path in platform_compat._CLI_BIN_DIRS_WINDOWS:
            self.assertEqual(":\\", path[1:3], path)

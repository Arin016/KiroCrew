...
def _is_available(value: object) -> bool:
    """True when a collected sub-record carries data rather than a sentinel.

    The collectors return ``{"available": False}`` instead of an empty dict when
    `gh` or the check-run history cannot be reached, so emptiness is the wrong test.
    """
    return isinstance(value, dict) and bool(value.get("available"))


def _untrusted_information(value: object) -> int:
    """Count the parts of the untrusted block that actually carry prose.

    ``untrusted`` always has four keys (a WARNING, both discussions, the culprit
    subject), so its length is constant whether collection succeeded or not.
    """
    if not isinstance(value, dict):
        return 0
    score = 0
    for key in ("fix_pr", "culprit_pr"):
        if _is_available(value.get(key)):
            score += 1
    if str(value.get("culprit_commit_subject") or "").strip():
        score += 1
    return score


def _evidence_census(bundle: dict) -> dict[str, int]:
    """Count the evidence axes a rebuild could lose.

    Deliberately counts rather than compares content: the question is only
    "does the new bundle still carry what the old one carried", and a count is
    something a reader can verify against the JSON by hand.
    """
    # Keys are named after the REAL bundle keys, and `test_census_keys_exist`
    # asserts they are a subset of `Bundle.to_dict()`. The first version of this
    # function read invented nested `culprit`/`fix` dicts, so every axis counted 0
    # and the guard could never fire -- a guard that cannot fail is not a guard,
    # and its tests passed only because they shared the same invented schema.
    # Found by review on PR #2354.
    return {
        # Sized containers: these really do go empty when collection fails.
        "culprit_commits": len(bundle.get("culprit_commits") or ()),
        "culprit_diff": len(str(bundle.get("culprit_diff") or "")),
        "fix_diff": len(str(bundle.get("fix_diff") or "")),
        "fix_touched_files": len(bundle.get("fix_touched_files") or ()),
        "tests_added_by_fix": len(bundle.get("tests_added_by_fix") or ()),
        "attribution": len(bundle.get("attribution") or {}),
        # Sentinel-bearing containers: NEVER empty, so length says nothing. A
        # degraded collection fills them with `{"available": False}` -- `untrusted`
        # is always a 4-key dict and `culprit_ci` always has at least `available`.
        # Counting length here is what made this guard unable to fire on the very
        # path it exists for (`gh` unavailable). Count the information instead.
        # Found by review on PR #2669.
        "untrusted": _untrusted_information(bundle.get("untrusted")),
        "culprit_ci": 1 if _is_available(bundle.get("culprit_ci")) else 0,
    }


def _loses_evidence(fresh: dict, stored: dict) -> bool:
    """True when ``fresh`` drops an axis the stored bundle had.

    Emptying an axis that previously held something is the signal -- not merely
    being smaller, since a legitimate re-attribution to a different culprit
    changes the content and may well be shorter.
    """
    new_c, old_c = _evidence_census(fresh), _evidence_census(stored)
    return any(old_c[k] > 0 and new_c[k] == 0 for k in old_c)
...
def _test_changes(diff_text: str) -> list[TestChange]:
    ...
    for fc in files:
        ...
        if kind != "test":
            continue
        # A DELETED test file is not an added test. It was still reported under
        # `tests_added_by_fix`, so a fix that REMOVED coverage read to the analyst
        # as a fix that added it -- inverting the one signal the "why tests missed
        # it" judgement depends on. Found by review on PR #2354.
...
def write_bundles(
    ...
    for rep in reports:
        ...
        # Found by review on PR #2354.
        fresh = redact_tree(b.to_dict())

        # A rebuild must not silently LOSE evidence. Rebuilding in a degraded
        # environment -- the clone no longer has the culprit commit, `gh` is
        # unavailable so titles and review comments come back empty -- produced a
        # poorer bundle that then overwrote the good one, and the analysis pass
        # would read the poorer one with no sign anything was lost.
        #
        # The compare-then-write runs under store's SHARED lock and its atomic
        # rename: the first version was a hand-rolled read-then-write, so two
        # writers (a cron scan and a manual CLI run) could interleave between the
        # decision and the write, and a crash mid-write left a truncated bundle
        # that `_read_json` would then treat as absent. Found by review on #2354.
        with store.exclusive(path):
            stored = _read_json(path)
            # Preservation only applies when the stored bundle is about the SAME
            # culprit. A rebuild naming a DIFFERENT culprit is a different subject,
            # not a degraded version of this one -- keeping the old bundle there
            # left the evidence on disk describing a pull request the attribution
            # no longer blames, and the analysis prompt was then built from it.
            # Found by review on PR #2669.
            same_culprit = (
                stored is not None
                and stored.get("culprit_pr") == fresh.get("culprit_pr")
            )
            if same_culprit and _loses_evidence(fresh, stored or {}):
                paths.append(path)
                continue

...
Write ONLY a single JSON object to:
    {out_path}
No prose, no markdown fence in the file.

`fix_pr` and `culprit_pr` MUST carry exactly the values shown below: they identify
the pair you were asked about, and a different value means the analysis is about
some other pair. It is rejected on those grounds alone.

When `culprit_link_verdict` is "rejected", OMIT the `candidate_rule` key entirely
and leave `failure_mode`, `root_cause`, `why_review_missed` and `why_tests_missed`
as empty strings. A rejected link is a complete result with no analysis attached,
so a rejected object that still carries a rule or a failure mode is contradictory
and is refused.
...
{{
  ...
  "why_review_missed": "",
  "why_tests_missed": "",
  "candidate_rule": {{          <-- omit this whole key when rejected
...
def load_and_validate(
    path: str, expected_culprit: object = _UNCHECKED
) -> tuple[dict | None, list[str]]:
    """Load an analysis and validate it, including WHICH PAIR it is about.

    Identity is checked here rather than in `validate`, which sees only the object
    and so cannot know what was requested. Both halves of the pair are covered:

    * ``fix_pr`` against the number in the filename, which is what the prompt told
      the analyst to write.
    * ``culprit_pr`` against ``expected_culprit`` when the caller knows it -- the
      bundle names the culprit the prompt was built for, so a different value means
      the content is about a different pair.

    Attributing a finding to the right pull request is this app's entire product,
    so accepting a mismatched file would corrupt exactly the thing being produced.
    Callers that genuinely have no expectation omit ``expected_culprit``; passing
    ``None`` is a real expectation (a culprit commit with no PR) and IS checked.
    """
    try:
    ...
    if isinstance(obj, dict):
        ...
        if named is not None and obj.get("fix_pr") != named:
            errs.append(
                f"fix_pr is {obj.get('fix_pr')!r} but the file is named for "
                f"#{named} -- the analysis is about a different pair"
            )
        if expected_culprit is not _UNCHECKED and obj.get("culprit_pr") != expected_culprit:
            errs.append(
                f"culprit_pr is {obj.get('culprit_pr')!r} but this pair was "
                f"requested for {expected_culprit!r} -- the analysis is about a "
                f"different pair"
            )
    return (obj if isinstance(obj, dict) else None), errs


def is_about(path: str, fix_pr: int, culprit_pr: object) -> bool:
    """True when the stored analysis at *path* is about exactly this pair.

    The single implementation of the pair-identity question. An unreadable or
    malformed file is NOT about the pair -- redoing it is cheap, and trusting it is
    not. Schema validity is deliberately not required: a pair can be re-analysed
    for being wrong about its subject even when it is well-formed.
    """
    try:
        with open(path, encoding="utf-8") as fh:
            stored = json.load(fh)
    ...
    if not isinstance(stored, dict):
        return False
    return stored.get("fix_pr") == fix_pr and stored.get("culprit_pr") == culprit_pr

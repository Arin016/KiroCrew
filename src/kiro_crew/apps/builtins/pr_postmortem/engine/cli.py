...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "prompts":
        ...
        for bpath in sorted(glob.glob(f"{args.bundle_dir}/bundle-*.json")):
            ...
            # The ATTRIBUTION REPORT is the source of truth for who the culprit
            # is -- not the bundle. Comparing the analysis against the bundle's
            # culprit let a stale bundle (one the no-downgrade guard preserved)
            # certify a stale analysis as fresh, so neither was ever regenerated.
            # A bundle that disagrees with the report is itself stale, and a pair
            # in that state is never "fresh". Found by review on PR #2669.
            report = load_report(fix_pr, include_evidence=False) or {}
            authoritative = report.get("culprit_pr")
            bundle_agrees = b.get("culprit_pr") == authoritative
            existing = os.path.exists(out_path)
            fresh = (
                existing
                and bundle_agrees
                and _analysis_is_about(out_path, authoritative)
            )
            if existing and not bundle_agrees:
                print(
                    f"bundle for #{fix_pr} names culprit "
                    f"#{b.get('culprit_pr')} but the report names "
                    f"#{authoritative}; re-analysing"
                )

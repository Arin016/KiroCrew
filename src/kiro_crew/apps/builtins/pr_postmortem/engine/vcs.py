...
# A pull-request number is small; the bound exists because `int()` RAISES above
# 4300 digits on Python 3.11+, and this parses a COMMIT SUBJECT -- text anybody who
# can land a commit controls. An unbounded digit run crashed discovery instead of
# simply not matching. Same rule as store.parse_proposal_id's.
...
def pr_from_subject(subject: str) -> int | None:
    ...
    if m is None or len(m.group(1)) > MAX_PR_DIGITS:
        return None

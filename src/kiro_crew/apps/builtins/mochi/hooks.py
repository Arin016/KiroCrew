...
from kiro_crew.apps.builtins.mochi.activity_budget import SpawnLedger
...
from kiro_crew.apps.builtins.mochi.soul_loader import SoulLoader, load_skill_line
...
class _PollerCallbacks:
    ...
    async def on_mood(self, action: dict[str, Any]) -> None:
        ...
        if mood:
            self._rt.state_manager.set_mood(str(mood), _now_ms())
...
class MochiRuntime:
    ...
    def _write_agent_prompt(self) -> None:
        ...
        """
        try:
            from kiro_crew.apps.builtins.mochi.soul_loader import write_agent_prompts
    ...
    def _stored_appearance(self) -> tuple[str | None, str | None]:
        """The active appearance pack id and its description.

        The description is only looked up for a NON built-in pack: that is where
        an imported pet's identity lives, and feeding it to the persona is what
        stops the prompt describing a cat while the art is something else. The
        built-ins have curated persona text keyed by pack id instead.

        Never raises: an unreadable settings file or pack must not stop the
        runtime from starting, and ``(None, None)`` reads as a generic companion.
        """
        try:
            from kiro_crew.apps.builtins.mochi.settings import BUILTIN_PACKS, load_settings
            ...
            if pack_id in BUILTIN_PACKS:
                return (pack_id, None)
            return (pack_id, self._pack_description(pack_id))
        except Exception:  # noqa: BLE001 — settings must never block startup
            logger.warning("[mochi] could not read the active appearance")
            return (None, None)

    def _pack_description(self, pack_id: str) -> str | None:
        """A user pack's own ``meta.description``, or None if unreadable."""
        try:
            from kiro_crew.apps.builtins.mochi.appearance_store import get_pack_detail

            detail = get_pack_detail(self.data_dir, pack_id) or {}
            value = (detail.get("meta") or {}).get("description")
            return value if isinstance(value, str) and value else None
        except Exception:  # noqa: BLE001 — a missing pack is not a startup error
            return None
    ...
    def _broadcast(self, channel: str, *args: Any) -> None:
        self.publish(channel, {"args": list(args)})
    ...
    def _watchlist_changed(self) -> None:
        ...
        try:
            ...
            items = []
        self.publish("mochi:watchlist-changed", {"items": items})
    ...
    def _push_to_chat(self, action: dict[str, Any]) -> None:
        ...
        self.publish("mochi:chat-push", {"content": content, "timestamp": now})
    ...
    def activity_budget(self):
        from kiro_crew.apps.builtins.mochi.activity_budget import resolve_activity_budget
...
class _WatchlistCallbacks:
    ...
    def on_notify_direct(self, summary: str) -> None:
        self._rt.publish("mochi:notify", {"summary": summary, "degraded": True})
...
def on_startup(ctx: Any) -> None:
    ...
    if _runtime is not None:
        _runtime.start()  # re-enable after disable: same instance restarts
        return

...
async def _handle_watchlist_update(request: web.Request) -> web.Response:
    ...
    if not isinstance(params, dict) or not (params.keys() & {"add", "cancel", "update"}):
        return web.json_response(
            {
                "error": "body must contain add, cancel and/or update",
...
async def _handle_pinned_get(request: web.Request) -> web.Response:
    return web.json_response({"pins": _rt().pinned.get_pins()})
...
def _redact_plan_tree(value: object) -> object:
    ...
    This mirrors the activity-log sink, which redacts the same two ways at its
    write chokepoint; the plan is the other persisted, agent-authored sink on
    that path. Applied at the read so a browser never receives an unredacted
    value regardless of what is on disk.
    """
    ...
    if isinstance(value, list):
        return [_redact_plan_tree(item) for item in value]
    if isinstance(value, dict):
        return {key: _redact_plan_tree(item) for key, item in value.items()}
    return value
...
async def _handle_packs_list(request: web.Request) -> web.Response:
    from kiro_crew.apps.builtins.mochi.appearance_store import list_packs
...
async def _handle_pack_detail(request: web.Request) -> web.Response:
    from kiro_crew.apps.builtins.mochi.appearance_store import PackError, get_pack_detail
...
async def _handle_pack_file(request: web.Request) -> web.Response:
    ...
    """
    from kiro_crew.apps.builtins.mochi.appearance_store import PackError, read_pack_file
...
async def _handle_pack_save(request: web.Request) -> web.Response:
    from kiro_crew.apps.builtins.mochi.appearance_store import PackError, save_sprite_pack
...
async def _handle_pack_delete(request: web.Request) -> web.Response:
    from kiro_crew.apps.builtins.mochi.appearance_store import PackError, delete_pack
    from kiro_crew.apps.builtins.mochi.settings import load_settings, save_settings
...
async def _handle_petdex_installed(request: web.Request) -> web.Response:
    ...
    from kiro_crew.apps.builtins.mochi.petdex_import import list_installed
...
def _reset_files() -> tuple[str, ...]:
    from kiro_crew.apps.builtins.mochi.activity_log import LOG_FILE, YESTERDAY_LOG_FILE
    from kiro_crew.apps.builtins.mochi.mcp_server import _QUEUE_FILE

...
def _blank_entry() -> dict:
    """A new top-level row in the shape `_load` will accept on the way back in.

    `_load` skips any entry without a `sid` KEY as corrupt, so a writer that creates a
    row without one produces state that is correct in memory, correct on disk, and gone
    after a restart. That contract is invisible at the call site -- `setdefault(key, {})`
    reads as obviously fine -- so it lives here rather than being spelled out per writer.
    """
...
class SessionMap:
    ...
    def _stage_mirror_link(
        ...
        *,
        ...
        # `_blank_entry`, not `{}`: a row with no `sid` KEY is discarded by `_load` as
        # corrupt. A takeover on a session that has never been persisted created exactly
        # that row, so the binding worked until the next restart and then disappeared.
        entry = data.setdefault(canon, _blank_entry())

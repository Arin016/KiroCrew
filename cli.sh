...
# Kiro Crew needs Python >=3.12 at runtime, matching the package's own
# requires-python -- older published wheels' METADATA claimed a lower bound, and
# pip would install fine there and then crash on first run. 3.12 is the series
# the project builds and tests on; 3.13 is untested and only a last resort
# before bare python3, which itself only counts if >=3.12. A host whose system
# interpreter is older is NOT turned away: it falls through to the managed-uv
# path below, which provisions a python-build-standalone 3.12 without sudo.
...
# whose base repos never reach 3.12. An installed `uv` on PATH is used as-is

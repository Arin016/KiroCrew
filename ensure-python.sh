...
#
# The package requires Python >= 3.12 (pyproject `requires-python`). Amazon
...
# import (AL2023/3.9). Neither distro can reach the floor from its own repos,
# which is exactly why the mise fallback below is not optional on them.
# Prefer any already-usable system Python >= 3.12;
...
# First usable system python, preferring the SUPPORTED minor (3.12 is the

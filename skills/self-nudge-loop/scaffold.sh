...
One page. Tells you how to launch an autonomous AutoNudge loop in this KiroClaw session for **$PROJECT**.
...
curl -X POST "http://127.0.0.1:7777/api/autonudge?token=\$TOKEN" \\
...
| Symptom | Check |
|---|---|
| UI icon dim | \`echo \$KIROCLAW_AUTONUDGE\` should be \`1\`. |
| Nudge never fires | \`cat ~/.kiroclaw/autonudge.json\` — loop \`active:true\`? Right slot_key? |
...
Skill reference: \`~/.kiroclaw/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".
EOF
)
...
3. Read the skill: \`~/.kiroclaw/skills/self-nudge-loop/SKILL.md\` §"Hardened Loop Pattern".

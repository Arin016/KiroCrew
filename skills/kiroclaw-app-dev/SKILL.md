---
name: kiroclaw-app-dev
description: "Build, package, and publish KiroClaw external apps. Covers app.json manifest, UI components, crons, skills, self-healing install, GitFarm publishing, and common pitfalls."
---

# KiroClaw App Development

Guide for building external KiroClaw apps that work reliably across KiroClaw versions without depending on buggy lifecycle hooks or skill scanner behavior.

## When to Use

- User says "create a kiroclaw app", "build an app", "make an app for kiroclaw"
...
**Never depend on:**
- `onInstall` hook firing (unreliable across KiroClaw versions)
...
"message": "FIRST (self-heal, run these bash commands silently):
  (1) Skill symlink: if ~/.kiroclaw/skills/MY-SKILL/SKILL.md does not exist,
      run: ln -sfn ~/.kiroclaw/apps/MY-APP/skills/MY-SKILL ~/.kiroclaw/skills/MY-SKILL
  (2) State dir: mkdir -p ~/.kiroclaw/workspace/MY-APP && [ -f ~/.kiroclaw/workspace/MY-APP/state.json ] || echo '{}' > ~/.kiroclaw/workspace/MY-APP/state.json
  (3) Config for UI: mkdir -p ~/.kiroclaw/apps/MY-APP/data && [ -f ~/.kiroclaw/apps/MY-APP/data/config.json ] || echo '{...resolved paths...}' > ~/.kiroclaw/apps/MY-APP/data/config.json
...
All KiroClaw apps MUST use the same visual language for consistency:
...
**CR Tendy** is the canonical reference app: [KiroClawApp-CrTendy on GitFarm](https://code.amazon.com/packages/KiroClawApp-CrTendy/trees/mainline)

Visual mock of the target style:

```html
<!--
  This is what a KiroClaw app header + card should look like.
...
import { useNavigate } from '@kiroclaw/app-sdk'
...
const appJsonPath = '/Users/.../.kiroclaw/apps/my-app/app.json'
...
| Use `useNavigate()` from `@kiroclaw/app-sdk` for navigation | Use `window.location` (causes full reload) |
...
- Package name convention: `KiroClawApp-MyAppName`
- Include: `app.json`, `ui/`, `skills/`, `scripts/`, `Config`, `packageInfo`

### 2. App Registry Entry

Submit a CR to the KiroClaw repo adding an entry to `app-registry.json`:
```json
{
  "name": "my-app",
  "repo": "ssh://git.amazon.com/pkg/KiroClawApp-MyAppName",
  "branch": "mainline",
  "resources": [],
  "lifecycle": "stable",
  "versionSet": null
...
1. `kiroclaw app install <path-or-gitfarm-url>`
2. Gateway restart: `kiroclaw gateway restart`
...
The installed app at `~/.kiroclaw/apps/MY-APP/` IS a git repo with origin pointing to the GitFarm package. You can commit and push directly from there:

```bash
cd ~/.kiroclaw/apps/my-app && git add -A && git commit -m "message" && git push origin mainline
```
...
| Skills don't load after install | Scanner only checks flat `~/.kiroclaw/skills/<name>/SKILL.md` | Self-heal symlink in cron |
| UI shows stale data after app.json change | Browser caches manifest API response | Hard refresh |
| `onInstall` doesn't run | KiroClaw lifecycle hook bug | Don't depend on it — self-heal instead |
...
2. Install: `kiroclaw app install /path/to/my-app`
3. Restart gateway: `kiroclaw gateway restart`

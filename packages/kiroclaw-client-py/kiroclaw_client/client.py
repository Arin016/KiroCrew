"""KiroClawClient — async Python client for the KiroClaw Gateway.

Standalone package (no dependency on kiro_claw main package).
Uses aiohttp for HTTP and WebSocket communication.

Mirrors the TypeScript @kiroclaw/client API surface.
"""
...
from kiroclaw_client.errors import KiroClawError, ErrorCode, http_error
...
class KiroClawClient:
    """Async HTTP client for the KiroClaw Gateway.
    ...
    def __init__(
        ...
        *,
        ...
        port = os.environ.get("KIROCLAW_PORT", "7777")
    ...
    async def __aenter__(self) -> KiroClawClient:
    ...
    async def _request(
        ...
        last_error: KiroClawError | None = None
        ...
        raise last_error or KiroClawError(ErrorCode.NETWORK_ERROR, "Request failed")
    ...
    async def register_mcp_server(
        ...
        if not name or not command:
            raise KiroClawError(ErrorCode.VALIDATION_ERROR, "MCP server requires name and command")

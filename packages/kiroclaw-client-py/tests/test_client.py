"""Tests for kiroclaw_client — mirrors TS @kiroclaw/client test coverage."""
from __future__ import annotations

import pytest

from kiroclaw_client import KiroClawClient, KiroClawError
from kiroclaw_client.errors import ErrorCode, http_status_to_code, http_error
...
class TestErrors:
    ...
    def test_to_dict(self):
        err = KiroClawError(ErrorCode.NOT_FOUND, "not found", status=404)
        d = err.to_dict()
...
class TestAuthCheck:
    def test_localhost_no_token_ok(self):
        mc = KiroClawClient(base_url="http://localhost:7777")
        # Should not raise on construction
        assert mc.base_url == "http://localhost:7777"

    def test_remote_no_token_raises(self):
        mc = KiroClawClient(base_url="http://example.com:7777")
    ...
    def test_remote_with_token_ok(self):
        mc = KiroClawClient(base_url="http://example.com:7777", token="test")
...
class TestMessageValidation:
    ...
    async def test_rejects_long_message(self):
        mc = KiroClawClient(message_length_limit=100)
        with pytest.raises(KiroClawError) as exc_info:
...
class TestContextBuffer:
    ...
    async def test_flush_preserves_failed_entries(self):
        mc = KiroClawClient(max_retries=0)

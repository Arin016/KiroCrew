...
from kiro_claw.cli_setup import (
    _configured_port,
...
class TestCustomDomainUsesConfiguredPort:
    def test_prints_configured_port(self, monkeypatch, capsys, tmp_path):
        monkeypatch.setenv("KIROCLAW_PORT", "4321")
        monkeypatch.setattr("kiro_claw.cli_setup.Path", lambda *a, **kw: tmp_path / "hosts")
        _maybe_setup_custom_domain()
...
class TestDashboardUrlUsesConfiguredPort:
    def test_input_prompt_contains_configured_port(self, monkeypatch, capsys):
        monkeypatch.setenv("KIROCLAW_PORT", "5678")
        ...
        monkeypatch.setattr("kiro_claw.cli_setup.KiroClawConfig.load", lambda: mock_cfg)
        monkeypatch.setattr("kiro_claw.cli_setup.config_path", lambda: MagicMock())
        monkeypatch.setattr("kiro_claw.cli_setup.socket.gethostbyname", lambda _: "10.0.0.1")
        monkeypatch.setattr("kiro_claw.cli_setup.socket.gethostname", lambda: "myhost")

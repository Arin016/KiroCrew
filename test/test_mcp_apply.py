...
# ---------------------------------------------------------------------------
# Scope helpers: _set_kiroclaw_entry, _set_scope_entry, _remove_kiroclaw_entry
# ---------------------------------------------------------------------------


class TestSetKiroclawEntry:
    def test_adds_entry_when_enabling_with_spec(self, tmp_path, monkeypatch):
        ...
        mc_path = tmp_path / "kiroclaw.mcp.json"
        monkeypatch.setattr(mcp_mod, "_KIROCLAW_MCP_JSON", mc_path)
        action = mcp_mod._set_kiroclaw_entry("srv", enabled=True, spec={"command": "x"})
        assert action == "added"
    ...
    def test_disables_existing(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_KIROCLAW_MCP_JSON", mc_path)
        action = mcp_mod._set_kiroclaw_entry("srv", enabled=False)
    ...
    def test_enabling_disabled_removes_flag(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_KIROCLAW_MCP_JSON", mc_path)
        action = mcp_mod._set_kiroclaw_entry("srv", enabled=True)
    ...
    def test_disabling_missing_with_spec_seeds_entry(self, tmp_path, monkeypatch):
        ...
        mc_path = tmp_path / "kiroclaw.mcp.json"
        monkeypatch.setattr(mcp_mod, "_KIROCLAW_MCP_JSON", mc_path)
        action = mcp_mod._set_kiroclaw_entry(
...
class TestApplyEndpoint:
    @pytest.mark.asyncio
    async def test_preservation_kiro_to_kiroclaw(self, tmp_path, monkeypatch):
        """Turning Kiro off when server was only in Kiro copies to KiroClaw first."""
        from kiro_claw.dashboard.handlers import mcp as mcp_mod
        ...
        agent_path = tmp_path / "kiroclaw_agent.json"
        ...
        import kiro_claw.agent

        monkeypatch.setattr(kiro_claw.agent, "rebuild_agent_config", lambda: None)
        ...
        # KiroClaw mcp.json should now have slack-mcp (preservation happened)
    ...
    async def test_calls_rebuild_agent_config_once(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mcp_mod, "_KIROCLAW_MCP_JSON", tmp_path / "mc.json")
        ...
        rebuild = MagicMock()
        monkeypatch.setattr(kiro_claw.agent, "rebuild_agent_config", rebuild)
        ...
        request = _make_request(
            {
                "changes": [
                    {"name": "a", "kiroclaw": True, "kiroGlobal": True, "ccGlobal": False},
                    {"name": "b", "kiroclaw": True, "kiroGlobal": False, "ccGlobal": True},
...
class TestHostileNameRejection:
    ...
    async def test_rejects_hostile_names(self, tmp_path, monkeypatch, bad_name):
        ...
        request = _make_request(
            {"changes": [{"name": bad_name, "kiroclaw": True}]}
        )
        resp = await mcp_mod.api_mcp_apply(request)
        body = json.loads(resp.body)

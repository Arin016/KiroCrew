...
class TestApprovalMode:
    """``pod up --approval`` is persisted per pod and forwarded at boot.

    The mode cannot ride the unit file: systemd/launchd re-enter the pod as
    ``kirocrew pod _run <name>`` with no flags, and one template unit is shared
    by every instance. So it travels through the per-pod env file, exactly as
    ``SEED`` does.
    """

...
from kiro_claw.dashboard.handlers.files import api_upload_file
...
def upload_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    pollute the real ``~/.kiroclaw/uploads/`` and don't race other tests."""
    target = tmp_path / "uploads"
    monkeypatch.setattr(
        "kiro_claw.dashboard.handlers.files._UPLOAD_DIR",
        target,
    )
    return target
...
async def test_upload_docx_emits_match_true_diagnostic(
    ...
    with caplog.at_level(
        logging.INFO, logger="kiro_claw.dashboard.handlers.files",
    ):

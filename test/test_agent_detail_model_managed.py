...
async def test_patch_explicit_model_freezes(tmp_path):
    cfg = tmp_path / "kiroclaw.json"
    cfg.write_text(json.dumps({"name": "kiroclaw", "model": "claude-old", "model_managed": True}))
    request = _patch_request("kiroclaw", {"model": "claude-new"})
...
async def test_patch_clear_model_resumes_tracking(tmp_path):
    cfg = tmp_path / "kiroclaw.json"
    cfg.write_text(
        json.dumps({"name": "kiroclaw", "model": "claude-pinned", "model_managed": False})
    )
    request = _patch_request("kiroclaw", {"model": ""})

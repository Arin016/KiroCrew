...
async def test_edit_resend_commit_keeps_a_row_injected_during_the_boundary(state) -> None:
    ...
    skips the cross-process-append scan -- so carrying it in the live window AND
    making that merged window durable is what keeps it.
    """
    slot = state.get_or_create_slot("s1")
    slot.append("user", "first")
    slot.append("assistant", "answer")
    ...
    # Carried is not enough: the rewrite persisted the prospective window only
    # and dropped the injected row's own durable copy, so the MERGED window gets
    # a durability pass before the request returns rather than waiting for the
    # periodic flush -- a crash in that gap would lose a result already shown.
    assert saved_windows[-1] == ["edited", "workflow finished"]


@pytest.mark.asyncio
async def test_edit_resend_skips_the_extra_persist_when_nothing_arrived(state) -> None:
    """The follow-up durability pass is for carried rows only.

    The normal path must not pay a second history write (nor a second await
    inside the boundary) just because the merge logic exists.
    """
    slot = state.get_or_create_slot("s1")
    slot.append("user", "first")
    slot.append("assistant", "answer")
    ...
    def _save(_state, saved_slot, msgs_snapshot=None, **_kwargs):
        window = msgs_snapshot if msgs_snapshot is not None else saved_slot.messages
        saves.append([m["content"] for m in window])
        return True

...
def test_shutdown_event_importable_without_running_loop() -> None:
    ...
    assert hasattr(kiro_claw, "shutdown_event")
    assert callable(kiro_claw.shutdown_event.set)
    assert callable(kiro_claw.shutdown_event.clear)
    assert callable(kiro_claw.shutdown_event.is_set)
...
def test_shutdown_event_survives_fresh_asyncio_run() -> None:
    ...
    async def main() -> None:
        ...
        await kiro_claw.shutdown_event.wait()

    asyncio.run(main())
    assert kiro_claw.shutdown_event.is_set()
    kiro_claw.shutdown_event.clear()
...
def test_shutdown_event_wait_for_timeout() -> None:
    ...
    async def main() -> str:
        try:
            await asyncio.wait_for(kiro_claw.shutdown_event.wait(), timeout=0.05)
...
def test_shutdown_event_does_not_bind_to_default_loop_via_get_event_loop() -> None:
    ...
    kiro_claw.shutdown_event.clear()
    kiro_claw.shutdown_event._event = None  # type: ignore[attr-defined]
    kiro_claw.shutdown_event._loop = None  # type: ignore[attr-defined]
...
def test_shutdown_event_pending_set_preserved_across_loops() -> None:
    ...
    async def main() -> bool:
        return kiro_claw.shutdown_event.is_set()
...
def test_shutdown_event_get_raises_without_loop() -> None:
    ...
    kiro_claw.shutdown_event._loop = None  # type: ignore[attr-defined]
    try:
        kiro_claw.shutdown_event._get()  # type: ignore[attr-defined]

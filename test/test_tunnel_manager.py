...
def manager():
    ...
    return TunnelManager(port=7777, name_mode="username", name_override=None)
...
class TestTunnelName:
    ...
    def test_hash_mode(self):
        mgr = TunnelManager(port=7777, name_mode="hash")
        name = mgr._tunnel_name()
        assert name.startswith("kiroclaw-")
        assert len(name) == len("kiroclaw-") + 8  # 8-char hash

    def test_override(self):
        mgr = TunnelManager(port=7777, name_override="my-custom-tunnel")

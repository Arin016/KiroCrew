...
class TestReplayRefusesAPopulatedRoot(unittest.TestCase):
    ...
    def test_an_EMPTY_owned_crew_does_not_block_a_retry(self):
        """The guard must refuse a populated root, not a half-created one.

        `_ensure_crew` creates the crew BEFORE the first work item is committed, so a
        failure in that window (ENOSPC, a crash) leaves a crew of ours holding no
        transitions. Counting it made the guard refuse forever: every retry saw
        "crews already exist" and stopped before recording anything, and the error
        told the operator to replay into a fresh root -- advice that cannot recover
        this state. Unrecoverable-by-construction, from one unlucky write.

        Safe to admit because the guard exists to prevent DUPLICATE transitions and
        the corrupted dwell they cause, and a crew with no work items has none.
        """
        # Reproduce the interrupted first commit: the crew gets created, then the
        # work-item write fails, so the crew exists and is owned but is empty.
        real_ensure = pipeline_bridge._ensure_crew
        created: list[str] = []

        def ensure_then_fail(owner, repo, pipeline, number, root):
            crew_id = real_ensure(owner, repo, pipeline, number, root)
            created.append(str(crew_id))
            raise OSError(28, "No space left on device")

        with mock.patch.object(pipeline_bridge, "_ensure_crew", ensure_then_fail):
            with self.assertRaises(OSError):
                self._replay(1)

        self.assertTrue(created, "the crew must have been created before the failure")
        self.assertEqual(
            pipeline_bridge.crew_store.list_work_items(OWNER, REPO, created[0], self.root),
            [],
            "the crew must be left EMPTY -- that is the state under test",
        )

        # The retry must proceed rather than refuse.
        summary = self._replay(1)
        self.assertIsNotNone(summary, "a retry after a failed first commit must not refuse")
    ...
    def test_the_guard_and_the_write_loop_are_one_locked_operation(self):
        ...
        Asserted STRUCTURALLY -- the lock acquired before the guard's read is still
        held when the loop performs its first write -- rather than by probing OS
        lock state. Two reasons, and the second is why this is not merely a
        portability fix:

        * The old probe called `fcntl.flock` directly, so it could not even import
          on Windows (`No module named 'fcntl'`) and reddened that shard, while the
          invariant it checks is platform-independent logic. Production locks
          through `platform_compat.file_lock`, which is portable; only the test was
          not.
        * The probe could only show that SOME lock was held during the write. It
          would still pass if the guard and the loop each took and released their
          own lock -- which is exactly the TOCTOU this test exists to forbid. This
          version fails on that split, because it requires no release between the
          guard read and the first write.
        """
        trail: list[str] = []
        real_lock = pipeline_bridge.platform_compat.file_lock
        real_list = pipeline_bridge.crew_store.list_crews
        real_ensure = pipeline_bridge._ensure_crew

        @contextlib.contextmanager
        def recording_lock(fd, **kwargs):
            trail.append("enter")
        ...
        def note_guard(*args, **kwargs):
            trail.append("guard")
            return real_list(*args, **kwargs)

        def note_write(owner, repo, pipeline, number, root):
            trail.append("write")
            return real_ensure(owner, repo, pipeline, number, root)

        with (
            mock.patch.object(pipeline_bridge.platform_compat, "file_lock", recording_lock),
            mock.patch.object(pipeline_bridge.crew_store, "list_crews", note_guard),
            mock.patch.object(pipeline_bridge, "_ensure_crew", note_write),
        ):
            self._replay(1)

        self.assertIn("guard", trail, "the precondition must read the crew list")
        self.assertIn("write", trail, "the loop must create a crew")
        guard_at = trail.index("guard")
        write_at = trail.index("write")
        self.assertLess(guard_at, write_at, "the guard must read before the loop writes")
        # A lock was taken before the guard...
        self.assertIn("enter", trail[:guard_at], "the guard must run under a lock")
        # ...and NOT released before the write, so both are one operation.
        self.assertNotIn(
            "exit",
            trail[guard_at:write_at],
            "the lock was released between the guard and the write -- that window is "
            "the TOCTOU this test forbids",
        )
...
class TestTornLinesAreCountedNotSilent(unittest.TestCase):
    ...
    def test_a_unicode_separator_inside_a_record_does_not_fragment_it(self):
        """JSONL's delimiter is the newline and nothing else.

        `splitlines()` also breaks on U+2028, U+2029, U+0085, \\v and \\f, and those
        arrive RAW inside records because the events carry forge text nobody escaped
        for us. Such a line was cut in two, both halves failed json.loads, so a VALID
        record was dropped and double-counted as unparseable -- its stage transition
        silently missing. The loss was invisible: the only signal was an unparseable
        count that looked like ordinary log corruption.
        """
        path = self.root / "audit.jsonl"
        # RAW separators, which is the whole point: `json.dumps` defaults to
        # ensure_ascii=True and escapes U+2028 into the six characters \\u2028, so a
        # record built that way carries no separator byte at all and CANNOT fragment.
        # A first version of this test did exactly that and passed against the broken
        # `splitlines()` reader -- a vacuous test. `ensure_ascii=False` passes the
        # character through, which is what a writer emitting UTF-8 JSONL does.
        hostile_title = "fix\u2028the\u2029line\u0085reader"
        lines = [
            json.dumps({"event": "scan", "issue": 1}),
            json.dumps(
                {"event": "implement_start", "issue": 2, "title": hostile_title},
                ensure_ascii=False,
            ),
            json.dumps({"event": "scan", "issue": 3}),
        ]
        raw = "\n".join(lines) + "\n"
        # Guard the fixture itself: if this is ever escaped again the test silently
        # stops testing anything.
        self.assertIn("\u2028", raw, "the fixture must contain a RAW separator")
        path.write_text(raw, encoding="utf-8")

        events, unparseable = pipeline_bridge.read_event_log(path)
        self.assertEqual(unparseable, 0, "a valid record must not read as corruption")
        self.assertEqual([e["issue"] for e in events], [1, 2, 3])
        # And the text survives intact rather than arriving truncated at the separator.
        self.assertEqual(events[1]["title"], hostile_title)

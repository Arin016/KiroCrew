...
class TestATakeoverOnAFreshSlotSurvivesARestart:
    """In-memory correctness is not persistence: `_load` skips rows with no `sid` KEY.

    A confirmed takeover by a session that has never been saved created its row with a
    bare `{}`, so the row carried only `mirrors`. Everything looked right — the claim
    succeeded, the occupancy scan saw it, the file on disk contained it — and then the
    next restart dropped the row as corrupt and the channel binding was simply gone.
    Asserted by reading it back through a SECOND SessionMap over the same file, because
    that is the only place the defect is visible.
    """

    LOC = ChannelLink(channel_type="discord", channel_id="dm-fresh")

    def test_the_binding_is_still_there_after_reloading_from_disk(self, tmp_path):
        with patch("kiro_crew.session_map.config_dir", return_value=tmp_path):
            first = SessionMap()
            # The occupant is a persisted session; the claimant has never been saved.
            first.set_mirror_link("dashboard:occupant", self.LOC, accepts_inbound=True)

            displaced = first.replace_mirror_owner("dashboard:newcomer", self.LOC)
            assert displaced, (
                "the takeover displaced nobody, so this is not the case under test"
            )
            assert first.get_mirror_link("dashboard:newcomer", "discord") == self.LOC

            # A second map over the same file is what a restart actually does.
            reloaded = SessionMap()

        assert reloaded.get_mirror_link("dashboard:newcomer", "discord") == self.LOC, (
            "the takeover's row was discarded on load, so the channel the user just "
            "connected is silently unbound after a restart"
        )
        assert reloaded.find_mirror_sessions(self.LOC) == ["dashboard:newcomer"], (
            "the reloaded map does not recognise the conversation's owner"
        )

...
def _config_dir_stays_in_tmp(tmp_path, monkeypatch):
    """Keep the incidental ``config_dir()`` write off the checkout root.

    These tests point ``KIROCREW_HOME`` at deliberately bogus spellings -- Windows
    drive paths (``D:/opt/crewdata``), bare roots (``\\``), and other RELATIVE
    strings -- purely to exercise ``security``'s container-name *string* matching.
    They are not real homes. But the scanner's ``_custom_home_ancestors`` reads the
    configured sandbox tier via ``sandbox.configured_sandbox_mode()``, which runs a
    full ``KiroCrewConfig.load()``, and ``config_dir()`` CREATES its directory as a
    side effect of merely resolving where config lives. On Linux a relative
    ``KIROCREW_HOME`` resolves against the CWD -- the repo checkout -- so that mkdir
    lands entries like ``D:`` and ``\\`` at the repository root, which the
    ``pytest_sessionfinish`` residue guard then fails the whole shard on (CI-only,
    because only there is the CWD the checkout).

    Redirect the loader's ``config_dir`` seam -- the one ``config.loader`` documents
    for exactly this ``patch("kiro_crew.config.loader.config_dir", ...)`` purpose --
    to a per-test tmp dir. The matching logic under test is unaffected: it reads
    ``KIROCREW_HOME`` straight from the environment, not through ``config_dir()``.
    """
...
class TestTheNamedUserTildeIsNotAWayAround:
    """`~name` expands the same as `~`, so it must fence the same.

    `os.path.expanduser("~runner")` and bash both resolve a POSIX named-user tilde to
    that user's home, so `~<owner>/.kiro/crew` names the real container. The raw scan
    anchored only on a BARE `~`, so an interpreter payload -- the ONE layer that sees
    inside a `-c` string -- passed `os.rename(os.path.expanduser("~runner/.kiro/crew"))`
    straight through and could relocate the fenced container (GPT review).
    """
    ...
    @pytest.mark.parametrize(
        "command",
        [
            # A concrete username (the running user, so the expansion is real here too)
            f"mv ~{_USER}/.kiro/crew /tmp/x",
            f"python3 -c \"import os; os.rename(os.path.expanduser('~{_USER}/.kiro/crew'),'/tmp/x')\"",
            # A generic name the raw scan cannot resolve but bash would -- the payload
            # form is the one the operand normalizer never sees.
            "mv ~runner/.kiro/crew /tmp/x",
            "python3 -c \"import os; os.rename('~runner/.kiro/crew','/tmp/x')\"",
            # Native Windows spelling in a payload: `win_home_alts` reuses the same
            # tilde alternative, so the login-name run must stop at the backslash.
            "python3 -c \"import os; os.rename('~runner\\\\.kiro\\\\crew','D:\\\\tmp')\"",
        ],
    )
    def test_a_named_user_tilde_still_names_the_container(self, command: str) -> None:
        assert _blocked(command), f"{command!r} relocates the container via ~name"

    @pytest.mark.parametrize(
        "command",
        [
            f"cat ~{_USER}/.kiro/crew/sessions/a.json",
            "echo ~someuser",
            "cat ~/notes.txt",
        ],
    )
    def test_it_does_not_over_reach(self, command: str) -> None:
        """The named-user run must not fence a read UNDER the container, nor a bare
        `~name` mention that names no protected path -- the exact-not-prefix property
        the container gate already promises for bare `~`."""
        assert not _blocked(command), f"{command!r} is ordinary usage, not a relocation"

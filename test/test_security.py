...
class TestModelDirectoryIsWriteProtected:
    """The downloaded-model directory is an input to a trust decision, so it is write-only.

    ``embeddings.models_dir()`` is ``<data home>/models``: it verifies its GGUF against a
    pinned sha256 and then hands the PATH to a native loader, so a writable directory
    leaves a window between the digest and the open in which the bytes can be swapped.
    Re-hashing does not close it, because the loader re-opens by name; removing the
    writability does. A poisoned model is persistent and invisible.
    ...
    MODEL_PATHS = (
        ...
        "~/.kiro/crew/models/nested/weights.gguf",
        "~/.kirocrew/models/qwen3-embedding-0.6b.gguf",
    )
    ...
    @pytest.mark.parametrize(
        ...
        (
            'cp /tmp/evil.gguf "{p}"',
            'echo forged > "{p}"',
            'dd if=/tmp/evil.gguf of="{p}"',
            'install -m 0644 /tmp/evil.gguf "{p}"',
            'tee "{p}" < /tmp/evil.gguf',
        ),
    )
    ...
    @pytest.mark.parametrize(
        "command",
        (
            "cd ~/.kiro/crew/models && cp /tmp/evil.gguf w.gguf",
            "cd ~/.kirocrew/models ; echo x > a.gguf",
            "cd ~/.kirocrew/models/ ; echo x > a.gguf",
            # The archive form, where the member name is INSIDE the tarball and so is
            # unavailable to a name match. The `cd` target is what catches it.
    ...
    @pytest.mark.parametrize(
        "command",
        (
            # An unrelated directory called `models`, and an unrelated `.gguf`.
            "cp /tmp/e models/a.gguf",
            "cd models && ls",
            "grep -r models src/",
        ),
    )
    def test_an_unanchored_models_directory_stays_allowed(self, command: str) -> None:
        """The fence is home-anchored: a project-local `models/` is not the data home's."""

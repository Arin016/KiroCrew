...
class TestImportedTextIsNeverExpanded:
    def test_a_skill_body_token_is_left_literal(self):
        ...
        with patch.object(
            ...
            authored, blocks, names = chat_runner._resolve_dollar_skills(
                "run $dep", state, slot, "dashboard:x"
            )
        # Names, not a count: the resolver runs under `asyncio.to_thread`, so it hands
        # the chip data back for the caller to announce on the loop thread rather than
        # touching `slot` itself.
        assert names == ["dep"]

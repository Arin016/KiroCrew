...
def _import_ec2_profile_re():
    ...
    # After #6064: ec2 no longer has a local _PROFILE_RE; it aliases
    # _PROFILE_SPEC = profiles_mod.PROFILE_SPEC.  Reconstruct the regex from
    # the spec's pattern (same object as constants.AWS_PROFILE_NAME_RE).
    spec = ec2._PROFILE_SPEC
    return spec.pattern, spec
...
def _import_handlers_profile_re():
    ...
    # After #6064: handlers no longer has a local _PROFILE_RE; it aliases
    # _PROFILE_SPEC = profiles_mod.PROFILE_SPEC.  Reconstruct the regex from
    # the spec's pattern (same object as constants.AWS_PROFILE_NAME_RE).
    spec = handlers._PROFILE_SPEC
    return spec.pattern, spec
...
class TestSourcePatternsMatch:
    """Verify each source file contains the correct profile-regex reference.

    After #6064 (which absorbed #6055's fix), the four sites no longer each
    spell out re.compile(r"...").  Instead they alias to the single source of
    truth in constants.AWS_PROFILE_NAME_RE.  These tests pin that alias shape
    so a future refactor cannot silently re-introduce a divergent copy.
    """
    ...
    def test_ec2_pattern(self):
        ...
        # ec2 aliases to profiles_mod.PROFILE_SPEC (single source of truth, #6064)
        assert "_PROFILE_SPEC = profiles_mod.PROFILE_SPEC" in src

    def test_profiles_pattern(self):
        src = self._read_source("src/kiro_crew/deploy/profiles.py")
        # profiles.py aliases to constants.AWS_PROFILE_NAME_RE (#6064)
        assert "_PROFILE_RE = AWS_PROFILE_NAME_RE" in src

    def test_handlers_pattern(self):
        src = self._read_source("src/kiro_crew/deploy/handlers.py")
        # handlers aliases to profiles_mod.PROFILE_SPEC (single source of truth, #6064)
        assert "_PROFILE_SPEC = profiles_mod.PROFILE_SPEC" in src

    def test_validation_pattern(self):
        src = self._read_source("src/kiro_crew/validation.py")
        # validation.py aliases to constants.AWS_PROFILE_NAME_RE (#6064)
        assert "_WM_PROFILE_RE = AWS_PROFILE_NAME_RE" in src

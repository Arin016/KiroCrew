...
from kiro_claw.slack.events import _publish_home_tab
...
class TestPublishHomeTabHappyPath:
    ...
    async def test_builds_valid_home_view(self, _mw, _fmt, _yolo):
        ...
        blocks = view["blocks"]
        text = str(blocks)
        assert "KiroClaw Status" in text
...
class TestViewTypeAlwaysHome:
    @pytest.mark.asyncio
    @patch("kiro_claw.slack.events.is_yolo_mode", return_value=False)
    @patch("kiro_claw.slack.events.format_schedule", return_value="*")
...
class TestPublishHomeTabCapabilities:
    ...
    @patch("kiro_claw.slack.events.is_yolo_mode", return_value=False)
    @patch(
        "kiro_claw.slack.events.list_servers",
        return_value=[SimpleNamespace(name="builder-mcp"), SimpleNamespace(name="kiroclaw-core")],
    )
    @patch(
...
class TestPublishHomeTabUptime:
    ...
    @pytest.mark.asyncio
    @patch("kiro_claw.slack.events.is_yolo_mode", return_value=False)
    @patch("kiro_claw.slack.events.Stats")
    @patch("kiro_claw.slack.events.list_servers", return_value=[])
    @patch("kiro_claw.slack.events._get_skills_loader")
    @patch(
        "kiro_claw.slack.events.get_midway_status_line",
...
class TestPublishHomeTabSessions:
    ...
    def _grant_home_tab_auth(self, monkeypatch):
        ...
        """
        monkeypatch.setattr("kiro_claw.slack.events.is_owner", lambda _: True)
        monkeypatch.setattr(
            "kiro_claw.slack.events.is_allowed_user", lambda _: True
        )

...
def _stat_only_for(monkeypatch, target: object, st: os.stat_result) -> None:
    """Report *st* for *target* alone, delegating every other path to the real stat.

    A blanket ``os.stat`` stub is not usable here: ``Path.exists()`` calls ``os.stat``,
    so replacing it wholesale makes EVERY path report as existing -- which silently flips
    the guard's "was a managed directory ever provisioned here" test and made an
    unprovisioned-path test exercise the provisioned branch instead.
    """
    real = os.stat
    ...
    monkeypatch.setattr(governance.os, "stat", fake)
...
class TestTheManagedDirectoryMustNotBeUserWritable:
    ...
    def test_a_sticky_group_writable_holder_reads_as_genuine_absence(self, monkeypatch, tmp_path):
        ...
        # Root-owned AND sticky, exactly as macOS ships /Library. Synthetic because a test
        # can neither chown to root nor rely on a runner's own /Library.
        monkeypatch.setattr(
            governance.os,
            "stat",
            lambda p, **kw: _fake_dir_stat(uid=0, mode=0o1775),
        )
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    @_POSIX_ONLY
    def test_a_sticky_holder_owned_by_someone_else_is_still_refused(self, monkeypatch, tmp_path):
        """Sticky protects the OWNER's entries, so the exemption is root-only.

        A sticky directory owned by a non-root account still lets that owner remove
        anything in it, so exempting sticky unconditionally would have handed the
        exemption to whoever owns the directory. Real here: the directory is genuinely
        ours, genuinely sticky, and genuinely group-writable.
        """
    ...
    def test_a_holder_owned_by_another_non_root_account_is_not_a_route_for_us(
        ...
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        monkeypatch.setattr(os, "getuid", lambda: 1000, raising=False)
        # Scoped to ``tmp_path`` so the absent managed directory really reads as absent.
        _stat_only_for(monkeypatch, tmp_path, _fake_dir_stat(uid=65534, mode=0o755))
        # The MANAGED DIRECTORY itself is absent -- nothing was ever provisioned here, so
        # the question lands on an ancestor whose ownership is a fact about the OS image.
        _point_managed(monkeypatch, tmp_path / "absent-kirocrew" / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    @_POSIX_ONLY
    def test_a_provisioned_directory_owned_by_another_account_IS_refused(
        self, monkeypatch, tmp_path
    ):
        """The counterpart, and the hole the id-mapped carve-out would otherwise leave.

        When the managed directory EXISTS it is a provisioned artifact, so its owner can
        remove its entries and the absence cannot be trusted -- whoever that owner is.
        Judging the artifact directly is what lets the unprovisioned case stay lenient
        without also excusing a tampered one.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        monkeypatch.setattr(os, "getuid", lambda: 1000, raising=False)
        holder = tmp_path / "kirocrew"
        holder.mkdir()
        _stat_only_for(monkeypatch, holder, _fake_dir_stat(uid=65534, mode=0o755))
        _point_managed(monkeypatch, holder / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
            governance._read_managed_policy()

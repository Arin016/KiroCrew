...
from kiro_claw.dashboard.handlers import api_file_raw
...
def mock_sel():
    with patch("kiro_claw.sel.sel") as m, \
         patch("kiro_claw.security.is_sensitive_path", return_value=False):
        instance = MagicMock()
...
async def test_rejects_missing_file(tmp_path, mock_sel):
    ...
    with patch("kiro_claw.dashboard.handlers._validate_dashboard_path", return_value=missing):
        async with TestClient(TestServer(_make_app())) as client:

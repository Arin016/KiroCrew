...
from kiro_claw.sandbox import _probe_sandbox_exec

_MAC_VER_BELOW_26 = ("15.0.0", ("", "", ""), "")
_MAC_VER_26_PLUS = ("26.4.1", ("", "", ""), "")


@patch("kiro_claw.sandbox.sys")
def test_non_darwin_returns_false(mock_sys):
    mock_sys.platform = "linux"
    assert _probe_sandbox_exec() is False


@patch("kiro_claw.sandbox.platform.mac_ver", return_value=_MAC_VER_26_PLUS)
@patch("kiro_claw.sandbox.sys")
...
@patch("kiro_claw.sandbox.platform.mac_ver", return_value=_MAC_VER_BELOW_26)
@patch("kiro_claw.sandbox.sys")
@patch("kiro_claw.sandbox.shutil.which", return_value=None)
...
@patch("kiro_claw.sandbox.platform.mac_ver", return_value=_MAC_VER_BELOW_26)
@patch("kiro_claw.sandbox.sys")
@patch("kiro_claw.sandbox.shutil.which", return_value="/usr/bin/sandbox-exec")
@patch("kiro_claw.sandbox.subprocess.run")
...
@patch("kiro_claw.sandbox.sys")
@patch("kiro_claw.sandbox.shutil.which", return_value="/usr/bin/sandbox-exec")
@patch("kiro_claw.sandbox.subprocess.run", side_effect=OSError("timeout"))

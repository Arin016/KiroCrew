...
async def test_watchdog_shuts_down_when_assets_vanish():
    ...
    with patch(
        "kiro_claw.dashboard.stale_asset_watchdog.assets_present",
...
def test_token_probe_warns_on_stale_dashboard():
    ...
    from kiro_claw.cli_server import _probe_dashboard_health
    ...
    with patch("kiro_claw.cli_server.urllib.request.urlopen", return_value=mock_resp), \
...
def test_token_probe_silent_on_healthy_dashboard():
    ...
    healthy_body = b"<!DOCTYPE html><html><head><title>KiroClaw</title></head></html>"
    mock_resp = MagicMock()
...
def test_token_probe_silent_on_network_error():
    ...
    with patch("kiro_claw.cli_server.urllib.request.urlopen", side_effect=OSError("connection refused")), \
         patch("sys.stderr", stderr_capture):
        _probe_dashboard_health(7777)

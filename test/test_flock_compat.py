...
class TestFlockCompat:
    def test_posix_delegates_to_real_fcntl(self):
        ...
        from kiro_claw import flock_compat
    ...
    def test_windows_fallback_is_a_noop(self, monkeypatch):
        ...
        # CLI (and `kiroclaw cloud` on Windows) can import the lock-using modules.
        sys.modules.pop("kiro_claw.flock_compat", None)
        ...
        try:
            fc = importlib.import_module("kiro_claw.flock_compat")
            ...
            monkeypatch.undo()
            sys.modules.pop("kiro_claw.flock_compat", None)
            importlib.import_module("kiro_claw.flock_compat")
    ...
    def test_cli_import_graph_has_no_bare_fcntl_import(self):
        ...
        # `kiro_claw.cli` at import time may do a bare `import fcntl` (POSIX-only)
        # — they must go through flock_compat. A future bare import would crash
        # `python -m kiro_claw cloud launch` on Windows before the handler runs.
        #
        ...
        code = (
            ...
            "    if not name.startswith('kiro_claw'):\n"
            "        continue\n"

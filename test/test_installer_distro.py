...
def _run_cli_with_fake_env(
    ...
    *,
    ...
    #
    # This list is deliberately a SUPERSET of cli.sh's ladder: it still shadows
    # python3.11/python3.10 even though the >=3.12 floor means cli.sh no longer
    # probes them. Shadowing a name cli.sh ignores costs one unused stub, while
    # failing to shadow one it might probe lets a real host interpreter (or a
    # version-manager shim) leak into an "isolated" PATH and silently invalidate
    # the no-usable-python premise -- so the list errs toward covering more.

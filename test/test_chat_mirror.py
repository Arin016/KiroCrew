...
def _wire_binding_state(sessions, initial=None, *, paused=False):
    ...
    The MUTE is part of this state, seeded with ``paused=True`` for the reconnect
    fixtures. It is wired rather than frozen because a rebind DROPS the mute in
    production: freezing it True across a reconnect described a system that does not
    exist, and made an exemption from the mute check look necessary when it was not.

    Do NOT reassign `set_mirror_link` / `set_mirror_paused` / `is_mirror_paused` to a
    blank `MagicMock()` to inspect a call -- the mocks installed here already record
    `call_args`, and replacing one drops the state, so the next read answers wrongly.
    """
    ...
    if initial is not None:
        ...
        if paused:
            muted.add(initial.channel_type)
    ...
    def _set(_key, link, *, accepts_inbound=False):
        ...
        # A rebind drops the mute, like the real writer: this is the step that lifts
        # it on reconnect, so the catch-up that follows runs against an UNMUTED
        # binding and needs no exemption from the mute check.
        muted.discard(link.channel_type)

    def _clear(_key, channel_type=""):
        if channel_type:
            muted.discard(channel_type)
    ...
    def _set_paused(_key, value, channel_type=""):
        target = channel_type or (next(iter(bindings)) if len(bindings) == 1 else "")
        if not target:
            return
        if value:
            muted.add(target)
    ...
    def _is_paused(_key, channel_type=""):
        # A real bool: the production predicate demands `is True`, so a truthy
        # MagicMock child would read as NOT muted and sail through every gate.
        if channel_type:
            return channel_type in muted
        return bool(bindings) and all(ct in muted for ct in bindings)
    ...
    sessions.set_mirror_paused = MagicMock(side_effect=_set_paused)
    sessions.is_mirror_paused = MagicMock(side_effect=_is_paused)
    return bindings


def _wire_session_scoped_bindings(sessions, initial=None, muted=()):
    """Bindings and mutes keyed by SESSION as well as channel -- the takeover shape.

    `_wire_binding_state` keys by channel alone, which is enough while one session is
    involved. A takeover involves two, and the claim BINDS THE CLAIMANT -- so a double
    that answers `None` for the claimant makes the still-bound check at the send
    boundary refuse a delivery the test is trying to observe failing.

    *initial* maps session key to its held link; *muted* names the sessions that start
    muted.
    """
    bindings: dict[str, object] = dict(initial or {})
    paused: set[str] = set(muted)

    def _get(key, channel_type=""):
        link = bindings.get(key)
        if link is None:
            return None
        if channel_type and getattr(link, "channel_type", None) != channel_type:
            return None
        return link

    def _set(key, link, *, accepts_inbound=False):
        bindings[key] = link
        # A rebind drops the mute, like the real writer.
        paused.discard(key)

    def _clear(key, channel_type=""):
        paused.discard(key)
        return bindings.pop(key, None) is not None

    def _clear_at(link):
        hit = [key for key, held in bindings.items() if held == link]
        for key in hit:
            bindings.pop(key, None)
            paused.discard(key)
        return hit

    def _set_paused(key, value, channel_type=""):
        if value:
            paused.add(key)
        else:
            paused.discard(key)

    def _is_paused(key, channel_type=""):
        return key in paused

    sessions.get_mirror_link = MagicMock(side_effect=_get)
    sessions.set_mirror_link = MagicMock(side_effect=_set)
    sessions.clear_mirror_link = MagicMock(side_effect=_clear)
    sessions.clear_mirror_links_at = MagicMock(side_effect=_clear_at)
    sessions.set_mirror_paused = MagicMock(side_effect=_set_paused)
    sessions.is_mirror_paused = MagicMock(side_effect=_is_paused)
    return bindings
...
class TestTheConversationIsClaimedBeforeAnythingIsDelivered:
    ...
    async def test_the_claim_is_written_before_any_message_is_sent(
        ...
        order: list[str] = []
        # Chained, not replaced: the wired side effect is what records the binding,
        # and the send below re-asks whether this session is still bound to it.
        _really_claim = state.sessions.set_mirror_link.side_effect

        def _note_claim(*args, **kwargs):
            order.append("claim")
            return _really_claim(*args, **kwargs)

        state.sessions.set_mirror_link.side_effect = _note_claim
...
class TestATakeoverIsReversibleAndSerialized:
    ...
    async def test_a_failed_takeover_restores_the_evicted_binding(
        ...
        _wire_session_scoped_bindings(state.sessions, {self.OCCUPANT: held})
        _wire_replace_mirror_owner(state.sessions)
    ...
    async def test_a_failed_rebind_restores_the_previous_INBOUND_MODE_too(
        ...
        # Chained onto the stateful write, not instead of it: dropping the binding
        # write makes the send boundary's still-bound check refuse the delivery whose
        # failure this test is about.
        _really_set = state.sessions.set_mirror_link.side_effect

        def _set_link(key, link, *, accepts_inbound=False):
            if accepts_inbound:
                claimed["inbound"] = True
            return _really_set(key, link, accepts_inbound=accepts_inbound)
    ...
    async def test_a_failed_takeover_restores_the_occupants_MUTE_too(
        ...
        held = ChannelLink(channel_type="discord", channel_id="user:123")
        state.sessions.find_mirror_sessions = MagicMock(return_value=[self.OCCUPANT])
        # The occupant holds the conversation and had MUTED it before the takeover.
        _wire_session_scoped_bindings(
            state.sessions, {self.OCCUPANT: held}, muted=[self.OCCUPANT]
        )
        _wire_replace_mirror_owner(state.sessions)
        state.sessions.mirror_accepts_inbound = MagicMock(return_value=True)
...
class TestADisconnectDuringTheClaimStopsTheFirstSend:
    """The connect's OWN first message is gated too, not just the replay after it.

    Claiming the conversation is disk I/O behind `asyncio.to_thread`, so a disconnect has
    time to land between the claim and the greeting that follows it. Governance alone
    cannot see that: it answers "is this channel allowed", not "is this session still
    attached to it, and did the user just mute it".
    """

    LINK = ChannelLink("discord", channel_id="dm-1")

    @pytest.mark.asyncio
    async def test_nothing_is_sent_when_the_user_disconnects_during_the_claim(
        self, tmp_path, monkeypatch
    ):
        state = _prep(tmp_path, monkeypatch)
        transport = _fake_transport("discord", resumes=True)
        ...
        # The disconnect lands as the claim completes.
        _really_claim = state.sessions.set_mirror_link.side_effect

        def _claim_then_disconnect(key, link, *, accepts_inbound=False):
            _really_claim(key, link, accepts_inbound=accepts_inbound)
            state.sessions.set_mirror_paused(key, True, link.channel_type)

        state.sessions.set_mirror_link = MagicMock(side_effect=_claim_then_disconnect)
        ...
        assert resp.status == 403, (
            "a connect the user cancelled mid-claim reported success"
        )
        transport.send_message.assert_not_awaited()


class TestADisconnectDuringTheReplayStopsIt:
    """The replay re-asks the mute per unit, so a mid-flight disconnect ends it.

    The rebind at the top of a reconnect is what LIFTS the mute, so the replay always
    starts unmuted -- which means a mute observed part-way through is not the old one
    lingering, it is a fresh disconnect. The rest of a private transcript must not
    follow the user out of the conversation they just left.
    """

    LINK = ChannelLink("discord", channel_id="dm-1")

    @pytest.mark.asyncio
    async def test_the_remaining_history_is_not_posted(self, tmp_path, monkeypatch):
        state = _prep(tmp_path, monkeypatch)
        transport = _fake_transport("discord", resumes=True)
        state.register_channel_transport(transport)
        _wire_binding_state(state.sessions, self.LINK, paused=True)
        state.sessions.find_mirror_sessions = MagicMock(return_value=[])
        state.sessions.mirror_accepts_inbound = MagicMock(return_value=True)

        slot = state.get_or_create_slot("s1")
        for i in range(8):
            slot.append("user" if i % 2 == 0 else "assistant", f"private line {i}")
        slot.drain()

        # The user disconnects while the second unit is in flight.
        sent: list[str] = []

        async def _send(_conversation, text, thread_id=None):
            sent.append(text)
            if len(sent) == 2:
                state.sessions.set_mirror_paused("dashboard:s1", True, "discord")
            return "ts"

        transport.send_message = AsyncMock(side_effect=_send)

        async with TestClient(TestServer(_make_mirror_app(state))) as client:
            resp = await client.post("/api/chat/slots/s1/mirror-link")

        assert resp.status == 403, (
            "a reconnect interrupted by a disconnect reported success"
        )
        assert len(sent) == 2, (
            "the replay kept posting private history after the user disconnected: "
            f"{len(sent)} units reached the conversation"
        )

    @pytest.mark.asyncio
    async def test_an_uninterrupted_replay_still_delivers_everything(
        self, tmp_path, monkeypatch
    ):
        """The gate must not be closed by default -- the rebind lifts the mute."""
        state = _prep(tmp_path, monkeypatch)
        transport = _fake_transport("discord", resumes=True)
        state.register_channel_transport(transport)
        _wire_binding_state(state.sessions, self.LINK, paused=True)
        ...
        slot = state.get_or_create_slot("s1")
        for i in range(4):
            slot.append("user" if i % 2 == 0 else "assistant", f"line {i}")
        slot.drain()
        ...
        assert transport.send_message.await_count >= 2, (
            "the replay delivered nothing — asking the mute per unit closed the gate "
            "on the very operation that lifts it"
        )
...
class TestMirrorReminder:
    ...
    async def test_a_muted_mirror_reconnects_and_catches_the_conversation_up(
        ...
        _wire_binding_state(state.sessions, link, paused=True)
...
class TestAFailedCatchUpIsNotReportedAsSuccess:
    ...
    def _prepped(tmp_path, monkeypatch, *, muted):
        ...
        state.register_channel_transport(transport)
        link = ChannelLink("discord", channel_id="dm-1")
        _wire_binding_state(state.sessions, link if muted else None, paused=muted)
        state.sessions.find_mirror_sessions = MagicMock(return_value=[])
        state.sessions.clear_mirror_links_at = MagicMock(return_value=[])
...
class TestAReconnectRollbackDoesNotResurrectAnUnlink:
    ...
    This was a check followed by two writes, which left a window rather than closing
    one: the in-channel handlers take no conversation lock, so a clear landing between
    the check and the writes made the guard a lie. It is a single
    `restore_mirror_owner` call now, which samples and writes under one lock hold.
    ...
    def test_the_rollback_is_one_atomic_restore(self):
        import inspect

        from kiro_crew.dashboard import chat_mirror

        src = inspect.getsource(chat_mirror)
        # Pinned to the whole argument list. The bare name appears in prose AND this
        # module makes a SECOND `restore_mirror_owner` call at a different indent, so a
        # substring match on the name passed with this call site reverted -- the probe
        # caught exactly that.
        assert (
            ...
            "                        link,\n"
            "                        [],\n"
            "                        (link, previous_inbound, True),\n"
        ) in src, (
            "the reconnect rollback no longer goes through the atomic restore with all "
            "three fields, so the binding sample and the writes can drift apart again "
            "and a rollback can leave a binding connected that the user had muted"
        )
        # The composed version must be GONE, not merely superseded: its window is the
        # entire reason for the change.
        assert "                if current != link:\n" not in src, (
            "the old check-then-write rollback is still there, and an unlink landing "
            "between its check and its writes still gets overruled"
        )

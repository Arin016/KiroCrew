...
def _req(
    *,
    ...
    ``app_token`` is the verified app claim the auth middleware publishes, and it is a
    different thing from ``app`` (the aiohttp application): ``""`` means the dashboard
    user, a name means an app token, and ``None`` reproduces a path where no auth
    middleware ran and the claim is absent.
    """
...
class TestSttInstall:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("app_token", ("some-app", None))
    async def test_an_app_token_cannot_reach_the_host_installer(
        self, monkeypatch, fake_sel, stt_status, app_token
    ) -> None:
        """`request["user"]` is truthy for an app token too, so a cookie check alone
        would admit an app that named this path in its manifest — and this endpoint runs
        brew/pipx/pip against the operator's machine. An ABSENT claim is refused with a
        named one, so a route with no auth middleware fails closed.

        Refused before the busy check, so an app cannot even reserve the slot.
        """
        core_mod._stt_install_status = {"step": "idle", "detail": "", "error": ""}
        spawned: list = []
        monkeypatch.setattr(
        ...
        resp = await core_mod.api_stt_install(_req(app_token=app_token))
        assert resp.status == 403
        assert json.loads(resp.body)["code"] == "dashboard_user_required"
        assert spawned == [], "the installer was spawned for an app token"
        assert core_mod._stt_install_status["step"] == "idle", "the busy slot was reserved"

    @pytest.mark.asyncio
    async def test_an_unnamed_failure_still_ends_terminal_and_reaps_the_child(
        self, monkeypatch, fake_sel, stt_status
    ) -> None:
        """A failure the enumerated arms do not name must not latch the busy gate.

        The 409 check at the top reads the status global, so a non-terminal step left
        behind blocks every later install for the process lifetime. The enumerated arms
        cannot be exhaustive — a child line longer than the reader's limit with no newline
        raises ``ValueError`` out of ``readline`` — so this pins the catch-all: terminal
        status, child reaped, and a following install admitted.
        """
        core_mod._stt_install_status = {"step": "idle", "detail": "", "error": ""}
        proc = SimpleNamespace(
            returncode=None,
            stdout=SimpleNamespace(
                readline=AsyncMock(side_effect=ValueError("Separator is not found")),
            ),
        ...
        assert resp.status == 500
        assert json.loads(resp.body)["code"] == "stt_install_failed"
        assert core_mod._stt_install_status["step"] == "error"
        proc.kill.assert_called_once()
        # The busy gate must admit the next attempt rather than answer 409 forever.
        assert core_mod._stt_install_status["step"] not in ("starting", "installing_whisper")
...
class TestSttTranscribe:
    ...
    @pytest.mark.asyncio
    async def test_the_availability_probe_runs_off_the_event_loop(self, monkeypatch) -> None:
        """``is_available()`` shells out, so it must not run on the loop thread.

        It probes for the whisper CLI and ffmpeg with a synchronous ``subprocess.run``,
        which blocks every other request on the gateway for the duration. The hop is
        fail-silent — remove it and every assertion here still passes — so this observes
        the thread the probe actually ran on.
        """
        import threading
        ...
        monkeypatch.setattr("kiro_crew.transcribe.is_available", _probe)
        resp = await core_mod.api_stt_transcribe(_req())

        assert resp.status == 503
        assert ran_on == [False], "the availability probe blocked the event loop thread"

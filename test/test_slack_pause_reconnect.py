...
class TestAnUnlinkDuringReconnectIsNotOverruled:
    """A reconnect that loses the race reports a conflict instead of half-resuming.

    The rebind is in-memory only, so rebuilding it around coordinates the user has just
    unlinked produces a connection that exists nowhere but memory: gone on the next
    restart, and unusable by the reply path, which reads the stored row. Re-creating the
    row instead would overrule someone who unlinked on purpose.
    """

    @pytest.mark.asyncio
    async def test_an_unlink_inside_the_unpause_await_returns_a_conflict(
        self, tmp_path, monkeypatch
    ):
        from kiro_crew.dashboard import chat_slack
        ...
        monkeypatch.setattr("kiro_crew.dashboard.state.config_dir", lambda: tmp_path)
        monkeypatch.setattr("kiro_crew.dashboard.chat_slack.sel", lambda: MagicMock())
        monkeypatch.setattr(chat_slack, "slack_mirror_is_paused", lambda *a, **k: True)
        monkeypatch.setattr(
            chat_slack,
            "vet_and_audit",
        ...
        seeded: list[tuple] = []
        monkeypatch.setattr(
            chat_slack, "_spawn_slack_backfill", lambda *a: seeded.append(a)
        )
        ...
        # Linked when the handler looks; unlinked by the time the unpause returns.
        link = {"v": ("ts123", "C123")}
        state.sessions.get_slack_link = MagicMock(side_effect=lambda _key: link["v"])

        def _unpause_then_unlink(*_args, **_kwargs):
            link["v"] = (None, None)
            return True

        state.sessions.set_slack_paused = MagicMock(side_effect=_unpause_then_unlink)

        async with TestClient(TestServer(_pause_app(state))) as client:
            resp = await client.post("/api/chat/slots/s1/slack-link")
            assert resp.status == 409, await resp.text()
            assert (await resp.json())["code"] == "slack_link_changed"

        state.link_slack.assert_not_called()
        assert not seeded, (
            "the reconnect seeded transcript history into a thread the user had "
            "just unlinked"
        )

    @pytest.mark.asyncio
    async def test_an_untouched_link_still_reconnects(self, tmp_path, monkeypatch):
        """The guard must not close on the ordinary reconnect it sits in front of."""
        from kiro_crew.dashboard import chat_slack
        ...
        state.link_slack.assert_called_once()
        assert seeded, "an untouched reconnect did not re-seed the thread"


class TestReconnectDoesNotWriteTheSessionMapOnTheLoop:
    """Reconnect re-bound through `link_slack`, whose last step is a session-map write.

    That write is filesystem I/O on the event loop, so every other task — chat turns,
    the Slack socket, the watchdog — stalls for as long as the disk takes. It was also
    redundant: the coordinates come out of the stored row, and the unpause beside it is
    already offloaded. The in-memory half still runs, because it repairs the slot's link
    flags and the inbound routing map.

    Asserted by which THREAD the write happens on rather than by the call being absent:
    deleting it and offloading it both satisfy the invariant, doing it inline does not.
    """

    @pytest.mark.asyncio
    async def test_no_write_lands_on_the_event_loop_thread(self, tmp_path, monkeypatch):
        import threading
        ...
        monkeypatch.setattr(chat_slack, "_spawn_slack_backfill", lambda *a, **k: None)
        ...
        def _record_write(*_args, **_kwargs):
            wrote_on.append(
                "loop" if threading.current_thread() is loop_thread else "worker"
            )

        state.sessions.set_slack_link = MagicMock(side_effect=_record_write)

        async with TestClient(TestServer(_pause_app(state))) as client:
            resp = await client.post("/api/chat/slots/s1/slack-link")
            assert resp.status == 200, await resp.text()

        assert "loop" not in wrote_on, (
            "the reconnect wrote the session map on the event loop, stalling the "
            f"gateway for the length of that write (writes seen: {wrote_on})"
        )

    @pytest.mark.asyncio
    async def test_the_in_memory_rebind_still_happens(self, tmp_path, monkeypatch):
        """Removing the write must not remove the repair it was bundled with."""
        from kiro_crew.dashboard import chat_slack
        ...
        state.link_slack.assert_called_once()
        assert state.link_slack.call_args.kwargs.get("persist") is False, (
            "the reconnect asked for the persisting rebind again, which is the "
            "event-loop write this test exists to keep out"
        )
...
class TestSlackTurnEgressIsRecheckedPerSend:
    ...
    def test_every_pause_gated_turn_site_also_asks_the_policy(self):
        ...
        # belongs to someone else. Pinned as the comparison AND as the delegation that
        # feeds this turn's captured pair into it, because a `return False` in either
        # place passes every mute-based test.
        assert '!= (channel or "")' in src, (
            "the Slack leg stopped comparing its link against the channel it is "
            "posting to, so a rebind mid-turn keeps posting to the old thread"
        )
        assert "return _slack_leg_detached(_mirror_chan, _mirror_thread)" in src, (
            "the turn's leg check no longer compares against the pair THIS turn "
            "captured, so a rebind mid-turn is invisible to it"
        )
        # The approval prompt addresses its own pair, so it must be checked against
        # that pair rather than the turn's — an approval is actionable, so leaking one
        # into a rebound thread lets a stranger answer for this session.
        assert (
            "and not _slack_leg_detached(\n"
            "                        slot._slack_channel, slot._slack_thread_ts\n"
            "                    )\n"
        ) in src, (
            "the Slack approval prompt no longer re-checks the leg after its "
            "governance await, so a disconnect during that await still posts an "
            "actionable prompt into the detached thread"
        )
    ...
    def test_the_user_echo_gate_re_asks_the_mute_after_its_policy_await(self):
        """The gate that opens the tool stream asks the mute on BOTH sides of the await.

        A disconnect MUTES without removing the link, so the coordinates the gate reads
        afterwards still look valid and the pre-await answer is the only thing standing
        between a detached thread and this turn's output. Asserted as a count inside the
        condition rather than by presence: the predicate was already named there before
        the fix, and appearing exactly once is precisely the defect.
        """
        import inspect
        ...
        src = inspect.getsource(chat_runner)
        start = src.index("# ── Bidirectional sync: mirror user message to linked Slack")
        gate = src[start:start + 2000]
        asks = gate.count("slack_mirror_is_paused(state, session_key)")
        assert asks >= 2, (
            "the user-echo gate asks the mute once, before its awaited policy read, so "
            f"a disconnect landing inside that await still posts (found {asks} ask(s))"
        )
        offload = gate.index("asyncio.to_thread(")
        last_ask = gate.rindex("slack_mirror_is_paused(state, session_key)")
        assert last_ask > offload, (
            "every mute check sits BEFORE the await it is supposed to outlive"
        )

...
def test_a_wrong_shaped_state_file_does_not_500_every_refresh(
    tmp_path: Path, document: object
) -> None:
    """GPT BLOCKING (span dc2f58772b80): a malformed container must not raise.

    ``.get(key, [])`` does NOT cover a key that is PRESENT and null -- the default
    is only consulted for a missing key -- so ``{"chain_peers": null}`` iterated
    ``None`` and raised ``TypeError`` inside the constructor. The constructor runs
    from ``_get_state()``, so that exception surfaced as a 500 on EVERY
    ``/api/auth/refresh`` until the file was hand-repaired: a total dashboard
    lockout from one malformed byte-range.

    Parametrized across all three record keys and three non-object documents
    because the reported crash was never specific to ``chain_peers`` -- the two
    pre-existing keys carried the identical shape, and a top-level list raises
    ``AttributeError`` on ``.get`` before any of them is reached.
    """
    ...
    # Constructed, empty, and still usable -- the store rebuilds rather than
    # denying every rotation.
    assert mgr.chain_peer("anything") == ""
    assert mgr.is_consumed("anything") is False
    assert mgr.is_chain_revoked("anything") is False
    mgr.bind_chain_peer("fresh", LAPTOP_KEY, 9e12)
    assert mgr.chain_peer("fresh") == LAPTOP_KEY


@pytest.mark.asyncio
async def test_a_wrong_shaped_state_file_still_serves_a_rotation(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The same finding at the endpoint it actually broke.

    The unit test above proves the constructor survives; this proves the
    consequence GPT named is gone -- ``/api/auth/refresh`` answers instead of
    500ing, which is what "every refresh returns 500 until manual repair" meant.
    """
    path = tmp_path / "refresh_chains.json"

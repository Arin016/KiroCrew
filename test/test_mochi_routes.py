...
async def _live_runtime(tmp_path):
    ...
    """
    hooks.on_startup(_Ctx(tmp_path))
    try:
...
class TestPackFileServing:
    ...
    async def test_svg_pack_file_served_as_text_plain_nosniff(self, tmp_path, monkeypatch):
        from kiro_crew.apps.builtins.mochi import appearance_store
...
class TestReset:
    ...
    async def test_reset_stats_serializes_with_tick_and_clears_pending_flush(self, tmp_path) -> None:
        ...
        async with _live_runtime(tmp_path) as rt:
            ...
            # tick() and reset() share one lock so they cannot interleave.
            assert isinstance(rt.stats._lock, type(threading.Lock()))
    ...
    async def test_reset_reloads_pinned_while_holding_its_lock(self, tmp_path) -> None:
        ...
        # _wipe imports pins_mutation from the module at call time, so patching
        # the module attribute is picked up.
        import unittest.mock as _mock

        with _mock.patch.object(pfs, "pins_mutation", _spy_lock):
            async with _live_runtime(tmp_path) as rt:
                held = {"v": False}
                orig = rt.pinned.load
...
class TestResetSerializesUnlinks:
    ...
    async def test_reset_enters_each_store_lock(self, tmp_path, monkeypatch):
        ...
        async with _live_runtime(tmp_path):
            ...
            _spy(queue_file, "queue_mutation")
            _spy(watchlist_file, "watchlist_mutation")
            _spy(pinned_files_service, "pins_mutation")
            _spy(activity_log, "activity_mutation")

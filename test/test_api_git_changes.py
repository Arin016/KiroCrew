...
@requires_git
@posix_only
async def test_hardlinked_alternates_file_is_refused(tmp_path):
    """info/alternates HARD-LINKED to a protected file is refused, never read through.

    A hard link is a regular file with no target, so neither the S_ISREG check nor
    O_NOFOLLOW can see it -- only the link count on the opened descriptor can.
    """
    _init_repo(tmp_path, commit=True)
    (tmp_path / "changed.txt").write_text("x\n")
    secret = tmp_path / "secret-target"
    ...
    info = tmp_path / ".git" / "objects" / "info"
    info.mkdir(parents=True, exist_ok=True)
    os.link(secret, info / "alternates")

...
class TestBothExpandingCallersHandOverTheRawText:
    ...
    def test_the_capture_precedes_the_prompt_and_skill_stages(self):
        ...
        # Located by the assignment, not by a direct call: the skill stage runs through
        # `asyncio.to_thread`, so the resolver's name is an argument rather than being
        # followed by "(". Anchoring on the tuple unpack keeps this pinned to the STAGE
        # rather than to how it happens to be invoked.
        skills_at = source.index("message, skill_blocks, _skill_names = ")

...
type KiroClawAgentCfg = Omit<KiroClawAgent, 'name'>
interface WorkspaceCfg { dir: string }
interface MemoryStoreCfg { description: string; embedding_provider: string }
interface KiroClawCfg {
  agents: Record<string, KiroClawAgentCfg>
...
export default function KiroClawCfgTab() {
  const provider = useProvider()
  const queryClient = useQueryClient()
  const { data: cfg = null, error: queryErr } = useQuery<KiroClawCfg>({
  ...
  const patchMut = useMutation({
    ...
    onSuccess: (updated) => { queryClient.setQueryData(['kiroclawConfig'], updated) },
    onError: (e: Error) => {
      setSaveErr(e.message)
      setTimeout(() => setSaveErr(''), 4000)
      queryClient.invalidateQueries({ queryKey: ['kiroclawConfig'] })
      setRev(r => r + 1)
    },
  })
  ...
  return (
    <>
      {/* Agents */}
      <Card>
        <CardTitle><Bot className="lucide-inline" /> KiroClaw Agents <InfoTip text={`Named agent definitions that bind a ${provider.labels.agentTemplateField.toLowerCase()}, workspace, and memory store together. Edit config.json to add or modify agents.`} /></CardTitle>
        {agents.length === 0 ? (
      ...
      {/* Subagent Settings */}
      <SubagentSettings cfg={cfg} onSaved={() => queryClient.invalidateQueries({ queryKey: ['kiroclawConfig'] })} />

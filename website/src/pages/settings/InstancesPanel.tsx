      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <label className="flex flex-col gap-1 text-[13px] text-muted sm:col-span-2">
          ...
          <input
            ...
            placeholder="/home/you/.local/bin/kiroclaw  —  leave blank for standard installs"
          />
          <span className="text-[12px] text-muted leading-snug">
            Only needed if <code className="text-text">kiroclaw</code> is installed somewhere
            non-standard on the remote. Leave blank for a normal pip install on PATH. To find
            it, run on the remote host: <code className="text-text">command -v kiroclaw</code>{' '}
            (commonly <code className="text-text">~/.local/bin/kiroclaw</code>).
    ...
    return (
      <Card>
        ...
        <p className="text-[13px] text-muted mb-3">
          Enable it to let this gateway open SSH tunnels to your remote KiroClaws and switch
        ...
        {restartPending && (
          <div role="status" className="flex items-start gap-2 px-3 py-2 mb-3 text-[13px] rounded-md bg-warning/10 text-warning border border-warning/30">
            ...
            <span>
              Disabled in config. Restart the gateway (<code className="text-text">kiroclaw restart</code>){' '}
        ...
        <p className="mt-2 text-[12px] text-muted">
          Equivalent CLI: <code className="text-text">kiroclaw config set instances.enabled true</code> then{' '}
          <code className="text-text">kiroclaw restart</code>.
        </p>
      </Card>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {!active && (
        <div role="status" className="flex items-start gap-2 px-3 py-2 text-[13px] rounded-md bg-warning/10 text-warning border border-warning/30">
          <AlertTriangle size={14} className="lucide-inline mt-0.5 shrink-0" />
          <span>
            Enabled, but not active yet. Restart the gateway (<code className="text-text">kiroclaw restart</code>){' '}
          ...
          {instances.length > 0 ? (
            <Card>
              ...
              <p className="mt-2 text-[12px] text-muted">
                ...
                demand. Tune with{' '}
                <code className="text-text">kiroclaw config set instances.warm_set_cap N</code>.
              </p>
            </Card>
          ) : (
            <Card>
              <div className="text-[13px] text-muted">
                No instances configured yet. Add one below to manage a remote KiroClaw.
              </div>
            </Card>
          )}

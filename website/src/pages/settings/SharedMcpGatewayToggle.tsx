...
/**
 * Shared MCP backend pooling.
 *
 * Scope is process topology: whether each session spawns its own MCP backends or
 * they are shared across sessions. MCP Apps rendering depends on pooling (the
 * broker is what intercepts a tool result), but that is stated and handled on
 * Settings > MCP Apps — this card does not ask the user to reason about it.
 */
export function SharedMcpGatewayToggle() {
  ...
  //
  // Stays on this page on success: enabling the pool is the FIRST half of the
  // job (the user then picks which servers to pool, on this very page), so
  // reporting the verified state here and letting the user choose where to go
  // next is the honest shape.
  ...
  return (
    <SettingsSection title={i18nT('pages.settings.sharedMcpGatewayToggle.shared_mcp_gateway')}>
      <SettingsCard>
        <SettingsToggle
          label={i18nT('pages.settings.sharedMcpGatewayToggle.share_one_backend_per_server')}
          description={subStatus}
          checked={enabled}
          disabled={statusQ.isLoading || busy || (!supported && !enabled)}
          configKey="mcp_gateway.enabled"
          onChange={next => { if (!supported && next) return; setTarget(next); setPhase('confirm') }}
        />
      </SettingsCard>

      {/* Lives on THIS tab, not on MCP Apps. It teaches what a shared backend is
          and when to keep a server out of one — a decision the reader makes right
          here, row by row, in the list below. It used to sit on the MCP Apps tab
          answering "does rendering need this?", which was a question that page
          created by naming sharing at all; the honest fix was to stop raising it
          there. Native <details>: keyboard- and screen-reader-operable with no JS,
          and the content is prose, not an animation. */}
      <details className="mt-1 rounded-lg border border-border bg-card px-4 py-3">
        <summary className="cursor-pointer text-sm font-semibold text-text">
          {i18nT('pages.settings.sharedMcpGatewayToggle.explain_title')}
        </summary>
        <div className="mt-3 flex flex-col gap-2 text-sm text-muted leading-relaxed">
          <p className="m-0">{i18nT('pages.settings.sharedMcpGatewayToggle.explain_normal')}</p>
          <p className="m-0">{i18nT('pages.settings.sharedMcpGatewayToggle.explain_shared')}</p>
          <p className="m-0">{i18nT('pages.settings.sharedMcpGatewayToggle.explain_tradeoff')}</p>
        </div>
      </details>

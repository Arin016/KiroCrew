  ...
  return (
    <div className="py-2">
      ...
      {/* The rejected write for THIS row, beside the switch that snapped back —
          a banner at the top of a 137-rule list is off-screen from the toggle
...
function DeniedCommandsSection({ draft, onDraftChange, noteDraft, onNoteDraftChange }: { draft: string; onDraftChange: (next: string) => void; noteDraft: string; onNoteDraftChange: (next: string) => void }) {
  ...
  // control — a 137-rule list puts a top-of-card banner off-screen from the
  ...
  const DISABLE_ALL_ID = '__disable_all__'
  ...
  const setDisableAll = useMutation({
    ...
    onMutate: () => setRuleError(null),
    onSuccess: applySnapshot,
    onError: (err: unknown) => setRuleError({ id: DISABLE_ALL_ID, message: trustFailureMessage(err) }),
  })
  ...
  const ruleErrorRowVisible = (() => {
    if (!ruleError || ruleError.id === DISABLE_ALL_ID) return true
    ...
    <SettingsSection title={i18nT('pages.settings.securityPanel.denied_commands')}>
      ...
      <SettingsCard>
        ...
        <ErrorNotice
          variant="inline"
          className="mb-1"
          message={ruleError?.id === DISABLE_ALL_ID ? ruleError.message : undefined}
          askAgent={errorHandoff}
          testId={ruleError?.id === DISABLE_ALL_ID ? 'denied-disable-all-error' : undefined}
        />

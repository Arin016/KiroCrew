...
export default function KiroClawAgentsPage() {
  const provider = useProvider()
  const refreshTrigger = useAppSelector(s => s.dashboard.refreshTrigger)

  const { data: agentsData, refetch: refetchAgents } = useQuery({
    queryKey: ['kiroclaw-agents', refreshTrigger],
    queryFn: () => api.kiroclawAgents(),
  })
  const agents: KiroClawAgent[] = agentsData?.agents || []
  ...
  const memoryStoreOptions = kiroclawCfg?.memory_stores ? Object.keys(kiroclawCfg.memory_stores) : ['default']

  const [filter, setFilter] = useState('')
  const [error, setError] = useState('')
  const [name, setName] = useState('')
  const [kiroAgent, setKiroAgent] = useState('kiroclaw')
  ...
  const startEdit = (a: KiroClawAgent) => {
                  ...
                  <tr key={`${a.name}-${a.project_path || 'global'}`} className="hover:bg-bg-hover transition-colors">
                    ...
                    {editing === a.name ? (
                      <>
                        ...
                        <td className="px-2.5 py-2 border-b border-border text-sm"><SourceBadge source={a.source || 'kiroclaw'} /></td>
                        <td className="px-2.5 py-2 border-b border-border">
                          <StyledSelect

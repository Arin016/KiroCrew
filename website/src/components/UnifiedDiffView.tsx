...
export default function DiffView({ patch, path }: { patch: string; path: string }) {
  ...
  if (!ready) return <div className="px-3 py-3 text-[11px] text-muted">{i18nT('components.unifiedDiffView.loading_diff')}</div>
    ...
    <div className="min-w-max text-[11px] leading-5 font-mono">
      ...
      {rowsTruncated && (
        <div className="px-3 py-1.5 text-warn bg-bg-elevated/60 select-none">
          {i18nT('components.unifiedDiffView.diff_display_capped', { count: fmtNumber(MAX_RENDERED_ROWS) })}
        </div>
      )}
    </div>
  )

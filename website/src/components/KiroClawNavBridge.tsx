 ...
 *
 * Also reports slot titles to the plugin via `window.__kiroclawReportSlotTitle`
 ...
 */
export default function KiroClawNavBridge() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('kiroclaw-soft-navigate', handler)
    return () => window.removeEventListener('kiroclaw-soft-navigate', handler)
  }, [navigate])

  // Report slot titles to the plugin whenever slots change
  useEffect(() => {
    const fn = (window as unknown as { __kiroclawReportSlotTitle?: (slug: string, title: string) => void }).__kiroclawReportSlotTitle

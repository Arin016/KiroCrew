...
export default function LocalChangesView({ projectDir, onFileOpen }: {
  ...
  if (!projectDir) {
    return <LocalChangesEmpty>{i18nT('components.localChangesView.pick_project_directory')}</LocalChangesEmpty>
  }
  if (query.isLoading) {
    return <LocalChangesEmpty>{i18nT('components.localChangesView.checking_for_changes')}</LocalChangesEmpty>
  }
  ...
  if (query.isError && !query.data) {
    return <LocalChangesEmpty>{i18nT('components.localChangesView.could_not_scan', { dir: projectDir })}</LocalChangesEmpty>
  }
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-overlay">
        {incomplete && (
          <div className="px-4 py-1.5 text-[11px] text-warn border-b border-border">
            {i18nT('components.localChangesView.partial_scan')}
          </div>
        )}
        {filtersUnsafe && (
          <div className="px-4 py-1.5 text-[11px] text-warn border-b border-border">
            {i18nT('components.localChangesView.repo_skipped_for_safety', { reason: filtersUnsafe })}
          </div>
        )}
        {!repo && !filtersUnsafe && !incomplete && (
          <LocalChangesEmpty>{i18nT('components.localChangesView.no_git_repository')}</LocalChangesEmpty>
        )}
        {repo && repo.files.length === 0 && !incomplete && (
          <LocalChangesEmpty>
            {repo.branch
              ? i18nT('components.localChangesView.working_tree_clean_on_branch', { branch: repo.branch })
              : i18nT('components.localChangesView.working_tree_clean')}
          </LocalChangesEmpty>
        )}
              ...
              <span className="ml-auto text-[11px] text-muted shrink-0">
                {repo.truncated && (
                  <span
                    ...
                    title={i18nT('components.localChangesView.scan_hit_limit')}
                  >
                    partial
                  </span>
                )}
    ...
    <div>
        ...
        >
          ...
          {onFileOpen && file.status !== 'deleted' && file.kind !== 'dir' && file.kind !== 'symlink' && file.kind !== 'hardlink' && (
            <button
              ...
              title={i18nT('components.localChangesView.open_in_editor', { file: file.rel })}
              aria-label={i18nT('components.localChangesView.open_in_editor', { file: file.rel })}
            >
              <Pen size={12} />
            </button>
          )}
      ...
      {open && (
        ...
        <div className="overflow-x-auto border-t border-b border-border">
          {diffQuery.isLoading ? (
            <div className="px-3 py-3 text-[11px] text-muted">{i18nT('components.localChangesView.loading_diff')}</div>
            ...
            <div className="px-3 py-3 text-[12px] text-muted flex items-center gap-2">
              <span>{i18nT('components.localChangesView.couldnt_load_diff')}</span>
              <button
                type="button"
              ...
              >
                {i18nT('components.localChangesView.retry')}
              </button>
            </div>
            ...
            <div className="px-3 py-3 text-[12px] text-warn">
              {i18nT('components.localChangesView.diff_not_shown_for_safety', { reason: diffQuery.data.error || i18nT('components.localChangesView.repo_refused_safe_read') })}
            </div>
          ) : diffQuery.data?.diff ? (
            ...
            <>
              {diffQuery.data.diff_truncated && (
                <div className="px-3 py-1.5 text-[11px] text-warn border-b border-border">
                  {i18nT('components.localChangesView.diff_truncated')}
                </div>
              )}
            ...
            <div className="px-3 py-3 text-[12px] text-warn">
              {diffQuery.data.error || i18nT('components.localChangesView.diff_unavailable_file_unreadable')}
            </div>
          ) : (
            <div className="px-3 py-3 text-[12px] text-muted">{i18nT('components.localChangesView.no_diff_available')}</div>
          )}
        </div>
      )}
    </div>

...
export default memo(forwardRef<MarkdownPanelHandle, Props>(function MarkdownPanel({ filePath, content, onContentChange, onSave, onClose, liveWatch, onSubmitComments, onRefresh, reserveWidth, initialDiffMode, onDiffModeChange, embedded, savedBaseline, revealLine, onRevealConsumed, onOpenFolder }: Props, ref) {
  ...
  const diffUnavailableTitle = diffData?.error
    ? i18nT('components.markdownPanel.diff_unavailable_with_error', { error: diffData.error })
    : diffNoBaseline
      ? i18nT('components.markdownPanel.diff_unavailable_no_git_repo')
      : diffError
        ? i18nT('components.markdownPanel.diff_unavailable_baseline_failed')
        : i18nT('components.markdownPanel.diff_unavailable_original_too_large')

...
export default function WorktreePicker({ open, onOpenChange, repo, activeSlot, anchorRef, anchorRect, onSelect }: Props) {
  ...
  const create = () => {
    ...
    api.createWorktree(repo, b)
      .then(res => {
        if (!res?.path) {
          setError(res?.error || i18nT('components.worktreePicker.worktree_creation_failed'))
          return
        }
        // `create` is idempotent: asking for a branch that already has a worktree
        // returns the EXISTING one (`reused`), which may be another session's live
        // project. Switching into it unconditionally walked straight past the guard
        // `selectRow` applies to the very same path, so the one route that refuses
        // to share a working tree could be bypassed by typing its branch name.
        // Re-check against the rows we already hold rather than trusting the verb.
        // Deliberately NO reload here: `load` clears `error` on entry, so
        // refreshing would wipe the refusal we just set and leave the click
        // looking like it silently did nothing.
        const owner = rows.find(r => r.path === res.path)?.active_session
        if (owner && owner !== activeSlot) {
          setError(i18nT('components.worktreePicker.in_use_by_another_session'))
          return
        }
        onSelect(res.path)
        onOpenChange(false)

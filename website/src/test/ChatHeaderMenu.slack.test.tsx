...
describe('Session menu — two channels toggled at once', () => {
   ...
   */
  it('patches the binding, not the read-only origin row for the same channel', () => {
    // A session born IN a Slack channel keeps an `origin` row for it, and can ALSO
    // hold a mirror binding to that same channel. Only the second row is the one a
    // toggle owns. Matching on `channel` alone found the origin row first, so a
    // connect or a mute patched something no control owns and the real binding never
    // moved — the row the user flipped snapped straight back. Asserted on the store
    // operation, like the sibling test below: the invariant is which row is rewritten.
    const origin = {
      channel: 'slack', label: '#eng', target: 'C1',
      direction: 'origin' as const, live: true,
    }
    const binding = {
      channel: 'slack', label: '#eng', target: 'C1',
      direction: 'out' as const, live: true, paused: true,
    }
    const store = createTestStore({
      dashboard: { ...dashboardState, slots: [{ key: 'chat-1-100', links: [origin] }] },
    })
    const linksOf = () =>
      store.getState().dashboard.slots.find((sl: ChatSlot) => sl.key === 'chat-1-100')
        ?.links ?? []

    // Connect: only an origin row exists, so the binding is APPENDED rather than
    // overwriting the row that records where the conversation began.
    store.dispatch(patchSlotLink({ key: 'chat-1-100', channel: 'slack', link: binding }))
    expect(linksOf()).toHaveLength(2)
    expect(linksOf().filter(row => row.direction === 'origin')).toHaveLength(1)

    // Unmute: the patch lands on the binding, and the origin row never gains the flag.
    store.dispatch(patchSlotLink({
      key: 'chat-1-100', channel: 'slack', link: null, patch: { paused: false },
    }))
    expect(linksOf().find(row => row.direction === 'out')?.paused).toBe(false)
    expect(linksOf().find(row => row.direction === 'origin')).not.toHaveProperty('paused')

    // Disconnect removes the binding and KEEPS the origin row.
    store.dispatch(patchSlotLink({ key: 'chat-1-100', channel: 'slack', link: null }))
    expect(linksOf()).toHaveLength(1)
    expect(linksOf()[0].direction).toBe('origin')
  })

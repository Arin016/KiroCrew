import { describe, it, expect, vi, beforeEach } from 'vitest'
import { render, screen, fireEvent, waitFor } from '@testing-library/react'
import WorktreePicker, { WorktreeRow } from '../components/WorktreePicker'

// Mock the API client — WorktreePicker only touches these three methods.
vi.mock('../api/client', () => ({
  api: {
    worktreeList: vi.fn(),
    createWorktree: vi.fn(),
    worktreeRemove: vi.fn(),
  },
}))
import { api } from '../api/client'

const rect = (): DOMRect =>
  ({ top: 400, bottom: 424, left: 100, right: 540, width: 440, height: 24, x: 100, y: 400, toJSON() {} }) as DOMRect

const wt = (over: Partial<WorktreeRow> = {}): WorktreeRow => ({
  path: '/repo',
  branch: 'main',
  is_main: true,
  detached: false,
  dirty: false,
  active_session: null,
  ...over,
})

function setup(props: Partial<React.ComponentProps<typeof WorktreePicker>> = {}) {
  const onSelect = vi.fn()
  const onOpenChange = vi.fn()
  const utils = render(
    <WorktreePicker
      open
      onOpenChange={onOpenChange}
      anchorRect={rect()}
      repo="/repo"
      activeSlot="chat-1"
      onSelect={onSelect}
      {...props}
    />,
  )
  return { onSelect, onOpenChange, ...utils }
}

describe('WorktreePicker', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'trunk', is_main: true, active_session: 'chat-1' }),
        wt({ path: '/repo-wt-feature', branch: 'feat/upload', is_main: false }),
      ],
    })
  })

  it('lists worktrees for the repo and marks the main + current session', async () => {
    setup()
    await waitFor(() => expect(api.worktreeList).toHaveBeenCalledWith('/repo'))
    expect(await screen.findByText('trunk')).toBeInTheDocument()
    expect(screen.getByText('feat/upload')).toBeInTheDocument()
    // The main worktree carries the "main" badge…
    expect(screen.getByText('main')).toBeInTheDocument()
    // …and the row bound to the active slot reads as "this session".
    expect(screen.getByText(/this session/i)).toBeInTheDocument()
  })

  it('renders nothing when closed', () => {
    const { container } = setup({ open: false })
    expect(container.firstChild).toBeNull()
  })

  it('renders nothing without an anchor rect', () => {
    const { container } = setup({ anchorRect: null })
    expect(container.firstChild).toBeNull()
  })

  it('selecting a worktree hands its path back and closes', async () => {
    const { onSelect, onOpenChange } = setup()
    const row = await screen.findByText('feat/upload')
    fireEvent.click(row)
    expect(onSelect).toHaveBeenCalledWith('/repo-wt-feature')
    expect(onOpenChange).toHaveBeenCalledWith(false)
  })

  it('does not let you select a worktree active in another session', async () => {
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'main', is_main: true, active_session: 'chat-1' }),
        wt({ path: '/repo-wt-busy', branch: 'feat/busy', is_main: false, active_session: 'chat-9' }),
      ],
    })
    const { onSelect } = setup()
    const row = await screen.findByText('feat/busy')
    fireEvent.click(row)
    expect(onSelect).not.toHaveBeenCalled()
    expect(screen.getByText(/^in use$/i)).toBeInTheDocument()
  })

  it('creates a new worktree and switches to it', async () => {
    ;(api.createWorktree as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true, path: '/repo-wt-new', branch: 'feat/new' })
    const { onSelect } = setup()
    await screen.findByText('feat/upload')
    fireEvent.change(screen.getByLabelText(/new branch name/i), { target: { value: 'feat/new' } })
    fireEvent.click(screen.getByRole('button', { name: /^create worktree$/i }))
    await waitFor(() => expect(api.createWorktree).toHaveBeenCalledWith('/repo', 'feat/new'))
    expect(onSelect).toHaveBeenCalledWith('/repo-wt-new')
  })

  it('a create that RETURNS an existing busy worktree is refused, not entered', async () => {
    // `create` is idempotent: an existing branch yields the worktree it already
    // has (`reused`), which may be another session's live project. Clicking that
    // row is refused, so typing its branch name must be refused identically --
    // otherwise the create field is a way around the one guard that protects it.
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'main', is_main: true, active_session: 'chat-1' }),
        wt({ path: '/repo-wt-busy', branch: 'feat/busy', is_main: false, active_session: 'chat-9' }),
      ],
    })
    ;(api.createWorktree as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true, path: '/repo-wt-busy', branch: 'feat/busy', reused: true,
    })
    const { onSelect, onOpenChange } = setup()
    await screen.findByText('feat/busy')
    fireEvent.change(screen.getByLabelText(/new branch name/i), { target: { value: 'feat/busy' } })
    fireEvent.click(screen.getByRole('button', { name: /^create worktree$/i }))
    await waitFor(() => expect(api.createWorktree).toHaveBeenCalled())
    await waitFor(() => expect(screen.getByText(/in use by another session/i)).toBeInTheDocument())
    expect(onSelect).not.toHaveBeenCalled()
    expect(onOpenChange).not.toHaveBeenCalledWith(false)
  })

  it('removes a clean worktree and refreshes the list', async () => {
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true, path: '/repo-wt-feature' })
    setup()
    const removeBtn = await screen.findByRole('button', { name: /remove worktree/i })
    fireEvent.click(removeBtn)
    await waitFor(() => expect(api.worktreeRemove).toHaveBeenCalledWith('/repo', '/repo-wt-feature', false))
    // A successful removal re-lists.
    await waitFor(() => expect(api.worktreeList).toHaveBeenCalledTimes(2))
  })

  it('asks for confirmation before force-removing a dirty worktree', async () => {
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockResolvedValue({ dirty: true, error: 'This worktree has uncommitted changes.' })
    setup()
    const removeBtn = await screen.findByRole('button', { name: /remove worktree/i })
    fireEvent.click(removeBtn)
    // First call is the non-force attempt that the server refused.
    await waitFor(() => expect(api.worktreeRemove).toHaveBeenCalledWith('/repo', '/repo-wt-feature', false))
    const force = await screen.findByRole('button', { name: /remove anyway/i })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true, path: '/repo-wt-feature' })
    fireEvent.click(force)
    await waitFor(() => expect(api.worktreeRemove).toHaveBeenCalledWith('/repo', '/repo-wt-feature', true))
  })

  it('a 409 is recognised by its code, not by English prose in the message', async () => {
    // The endpoint answers with `code: 'worktree_dirty'`; the confirm row must
    // key off that rather than matching /uncommitted/ in the human message,
    // which would also match the OPPOSITE ("could not check") case below.
    const err = Object.assign(new Error('localised text with no English keyword'), {
      status: 409,
      body: JSON.stringify({ error: 'localised', code: 'worktree_dirty' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByRole('button', { name: /remove anyway/i })).toBeInTheDocument()
    expect(await screen.findByText(/will be lost/i)).toBeInTheDocument()
  })

  it('an unverifiable dirty state does not claim there ARE uncommitted changes', async () => {
    // `worktree_dirty_unknown` means git could not be asked. Reporting it as
    // "uncommitted changes" would talk the user into a destructive click on a
    // fact the backend explicitly reported as unknowable.
    const err = Object.assign(new Error('Could not check for uncommitted changes; not removing.'), {
      status: 409,
      body: JSON.stringify({ error: 'could not check', code: 'worktree_dirty_unknown' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByRole('button', { name: /remove anyway/i })).toBeInTheDocument()
    expect(await screen.findByText(/couldn't verify/i)).toBeInTheDocument()
  })

  it('a non-dirty failure surfaces as an error, not as a force prompt', async () => {
    const err = Object.assign(new Error('Access denied'), {
      status: 403,
      body: JSON.stringify({ error: 'Access denied', code: 'worktree_path_sensitive' }),
    })
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockRejectedValue(err)
    setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByText(/access denied/i)).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /remove anyway/i })).not.toBeInTheDocument()
  })

  it("names THIS session in the tooltip when the session owning the tree is our own", async () => {
    // `active_session` covers both "another session" and "this one"; the row can
    // show the `this session` badge while the tooltip tells the user to switch
    // some other session's project — an instruction with no referent.
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'trunk', is_main: true }),
        wt({ path: '/repo-wt-feature', branch: 'feat/x', is_main: false, active_session: 'chat-1' }),
      ],
    })
    setup()
    const btn = await screen.findByRole('button', { name: /remove worktree/i })
    expect(btn.getAttribute('title')).toMatch(/switch this session/i)
    expect(btn.getAttribute('title')).not.toMatch(/another session/i)
  })

  it('still names the OTHER session when the tree belongs to one', async () => {
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({
      worktrees: [
        wt({ path: '/repo', branch: 'trunk', is_main: true }),
        wt({ path: '/repo-wt-feature', branch: 'feat/x', is_main: false, active_session: 'chat-9' }),
      ],
    })
    setup()
    const btn = await screen.findByRole('button', { name: /remove worktree/i })
    expect(btn.getAttribute('title')).toMatch(/another session/i)
  })

  it('Escape backs out of a pending force-confirm before it closes the picker', async () => {
    // The confirm row carries only the destructive action, so Escape is what
    // replaces a Cancel button — and it must not skip straight to closing.
    ;(api.worktreeRemove as ReturnType<typeof vi.fn>).mockResolvedValue({ dirty: true, error: 'uncommitted' })
    const { onOpenChange } = setup()
    fireEvent.click(await screen.findByRole('button', { name: /remove worktree/i }))
    expect(await screen.findByRole('button', { name: /remove anyway/i })).toBeInTheDocument()

    fireEvent.keyDown(document, { key: 'Escape' })

    // First Escape clears the confirm and leaves the picker open.
    await waitFor(() =>
      expect(screen.queryByRole('button', { name: /remove anyway/i })).not.toBeInTheDocument())
    expect(onOpenChange).not.toHaveBeenCalled()

    // A second Escape, with nothing pending, closes it.
    fireEvent.keyDown(document, { key: 'Escape' })
    await waitFor(() => expect(onOpenChange).toHaveBeenCalledWith(false))
  })

  it('shows an empty state when there are no worktrees', async () => {
    ;(api.worktreeList as ReturnType<typeof vi.fn>).mockResolvedValue({ worktrees: [] })
    setup()
    expect(await screen.findByText(/no worktrees found/i)).toBeInTheDocument()
  })
})

// Settings ▸ Chat ▸ Model — optimistic picker display (#6848).
//
...
//
// Every test here pins the pre-settle display with the PATCH promise held
// open, which the pre-#6848 code cannot produce: its triggers read the
// react-query cache alone, so they cannot change before the invalidated
// query has refetched.
//
...
describe('ChatPanel — pending ownership and reconciliation edges', () => {
  ...
  it('does NOT clear a failure banner that came from a non-picker save', async () => {
    ...
    // A model pick must leave that unresolved failure visible.
    const trigger = await pick('Default Model', 'claude-opus-4.8')
    await waitFor(() => expect(trigger).toHaveTextContent('claude-opus-4.8'))

...
describe('expectedAbsent', () => {
  /**
   * The carve-out that keeps `verify` in step with `catalogParity.test.ts`. Without
   * it, zh-CN reported 45 phantom missing keys — every `_one` form of a registered
   * plural, which Chinese cannot select and therefore must not carry.
   */
  it('excuses a plural form the locale has no category for', () => {
    expect([...expectedAbsent(['items_one', 'items_other'], ['items'], ['other'])]).toEqual(['items_one'])
  })

  it('demands every form the locale does have a category for', () => {
    expect([...expectedAbsent(['items_one', 'items_other'], ['items'], ['one', 'other'])]).toEqual([])
  })

  it('does not excuse a plural-shaped key that is not in the registry', () => {
    expect([...expectedAbsent(['settings_one'], [], ['other'])]).toEqual([])
  })

  it('does not excuse a non-plural key that merely ends in a category word', () => {
    expect([...expectedAbsent(['settings.other'], ['settings'], ['other'])]).toEqual([])
  })
})

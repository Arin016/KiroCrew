...
import {
  attachmentsFrom, composeMessage, filesFrom, ingestFiles,
...
export const ChatPanel: React.FC<ChatPanelProps> = ({ onToggleWatch, watchPanelVisible, onTogglePinned, pinnedPanelVisible, pinnedFileCount }) => {
  ...
  const [shortcuts, setShortcuts] = useState({ toggleWindow: '⌘⇧M', screenCapture: '⌘⇧X', hideAll: '⌘⇧H' })
  ...
  useEffect(() => {
    ...
    api?.getConfig?.().then((c: any) => {
      ...
      if (c?.shortcuts) {
        const fmt = (s: string) => s.replace(/CommandOrControl/g, '⌘').replace(/Option/g, '⌥').replace(/Shift/g, '⇧').replace(/\+/g, '')
        setShortcuts({
    ...
    const offConfig = api?.onConfigUpdated?.((m: any) => {
      ...
      api?.getConfig?.().then((c: any) => {
        if (c?.shortcuts) {
          const f = (s: string) => s.replace(/CommandOrControl/g, '⌘').replace(/Option/g, '⌥').replace(/Shift/g, '⇧').replace(/\+/g, '')
          setShortcuts({
  ...
  useEffect(() => {
    ...
    const offCapture = api?.onCaptureDone?.((b64: string) => setScreenshot(b64))
    ...
    const offApprovalResolved = api?.onApprovalResolvedExternal?.(() => {
      // Approval was handled externally (e.g. KiroCrew dashboard) — clear all pending approvals
      setPendingApprovals([])
      setMessages(prev => prev.map(m =>
        m.content.startsWith('__approval__')
          ? { ...m, content: i18nT('apps.mochi.approval.approved') }
          : m
      ))
    })
  ...
  const handleApproval = useCallback((id: string, action: string) => {
    api?.respondApproval?.(id, action)
    setPendingApprovals(prev => prev.filter(a => a.id !== id))
    setMessages(prev => prev.map(m => m.id === `approval-${id}`
      ? { ...m, content: action === 'reject' ? i18nT('apps.mochi.approval.rejected') : action === 'trust' ? i18nT('apps.mochi.approval.trusted') : i18nT('apps.mochi.approval.approved') }
      : m))
  }, [])
...
const LocalImage: React.FC<{ path: string; onClickImage?: (src: string) => void }> = ({ path, onClickImage }) => {
  ...
  return <img src={src} onClick={() => onClickImage?.(src)}
...
const mdComponents: Record<string, React.FC<any>> = {
  ...
  img: (p: any) => {
    const src = p.src
    if (src?.startsWith('/') && src.includes('.kiro/crew/screenshots/')) {
      return <LocalImage path={src} />
    }
...
const Bubble = React.memo<{ message: ChatMessage; onOption?: (text: string) => void; onImageClick?: (b64: string) => void; onApproval?: (id: string, action: string) => void; onEdit?: (content: string) => void; animate?: boolean }>(({ message, onOption, onImageClick, onApproval, onEdit, animate = true }) => {
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {([
              ...
              <button key={action} onClick={() => onApproval?.(req.id, action)} style={{
            ...
            {isUser
              ? <span style={{ whiteSpace: 'pre-wrap' }}>
                  {text.split(/\n/).map((line, i) => {
                    ...
                    if (mdImgMatch) {
                      return <LocalImage key={i} path={mdImgMatch[1]} />
                    }
                    ...
                    if (trimmed.startsWith('/') && /\.(?:png|jpg|jpeg|gif|webp)$/i.test(trimmed)) {
                      return <LocalImage key={i} path={trimmed} />
                    }

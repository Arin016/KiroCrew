      ...
      <SpriteImporter
        ...
        onDone={async (result: any) => {
          ...
          if (res?.ok) {
            ...
            setError(res?.error || i18nT('apps.mochi.errors.create_sprite_pack'))
          }
          setPrefill(undefined)
          setMode('gallery')
        }}
        onCancel={() => { setPrefill(undefined); setMode('gallery') }}
      />
    )
  }
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <button style={S.headerBtnAccent} onClick={() => setMode('petdex')}>
          <WithIcon icon={Plus}>{i18nT('apps.mochi.petdex.button')}</WithIcon>
        </button>

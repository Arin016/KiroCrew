...
export const WatchlistPanel: React.FC<WatchlistPanelProps> = ({ visible, onClose, petName }) => {
  ...
  // Hard delete. The old button set status='cancelled' but was labelled
  // "Cancel", which read as "dismiss this dialog" and left the item on the
  // list -- so the label and the effect were both wrong. "Delete" now removes.
  const handleDelete = useCallback((id: string) => {
    setItems(prev => prev.filter(item => item.id !== id))
    api?.deleteWatchItem?.(id)
  }, [])
  ...
  }, [visible, selectedItemId, onClose])
  ...
  const renderListMode = () => {
        ...
        >
          ...
          {!isTerminal && (
            ...
            >
              ...
              <button className="wl-btn-cancel" onClick={() => handleDelete(item.id)} style={{
                background: 'rgba(255, 107, 107, 0.08)', border: 'none', borderRadius: 4,
                color: 'var(--danger)', fontSize: 11, fontWeight: 500,
                padding: '3px 10px', cursor: 'pointer', transition: 'background 150ms ease',
              }}>{i18nT('apps.mochi.watchPanel.delete')}</button>
            </div>
          )}
    ...
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 0', minHeight: 0 }}>
        {loading && (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 11, padding: 24, opacity: 0.6 }}>...</div>
        )}
          ...
          {!TERMINAL_STATUSES.includes(selectedItem.status) && (
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
              ...
              <button className="wl-btn-cancel" onClick={() => {
                backToList()
                setTimeout(() => handleDelete(selectedItem.id), 50)
              }} style={{
                background: 'rgba(255, 107, 107, 0.08)', border: 'none', borderRadius: 4,
                color: 'var(--danger)', fontSize: 11, fontWeight: 500,
                padding: '3px 10px', cursor: 'pointer', transition: 'background 150ms ease',

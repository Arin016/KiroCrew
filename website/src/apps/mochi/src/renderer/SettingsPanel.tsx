...
const ELECTRON_MAP: Record<string, string> = {
  '⌘': 'CommandOrControl',
...
const MODIFIER_GLYPHS = ['⌘', 'Win', 'Ctrl', 'Alt', 'Shift']

/**
 * Canonical modifier order for the emitted accelerator.
 *
 * The recorder collects keys in PRESS order, so pressing Shift before Cmd
 * produced 'Shift+CommandOrControl+A'. Sorting makes the stored string a
 * function of the chord rather than of typing order, which matters because the
 * shell compares the configured accelerator against what it registered to
 * detect a rebind -- two spellings of one chord read as drift forever.
 */
    ...
    return (
      <div style={{ padding: '6px 0 2px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
          ...
          {toolData?.fromCache && <span style={{ fontSize: 9, color: 'var(--text-muted)' }}>{i18nT('apps.mochi.settingsPanel.mcp_from_cache')}</span>}
        </div>
        {/* Tool lists */}

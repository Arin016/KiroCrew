...
export async function clearCompletedWatchItems(): Promise<void> {
  await fetch('/api/apps/mochi/watchlist/clear-completed', {
    method: 'POST',
    credentials: 'same-origin',
    headers: { 'Content-Type': 'application/json' },
    body: '{}',
...
/**
 * Show an image full-size. Deliberately a NO-OP for now, and not a phantom:
 * chat images already open via `localFileUrl` in a new tab through
 * `openExternal`, and the original's separate lightbox WINDOW is a fidelity
 * item, not a functional gap. Kept so the ported call site has a real target.
 */
export function openLightbox(_src: string): void {
  /* fidelity follow-up: the original had a dedicated lightbox window */
}
    ...
    const res =
      route.kind === 'approval'
        ...
        : await fetch(`/api/chat/slots/${MOCHI_SLOT}/approve`, {
            ...
            credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: route.action, request_id: id }),
          })

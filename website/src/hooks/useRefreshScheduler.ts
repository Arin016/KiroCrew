...
export function useRefreshScheduler(opts: UseRefreshSchedulerOptions = {}): void {
  ...
  const refreshMutation = useMutation({
    mutationFn: async (): Promise<unknown> => {
      ...
      if (!resp.ok) {
        ...
        const err: RefreshError = new Error(`refresh ${resp.status}`)
  ...
  const meQuery = useQuery({
    ...
    queryFn: async (): Promise<AuthMeResponse | null> => {
      ...
      if (resp.status === 401) {
        // PROACTIVE on-mount/return: access cookie expired but the
        // refresh cookie may still be valid. Silently mint new cookies
        // BEFORE any auth-required UI flashes (banner-flash fix).
        try {

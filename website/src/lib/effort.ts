 ...
 * `kiro_claw/effort.py` so the UI and server agree on levels and per-model
 ...
 * (kiro_claw/effort.py): effort is Opus/Sonnet-only; Haiku/auto/empty and
 * third-party models cannot use it. Gates the dropdown so a non-capable model
 * never shows a control that would silently no-op on the backend.
 */

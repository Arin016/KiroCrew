...
const CATALOG = 'website/src/i18n/locales/en.json'
...
const head = flatten(JSON.parse(fs.readFileSync(path.join(REPO, CATALOG), 'utf-8')))

let base
try {
  const raw = execFileSync('git', ['show', `${BASE_REF}:${CATALOG}`], {
    cwd: REPO,
    encoding: 'utf-8',
    maxBuffer: 32 * 1024 * 1024,
  })
  base = flatten(JSON.parse(raw))
} catch {
  ...
  // new, which would fail every branch for the wrong reason.
  console.log(`OK: skipped — cannot read ${CATALOG} at ${BASE_REF} (shallow clone?).`)
  process.exit(0)
}

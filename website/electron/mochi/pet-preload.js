...
exposePetApi({
  ...
  /**
   * The global screen-capture accelerator fired. Capture runs in this renderer
   * on KiroCrew's existing getDisplayMedia path, so the shell only forwards the
   * request — there is no capture API exposed here.
   */
  onStartSnip: (cb) => {
    const handler = () => cb();

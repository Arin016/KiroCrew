...
async function resolveMochiTarget(settings) {
  ...
  const choice = settings && typeof settings.petInstance === "string" ? settings.petInstance : "";
 ...
 *
 * Capture itself runs in the PANEL renderer, on KiroCrew's existing
 ...
 * Recording permission and surfaces guidance when it is denied). The shell only
 * surfaces the panel and forwards the request, so there is no second capture
 * mechanism and no second permission path.
 */
function mochiStartSnip() {
  try {
    // showPanelView already handles the first-open timing (it waits for
    // did-finish-load before sending), so a cold panel works too.
    require("./panelWindow").showPanelView(mochiPetBaseUrl, "mochi-panel:start-snip", mochiPetToken);
  } catch (err) {

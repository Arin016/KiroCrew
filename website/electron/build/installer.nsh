...
; install and lets a dual-scope update exit its otherwise ambiguous chooser;
; Abort suppresses both the warning and the close once replacement has begun.
Function KiroAbortPre
  ${If} $KiroVisibleUpdate == 1
  ${AndIf} $KiroUpdateCommitted == 1
    Abort
  ${EndIf}
FunctionEnd
...
!macro customInstallMode
  !ifndef BUILD_UNINSTALLER
    ; Preserve an unambiguous registered install scope without asking the user
    ; to choose it again. If both per-user and per-machine copies are registered,
    ; electron-updater's --updated handoff does not identify which copy launched
    ; it, so leave the native scope chooser visible rather than update the wrong
    ; installation. multiUserUi consumes these flags in its page-pre callback,
    ; performs elevation when needed, and Abort-skips the page.
    ${If} $KiroVisibleUpdate == 1
      ${If} $KiroHasPerMachineInstallation == 1
      ${AndIf} $KiroHasPerUserInstallation == 0
        StrCpy $isForceMachineInstall 1
      ${ElseIf} $KiroHasPerUserInstallation == 1
      ${AndIf} $KiroHasPerMachineInstallation == 0

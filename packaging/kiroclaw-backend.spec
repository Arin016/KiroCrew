...
"""PyInstaller spec for the standalone KiroClaw backend binary.

Produces ``dist/kiroclaw-backend/kiroclaw-backend`` — a self-contained
executable that runs the full ``kiroclaw`` CLI (gateway, setup, chat, …) with
...
``kiroclaw-backend gateway`` (see website/electron/find-bin.js + main.js).

Build (from repo root, after the frontend dist is staged into
src/kiro_claw/static/dist):

    pyinstaller packaging/kiroclaw-backend.spec --noconfirm
...
PKG = os.path.join(SRC, "kiro_claw")
    ...
    if os.path.isdir(abs_src):
        for root, _dirs, files in os.walk(abs_src):
            for f in files:
                ...
                # Destination dir mirrors kiro_claw/<...> under the bundle.
                dst_dir = os.path.join(
                    "kiro_claw", os.path.relpath(os.path.dirname(full), PKG)
                )
                datas.append((full, dst_dir))
    elif os.path.isfile(abs_src):
        datas.append((abs_src, os.path.join("kiro_claw", os.path.dirname(rel_dst))))
...
# KIROCLAW_PROJECT_DIR). Bundle them so the standalone app has its agent config
...
hiddenimports += collect_submodules("kiro_claw.apps.builtins")
# The whole package, to be safe against lazy submodule imports.
hiddenimports += collect_submodules("kiro_claw")
...
# BEFORE importing kiro_claw.cli (which pulls aiohttp and caches its SSL

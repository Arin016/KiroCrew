#!/usr/bin/env python3
"""MeshClaw → KiroClaw migration — command-line entry point.

Usage:
    python transfer/meshclaw_to_kiroclaw/migrate.py [--dry-run] [--yes]

Requires KiroClaw to be installed (the engine imports ``kiro_claw`` so the
target path resolution matches the app, including ``KIROCLAW_HOME``).
"""
...
def _print_summary(res: dict) -> None:
    ...
    if res.get("backup_path"):
        print(f"  • backup of ~/.kiroclaw: {res['backup_path']}")
    for w in res.get("warnings") or []:
        print(f"  ⚠ {w}")
    if not res.get("dry_run"):
        print("\nRestart KiroClaw to load the imported data.")


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Migrate MeshClaw data into KiroClaw.")
    ...
    print("Your current ~/.kiroclaw is backed up first; ~/.meshclaw is never modified.")

"""MeshClaw → KiroClaw data migration engine.
...
current KiroClaw home (``~/.kiroclaw``, honoring ``KIROCLAW_HOME``).
...
* ``~/.kiroclaw`` is backed up before any write so the operation is reversible.
...
# Mesh-only legacy config keys that no longer exist in the KiroClaw schema.
...
def kiroclaw_dir() -> Path:
    """Current KiroClaw home (honors ``KIROCLAW_HOME``)."""
    return config_dir()
...
def _rewrite_paths(text: str, src: Path, dst: Path) -> str:
    ...
    return text.replace(str(src), str(dst)).replace("/.meshclaw/", "/.kiroclaw/")


def _backup_target(dst: Path) -> Path | None:
    """Snapshot the current KiroClaw home next to it.  Returns backup path."""
...
def _merge_config(src: Path, dst: Path, *, dry_run: bool) -> dict[str, Any]:
    """Merge user-data keys from MeshClaw config into the KiroClaw config.

    KiroClaw's install-specific values win; only keys that are missing/empty in
...
def migrate(*, dry_run: bool = False) -> dict[str, Any]:
    """Run the MeshClaw → KiroClaw migration.

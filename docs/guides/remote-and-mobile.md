...
- **Python**: 3.12 or newer (`setup.cfg` sets `python_requires = >=3.12`).
  No mainstream distro ships it as `python3` yet, so the `curl … | sh`
  installer provisions a python-build-standalone 3.12 for you rather than
  turning the host away; see the bootstrap note below.
...
CentOS 7, Amazon Linux, and Ubuntu 22.04, where no base-repo package supplies
Python 3.12+, it

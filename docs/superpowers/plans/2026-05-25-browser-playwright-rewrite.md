...
**Architecture:** KiroClaw provides only what Playwright MCP cannot: Amazon auth (Midway cookies, federate SSO, SPNEGO/Kerberos). The agent uses Playwright MCP tools (`browser_navigate`, `browser_click`, `browser_snapshot`, etc.) for all interaction. KiroClaw's role is (1) install Playwright + browsers during setup, (2) prepare auth before browsing starts, (3) inject cookies into the Playwright browser context, and (4) provide a skill/prompt that guides the agent through auth validation.
...
3. Skill instructs: run `kiroclaw browse auth health` → validate → then use Playwright MCP tools
4. Agent calls `browser_navigate`, `browser_snapshot`, `browser_click`, etc. via Playwright MCP
5. If auth fails: agent runs `kiroclaw browse auth inject` to push cookies into Playwright context
...
**Files:**
- Delete: `src/kiro_claw/browser/cdp.py`
- Delete: `src/kiro_claw/browser/session.py`
- Delete: `src/kiro_claw/browser/snapshot.py`
- Delete: `src/kiro_claw/mcp_browser.py`
...
```bash
git rm src/kiro_claw/browser/cdp.py
git rm src/kiro_claw/browser/session.py
git rm src/kiro_claw/browser/snapshot.py
git rm src/kiro_claw/mcp_browser.py
...
`auth.py` imports `from kiro_claw.browser.cdp import CDPClient, CDPError`. The `inject_cookies` and `inject_mcs` functions use CDPClient directly. These functions are no longer needed since Playwright manages its own cookies. Remove:
- `inject_cookies()`
- `inject_from_file()`
- `inject_mcs()`
- `get_midway_token()`
...
In `src/kiro_claw/dashboard/handlers/messaging.py`:
- Remove `api_browser_auth_retry` function (uses `_get_session` from deleted mcp_browser)
- Or rewrite it to call `kiroclaw browse auth inject` via subprocess

In `src/kiro_claw/cli_setup.py`:
- Remove `from kiro_claw.browser.chrome import find_chrome, profile_dir_for_session, sync_profile`
...
python -c "from kiro_claw.browser.auth import health, federate_auth, parse_netscape_cookies"
```
...
**Files:**
- Modify: `src/kiro_claw/browser/chrome.py`
...
Keep only:
- `_KIROCLAW_DIR` / `_PROFILES_DIR` constants
...
```bash
git rm src/kiro_claw/browser/chrome.py
...
**Files:**
- Create: `src/kiro_claw/browser/setup.py`
- Create: `test/test_browser_setup.py`

- [ ] **Step 1: Write failing tests**

```python
"""Tests for kiro_claw.browser.setup — Playwright install and MCP config."""
...
class TestEnsurePlaywrightInstalled:
    def test_installs_when_not_present(self, monkeypatch):
        ...
        monkeypatch.setattr("kiro_claw.browser.setup.is_playwright_installed", lambda: False)
        ensure_playwright_installed()
    ...
    def test_skips_when_already_present(self, monkeypatch):
        ...
        def mock_run(cmd, **kw):
            ...
            return MagicMock(returncode=0, stdout="")
        monkeypatch.setattr("subprocess.run", mock_run)
        monkeypatch.setattr("kiro_claw.browser.setup.is_playwright_installed", lambda: True)
        ensure_playwright_installed()
...
```python
"""Playwright browser setup — install and configure Playwright MCP for KiroClaw."""
...
```bash
git add src/kiro_claw/browser/setup.py test/test_browser_setup.py
...
**Files:**
- Rewrite: `src/kiro_claw/browser/cli.py`
...
Usage:
    ...
    kiroclaw browse auth health        # Check Midway/Kerberos auth status
    kiroclaw browse auth inject        # Inject cookies into running Playwright browser
    kiroclaw browse auth federate <url># Complete federate SSO flow for a URL
"""
...
from kiro_claw.browser.auth import (
    _krb5_env,
    federate_auth,
    has_kerberos_ticket,
...
def run_browse(args: list[str]) -> None:
    """Entry point for `kiroclaw browse <subcommand>`."""
    if not args:
        _print_help()
        return
    ...
    if cmd == "setup":
        ...
        print(f"Unknown command: {cmd}. Run 'kiroclaw browse' for help.", file=sys.stderr)
        sys.exit(1)


def _print_help() -> None:
    print("""kiroclaw browse — auth management for Playwright MCP browsing
...
In `src/kiro_claw/cli.py`, update the browse help text:
```
kiroclaw browse setup                        # Install Playwright + browsers
kiroclaw browse auth health                  # Check auth status
kiroclaw browse auth inject                  # Get cookies for injection
kiroclaw browse auth federate <url>          # Complete federate SSO
```
...
```bash
git add src/kiro_claw/browser/cli.py src/kiro_claw/cli.py
...
**Files:**
- Create: `src/kiro_claw/skills/browser-auth.md`
...
- `browser_snapshot` — get page structure (replaces `kiroclaw browse snap`)
...
- `kiroclaw browse setup` (one-time Playwright install)
```

- [ ] **Step 2: Commit**

```bash
git add src/kiro_claw/skills/browser-auth.md
...
**Files:**
- Modify: `src/kiro_claw/config/prompt.md`

- [ ] **Step 1: Rewrite the browser section**

Replace the entire `kiroclaw browse` section (lines ~123-182) with:
...
1. Validate auth: `kiroclaw browse auth health`
2. If unhealthy, tell user what to run (`kinit -f`, `mwinit -o`)
3. Get cookies: `kiroclaw browse auth inject` → inject via `browser_evaluate`
...
- **federate redirect** → `kiroclaw browse auth federate "<url>"` → navigate to `final_url`
...
```bash
git add src/kiro_claw/config/prompt.md
...
**Files:**
- Modify: `src/kiro_claw/cli_setup.py`
...
```python
    # ── Browser (Playwright) ──
    print("\n── Browser (Playwright) ──")
    from kiro_claw.browser.setup import is_playwright_installed, ensure_playwright_installed
...
```bash
git add src/kiro_claw/cli_setup.py
git commit -m "feat(setup): install Playwright MCP during kiroclaw setup"
```
...
**Files:**
- Modify: `src/kiro_claw/config/defaults.json`
...
This ensures Playwright MCP is available to all KiroClaw agents by default.

- [ ] **Step 2: Commit**

```bash
git add src/kiro_claw/config/defaults.json
...
**Files:**
- Modify: `src/kiro_claw/browser/auth.py`
...
- The import: `from kiro_claw.browser.cdp import CDPClient, CDPError`
...
```bash
git add src/kiro_claw/browser/auth.py test/test_browser_auth.py
...
**Files:**
- Modify: `src/kiro_claw/dashboard/handlers/messaging.py`
...
- Or rewrite to subprocess `kiroclaw browse auth inject`
...
```bash
grep -rn "from kiro_claw.mcp_browser\|from kiro_claw.browser.cdp\|from kiro_claw.browser.session\|from kiro_claw.browser.snapshot\|from kiro_claw.browser.chrome" src/ --include="*.py" | grep -v __pycache__
```
...
```bash
git add src/kiro_claw/dashboard/handlers/messaging.py
...
**Files:**
- Modify: `src/kiro_claw/browser/__init__.py`

- [ ] **Step 1: Update module docstring**

```python
"""KiroClaw browser auth — Midway/Kerberos/Federate SSO for Playwright MCP."""
```
...
```bash
python -m flake8 src/kiro_claw/browser/ --max-line-length=120
```

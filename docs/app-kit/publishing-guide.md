...
# Enable it
curl -X POST http://localhost:7777/api/apps/my-app/enable
```
...
curl http://localhost:7777/api/apps | python3 -m json.tool

# Check app manifest is valid
curl http://localhost:7777/api/apps/my-app/manifest | python3 -m json.tool
...
curl -X POST http://localhost:7777/api/apps/my-app/update
...
```bash
curl -X POST http://localhost:7777/api/apps/registry/install \
  -H 'Content-Type: application/json' \
  -d '{"name": "my-app"}'
```

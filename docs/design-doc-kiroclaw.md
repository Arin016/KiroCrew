...
To understand KiroClaw, you need to understand the three layers beneath it:
...
2. **Agent configs** (JSON files in `~/.kiro/agents/`) tell kiro-cli *how* to behave: which system prompt to use, which tools to enable, which MCP servers to load, which commands to deny. Every agent — `kiroclaw`, `reviewer`, `Customer360GenAIContext` — runs as `kiro-cli acp --agent <name>`. The `--agent` flag selects the config; the runtime is always kiro-cli.
...
### What KiroClaw Adds Over kiro-cli Alone

| Capability | kiro-cli alone | With KiroClaw |
|------------|---------------|---------------|
...
### The Agent Hierarchy: kiroclaw as Coordinator

`kiroclaw` is itself just an agent config — same level as any other. What makes it the coordinator is that the gateway defaults to it and it has MCP tools (`spawn_run`, `cron_add`, `task_run`) that trigger the gateway to spawn sessions with other agents.

```
KiroClaw Gateway
  ├── Slack DM / CLI chat              → always kiroclaw agent
  ├── Dashboard tab                    → user picks any agent from dropdown
  ├── Cron job                         → per-job agent_id (kiroclaw if unset)
  ├── Subagent (spawn_run)             → kiroclaw (inherits parent)
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                        KiroClaw Gateway (single asyncio process)        │
│                                                                         │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐                              │
...
```
initialize → session/new → set_mode (kiroclaw only) → set_model (kiroclaw only) → drain MCP notifications (10s) → ready
```
...
|---------|-------------------|
| CLI chat | `kiroclaw-workspace/cli_chat` |
| Slack thread | `kiroclaw-workspace/{thread_ts}` |
| Background | `kiroclaw-workspace/_bg` |
| Cron job | `kiroclaw-workspace/cron_{job_id}` |
| TaskRunner | `kiroclaw-workspace/taskrunner_main` |

Platform defaults: `/Volumes/workplace/kiroclaw-workspace` (macOS), `~/workplace/kiroclaw-workspace` (Linux).

### 2.4 Cross-Session Communication

Sessions are isolated processes, but KiroClaw bridges them:
...
kiroclaw's system prompt is thin (orchestration rules only). Step-specific expertise comes from dynamic context injection:
...
The pattern:
1. Run ad-hoc with kiroclaw (1-2 times) — observe which steps, tools, and skills activate
...
| Subagent | Always kiroclaw | No `agent` parameter |
...
| Project-level | `$KIROCLAW_PROJECT_DIR/skills/` |
| User-created (dashboard or manual) | `~/.kiroclaw/skills/` |
...
**Two skill systems**: KiroClaw's `SkillsLoader` (injects via prompt text for kiroclaw agent) and kiro-cli native (`skill://` URIs in agent config `resources` for custom agents). AIM packages bridge both.
...
**Discover** (`discover_new_servers()`): Compares `~/.kiro/settings/mcp.json` against the kiroclaw agent config. Returns servers not yet registered. Config-level only — no processes spawned.
...
**Installing**: AIM dashboard/CLI, `kiro-cli agent create`, or manual JSON file drop into `~/.kiro/agents/`. KiroClaw discovers agents by scanning that directory on every API request.
...
**Updating**: Dashboard AIM panel (per-agent or bulk), re-install via AIM CLI, or edit JSON directly. kiroclaw agent has a dashboard JSON editor with auto-restart.

### 5.3 Skill Authoring

**For kiroclaw**: Create in `~/.kiroclaw/skills/` (dashboard or manual). SKILL.md with optional YAML frontmatter (`name`, `description`, `triggers`, `always`). Can include scripts — LLM sees the `dir` path and uses `execute_bash` to run them.
...
|------------|-------------------|--------------------------|
| `kiroclaw-core` | `spawn_run`, `learn_add`, `learn_remove`, `task_run` | `spawn_list`, `learn_list` |
| `kiroclaw-cron` | `cron_add`, `cron_remove`, `cron_pause`, `cron_resume` | `cron_list`, `cron_remove_all` |
...
**Storage:** `~/.kiroclaw/security_events.jsonl` — append-only JSONL with HMAC-SHA256 integrity chain.
...
**Integrity chain:** Each entry's `entry_hash` is an HMAC-SHA256 over all fields (excluding `entry_hash` itself) plus the previous entry's hash. The HMAC key is a 32-byte random secret stored at `~/.kiroclaw/sel_hmac.key` (mode 0600). `verify_integrity()` walks the full chain and reports any tampered or missing entries.
...
| CLI | `kiroclaw security events [-n 20]`, `kiroclaw security verify` |
...
Currently, inter-step communication happens through two channels: the kiro-cli session context (LLM remembers prior turns) and WorkingMemory (text summaries injected into prompts). Both are internal to KiroClaw — they exist in the LLM's context window or in Python objects.
...
This also connects to distributed coordination (7.3): if agents on different machines share a git remote or a database, they can coordinate without any direct communication between kiroclaw instances. The external store becomes the message bus.

### 7.3 Distributed Coordination: Multiple Machines Running KiroClaw

Today, KiroClaw is a single-process, single-machine system. All kiro-cli processes, memory files, and the dashboard run on one host. This limits:
...
- **Collaboration**: One user's kiroclaw can't delegate to another user's kiroclaw. There's no way to say "run this step on the Cloud Desktop that has the build environment."

**Levels of distribution (from simple to ambitious):**

**Level 1 — Remote agent execution.** The simplest extension: kiroclaw on machine A dispatches a step to a kiro-cli process on machine B via SSH or an API. Machine B runs the agent and returns the result. Machine A remains the coordinator. This is essentially `ssh machine-b kiro-cli chat --no-interactive --agent coder "task"` — the same pattern as the skill-as-agent-wrapper, but remote.

**Level 2 — Shared memory across machines.** Multiple kiroclaw instances share the same memory store (preferences, projects, lessons, history). This could be a shared filesystem (NFS, EFS), a database, or a sync protocol. Each instance reads/writes to the shared store. Sessions on machine A and machine B both see the same accumulated knowledge.

**Level 3 — Federated kiroclaw.** Multiple kiroclaw gateways coordinate as peers. A task decomposed on machine A can dispatch steps to machine B's SessionManager. Each machine has its own warm pool, dashboard, and cron service, but they share a task queue and memory store. This is a distributed TaskRunner — the planner runs on one node, workers run on any node with capacity.
...
- Authentication between kiroclaw instances (mutual TLS or shared tokens)
...
- Custom context providers would let teams integrate their own knowledge sources without modifying KiroClaw's core code — just register a provider that returns text.

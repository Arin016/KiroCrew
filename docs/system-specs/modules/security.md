...
- **Shell path (paired since #4956):** both leaves are in `_WRITE_SHAPED_BASH_LEAVES`, so `is_sensitive_bash_command` refuses home-anchored WRITE-shaped commands — redirects, write verbs (`tee`/`mv`/`sed -i`/`rm`-class), `cp`/`scp`/`rsync` with the config as *destination*, script-opens, and the Windows-native spellings — on both crew-home prefixes. The tier is write-SHAPED rather than verb-independent (unlike `_WRITE_PROTECTED_BASH_LEAVES`) because `hooks.on_tool_call` runs this gate over file-READ tool titles (which can be the bare path), and the tool_input scan and cron script vetting reuse it — a verb-independent entry would deny the read tool itself. So `cat`, a bare-path title, `git show`, and copy-as-source all keep matching nothing. Deliberately ANCHORED, never bare-token (the `_BARE_TOKEN_PROTECTED_LEAVES` SCOPE note names `config.json` as the counter-example). Accepted residuals: a novel verb-less write form and the `cd`-into-home + relative-filename form — the load-time clamp neutralizes an inflated resource value that lands through either.
- Empty/unknown ACP tool kinds are intentionally left to the load-time clamp backstop rather than hard-denied, to avoid over-blocking config reads that arrive without a kind (governance's shape inference can apply both read+write scopes because it is a permissive policy intersection; this gate is a hard deny).

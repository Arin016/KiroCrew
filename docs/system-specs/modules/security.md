...
| YOLO mode abuse | Unbounded auto-approve window | Time-limited safety override: Slack 30min, dashboard 6h, config 24h (no permanent mode). Re-auth required after expiry, with ONE exception — while an unattended AutoNudge loop is in flight the grant is extended by short leases (bounded by `min(24h, 4 x` the chosen TTL`)` from activation, extend-only/never-revive, opt-out via `agent.override_lease_for_armed_loops`), because expiry with no human present stalls the run instead of degrading it. SEL audit on every lifecycle event |

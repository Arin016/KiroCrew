...
- **Symlink-staging block**: creating a symlink to a credential file is the staging step of a symlink-swap attack chain and slips past the read-verb matcher, so `is_sensitive_bash_command()` also blocks creation verbs (`ln`/`cp`, `_SYMLINK_CREATE_VERB_RE`) when any token names a sensitive dir via a home-anchored or relative-traversal form (`_SENSITIVE_STAGING_RE`), returning "command stages a symlink to a sensitive credential path"
...
- Sensitive paths: `~/.aws`, `~/.ssh`, `~/.gnupg`, `~/.gpg`, `~/.config/gcloud`, `~/.azure`, `~/.docker/config.json`, `~/.kube/config`, `~/.npmrc`, `~/.pypirc`, `~/.netrc`, `~/.git-credentials`, `~/.kiroclaw/.env`, `~/.kiroclaw/sel_hmac.key`, `~/.kiroclaw/security_events.jsonl`

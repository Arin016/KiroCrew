...
- **Shared terminator boundary** (`path_end`): what may END a fenced path token. Accepting only `/`, whitespace, end-of-string or a quote makes flush punctuation a bypass for **every** entry in the tier — `cd ~/.aws;`, `cd ~/.ssh;` and `cd ~/.kiro/crew/profiles;` are allowed while the same commands written with `&&` are blocked, purely because `&&` is preceded by a space. The class is therefore every character a shell treats as the end of a word (`; & | ( ) < > , :` and a backtick). This matters most for the generic `/home/<user>` / `/Users/<user>` anchors, which have no chdir/expansion tracker to rescue them, so on macOS — where the agent's own home *is* `/Users/<user>` — a comma or semicolon terminator would otherwise reach the keystones directly. Widening a deny boundary can only deny more; the rule it enforces is unchanged. `win_path_end` is the same class plus a literal `$`, since a Windows path has no `$` expansion to protect
...
- **Bash gate, anchored**: the same entry in `_WRITE_PROTECTED_BASH_LEAVES`, matched verb-independently. There is no anchor-independent filename tier: `models` itself must **never** join one — the SCOPE note there forbids generic names, and an unanchored `models` would refuse a large fraction of ordinary commands. The archive form (`cd <models>; tar -xf evil.tar`), where the member name is inside the tarball and so unavailable to a name match, is caught by the `cd` target through the shared `path_end` terminator class instead.

`test_security.py::TestModelDirectoryIsWriteProtected` pins both layers, and the false-positive cost of the directory being home-anchored.

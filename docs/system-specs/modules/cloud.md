...
| `iam.py` | Least-privilege launcher policy generator (applied by the user, never by KiroCrew) + `policy_grants()` / `IAM_POLICY_CLI` for the dashboard IAM card + read-only reachability check + the **content-fixed instance permissions-boundary document** (`boundary_policy_document`/`boundary_arn`) and its constants (`BOUNDARY_NAME`). `kirocrew cloud iam-policy` stdout stays the policy JSON only. |

...
through — leaving a batch half-removed. Both sites that hand a manifest name to a syscall
therefore catch `(OSError, ValueError)`: one unusable entry costs its own file and nothing
more.
...
**The snapshot approves a directory, not a name.** `staged_targets()` returns each id
together with the `(st_dev, st_ino)` of the directory it named while holding the mutation
lock, and the delete re-checks that pair with `fstat` on the descriptor it opened —
`identity_changed` if it differs. Without it, the lock being released for the async
handoff meant a directory moved into an approved name would be deleted on consent given
for a different one, destroying sessions the user was never shown. The check is on the
DESCRIPTOR, not a second stat of the path, because the fd is the object every removal
addresses: a swap after this point no longer reaches the data. A batch that cannot be
stat'd at snapshot time is dropped from the set rather than carried unchecked.

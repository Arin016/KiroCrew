...
Five properties are load-bearing on both endpoints, and each fails toward the
permissive answer if dropped:
...
- **A row that arrives during the boundary is carried, not replaced away.**
  ...
  by either the edit or an arrived row stays retired. **Carrying it in memory is
  not enough**: the rewrite persisted the prospective window only and dropped
  that row's own durable copy, so a crash before the next flush tick would still
  lose a result the user was shown — the merged window therefore gets its own
  guarded durability pass before the request returns, and only when something
  actually arrived, so the normal path adds no await. That pass runs *after* the
  dispatch is armed, so a cancellation on it cannot strand a committed edit whose
  turn never runs, and it is best-effort: the boundary has already committed, the
  write is additive, and a failure re-arms `_dirty` for the periodic flush.
  **`rewind` still replaces wholesale**, so the same injected-row loss is open
  there and on the other rewrite-save callers (`regenerate`, `fork`); closing it
  belongs with the shared boundary contract rather than one endpoint.

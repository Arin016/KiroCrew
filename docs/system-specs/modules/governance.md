...
**The DIRECTORY has to be root-owned too, and absence is what proves it.** The check
above judges a document that is *present*; nothing in it says anything about the
directory holding one. On a host whose managed directory is writable by the account,
a standard user does not need to edit the root-owned file at all — they unlink it. The
open then raises `FileNotFoundError`, which is the tier's own "no managed document"
signal, so the tier goes inert and the local tier governs: the exact override this tier
exists to remove, reached by deleting the document rather than editing it, and
invisible because deletion and "never provisioned" are the same syscall result.

So the `FileNotFoundError` arm asks whether this account could have *caused* the
absence, via `platform_compat.path_writable_by_current_user` — the same predicate the
distribution sources use, which walks every existing ancestor (as written **and** as
resolved) and answers from mode bits **and** `faccessat`, so a POSIX ACL entry or a
re-pointable symlink parent counts as a way in. Writable ⇒ raise; not writable ⇒ the
document is genuinely absent and the tier is inert, which keeps the compatibility
contract for every standalone install. The guard is on the **absence** path only: a
document that is actually there is judged by the file-trust check, and refusing a
present document for directory writability would break any fleet whose provisioning
writes as a non-root user.

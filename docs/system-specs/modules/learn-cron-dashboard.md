...
Detects user corrections (e.g. "use X instead of Y", "remember that X", "never use X") and stores them in `~/.kiroclaw/lessons.jsonl`. Categories: `tool`, `preference`, `knowledge`. Injected into LLM context as `[Learned corrections:]` block (max 50). Detection runs after each ACP response.
...
3. Otherwise the slot name (portion after the `:` prefix, or the whole key) must satisfy at least one of:
   ...
   - Key starts with `slack:` (future-reserved namespace), **OR**
   - The corresponding JSONL file exists under `~/.kiroclaw/sessions/{slot_name}.jsonl` or `~/.kiroclaw/sessions/dashboard_{slot_name}.jsonl` — resolved by `_session_has_persisted_history()` in `handlers/_shared.py`.

   If none match → HTTP 400 `unknown session`.

The JSONL-existence check exists because MCP subprocesses retain their original `KIROCLAW_SESSION_KEY` env var for the life of the process, but the gateway's idle-sweep loop evicts in-memory slots after ~30 minutes of inactivity (see `session.py`). Without this fallback, long-lived Slack threads and reopened dashboard tabs would deterministically fail `learn_add` once the slot is swept, even though the user is actively engaged. Ephemeral (incognito/temporary) sessions never write to disk, so JSONL presence is a reliable positive signal of a non-ephemeral established session.
...
| `sk.startswith("slack:")` | allowed | `slack_namespace` |
| JSONL exists under `~/.kiroclaw/sessions/` | allowed | `jsonl_fallback_recovery` |
...
- Job execution: resets LLM session, streams response, posts to Slack/dashboard (unless `silent`)
- Session scoping: each job tagged with `session_key` from `KIROCLAW_SESSION_KEY` env var; `cron_remove_all` only removes jobs matching the calling session's key (CLI/admin with no key still removes all)
- Silent mode: `CronJob.silent = True` suppresses auto-delivery of results; the agent decides when to notify the user via the `send_message` MCP tool
...
- CLI: `kiroclaw cron {list|add|update|remove|pause|resume|trigger}`. `add` and `update` accept `--every`, `--cron`, `--channel`, `--agent`, `--approval-mode`. CLI flags mirror the corresponding `cron_add`/`cron_update` MCP tool parameters.
...
- **Script mode**: `script` field specifies a Python callable path (`~/.kiroclaw/crons/file.py:function`). The function receives a `ScriptContext` with `ctx.call_tool()` (MCP tool access), `ctx.notify()` (deliver message), `ctx.message` (arguments from the `message` field). Control flow via exceptions: `raise Skip()` to silently retry next tick, `raise Done()` to complete and remove the job, `raise Report("msg")` to deliver a message and keep running.
...
- **Safety**: scripts must live under `~/.kiroclaw/crons/`. `is_sensitive_path()` blocks credential file access. SEL audit on every invocation. Auto-pause after 5 consecutive failures. Concurrent execution guard prevents double-fire.
- **Kind tag**: `cron_list` labels each job as `script`, `command`, or `agent` based on which mode is configured.

### Trigger Command

On-demand job execution via CLI (`kiroclaw cron trigger <job_id>`) and MCP tool (`cron_trigger`). Delegates to `POST /api/crons/{id}/run`. The endpoint returns job name and uses `create_task` for non-blocking execution. Both CLI and MCP paths include SEL audit logging.
...
- **WebSocket origin validation**: `ws.py:_check_ws_origin()` validates the `Origin` header on every WebSocket upgrade request before accepting the connection. Rejects missing Origin (non-browser clients) and cross-origin requests. Only allows `http://127.0.0.1:{port}`, `http://localhost:{port}`, and `http://kiroclaw.localhost:7777`. Prevents cross-origin WebSocket hijacking where a malicious page could connect to `ws://127.0.0.1:7777/api/ws` and passively exfiltrate conversation data.
...
Each chat tab gets its own kiro-cli session keyed by `dashboard:{slot_key}` with a corresponding JSONL file (`~/.kiroclaw/history/dashboard:{key}.jsonl`). Sessions move between active and history:
...
7. **Idle expiry**: per-tab sessions expire after `session.timeout_secs` (default 30 min) like any other session; on next message a fresh session is created with history re-injected
...
**Agent Config**: PUT saves to `~/.kiro/agents/kiroclaw.json` and auto-restarts all kiro-cli sessions so changes take effect immediately.
...
**Cron**: GET `/api/crons` (includes `last_run_ts`, `has_result`, `has_slot`, per-job `timezone`, top-level `server_tz`, `skip_dates`), POST `/api/crons` (create, optional `agent` field), DELETE `/api/crons/{id}`, PATCH `/api/crons/{id}` (partial update: name, message, cron, every, agent (or agent_id), channel, approval_mode, silent, strict_schedule, timezone), POST `/api/crons/{id}/enable`, POST `/api/crons/{id}/run` (immediate execution, returns `{name}`, non-blocking via `create_task`), POST `/api/crons/{id}/to-chat` (creates linked chat slot `cron-{id}` with `linked_session_key="cron:{id}"`, hydrates from session history, reuses existing slot)
**Messaging**: POST `/api/send-message` (send to Slack DM + dashboard notification; body: `{text, title?, blocks?}`; when `blocks` provided, sends Block Kit message via `post_blocks()` with `text` as fallback; used by `send_message` MCP tool in `kiroclaw-core`)
...
**MCP Servers**: GET `/api/mcp` (list with enabled/disabledTools state from `~/.kiro/settings/mcp.json`), GET `/api/mcp/active` (per-agent MCP servers — reads from agent config for non-kiroclaw agents, global mcp.json for kiroclaw), GET `/api/mcp/probe` (cached probe results, non-blocking, 10-min TTL, preserves enabled state), POST `/api/mcp/probe` (live probe all, merges enabled/disabledTools from global config), POST `/api/mcp/sync` (discover + add to both kiroclaw.json AND global mcp.json + session reset), POST `/api/mcp/toggle` (enable/disable server in global mcp.json + sync tools/allowedTools to kiroclaw.json), POST `/api/mcp/toggle-tool` (enable/disable specific tool via disabledTools in global mcp.json), POST `/api/mcp/toggle-all` (bulk enable/disable all servers in global mcp.json + sync to kiroclaw.json)
**Agent Config**: GET/PUT `/api/agent/config` (read/write `~/.kiro/agents/kiroclaw.json`, PUT auto-restarts sessions)
**Chat**: POST `/api/chat` (SSE stream, or JSON with `?ws=1` — chunks via WebSocket), `/api/chat/slots` (CRUD, POST accepts optional `agent` field to set agent at creation), resume from history, POST `/api/chat/slots/{slot}/generate-title`, POST `/api/chat/slots/{slot}/agent` (switch agent for slot), POST `/api/chat/slots/{slot}/fork` (fork session — copies visible messages into new slot, body: `{at_message_index?, prompt?}`, returns `{ok, key, title, messages, prompt}`, new slot has `forked_from` metadata), POST `/api/chat/slots/{slot}/edit-resend` (edit a user message and re-run; in-place truncation of `slot.messages`, body: `{index?, ts?, content}`), POST `/api/chat/slots/{slot}/rewind` (edit any past user message and re-run; fork-and-swap — truncates `slot.messages`, removes the slot's ACP session via `SessionManager.remove`, deletes orphaned kiro-cli session JSONL at `~/.kiro/sessions/cli/<id>.json[l]`, then runs the edited prompt against a fresh ACP session under the same slot key/title/folder. Mirrors kiro-cli `/rewind`. Body: `{at_message_index?, ts?, content}`)
**Agents**: GET `/api/agents/installed` (list all kiro-cli agents from `~/.kiro/agents/`, with `package` field extracted from filename), GET/DELETE `/api/agents/detail/{name}` (full agent config JSON; DELETE removes the config file, protected for kiroclaw/kiroclaw-lite)
...
**Claude Code Migration**: GET `/api/cc/mirror/preview` (dry-run kiro→CC mirror via `mirror_kiro_to_cc`, returns per-entry actions plus `summary` counts of agents/mcp/skills/mirrored/skipped/errors), POST `/api/cc/mirror/run` (apply mirror, body `{force: bool}`, pushes `refresh("agents")` + `refresh("skills")`, audit-logged as `cc_mirror.run` with `ok|partial` outcome), GET `/api/cc/aim/missing` (list kiro AIM packages not yet installed for Claude Code via `installed_kiro_packages_missing_from_cc`), POST `/api/cc/aim/sync` (install AIM packages as CC plugins via `install_cc_plugin(standalone=True)`, body `{packages: [str]|null}` — null syncs all missing, returns `{installed, failed[]}`, validates package names against `_VALID_PACKAGE_RE`, audit-logs each install). Surfaced in `ProviderPanel.tsx` "Migrate from kiro to Claude Code" section so users do not need the CLI; `kiroclaw mirror kiro-to-cc` and `kiroclaw aim sync-cc` remain as equivalent CLI affordances.
**Sessions**: GET `/api/sessions` (paginated list), GET `/api/sessions/{key}` (detail), DELETE `/api/sessions/{key}` (permanent delete), GET `/api/sessions/context` (context usage), GET `/api/sessions/usage` (kiro credit usage via background `kiro-cli chat --no-interactive --agent kiroclaw-lite /usage`, cached 10 min)
...
**Screenshot**: POST `/api/screenshot` (macOS only — `screencapture -i`, returns path to `~/.kiroclaw/screenshots/`)
...
- `ProjectAnimation.tsx` — image-based animation: KiroClaw logo (`icon-192.png`) with orbiting rings and pulsing glow. Theme-aware via `var(--accent-glow)`. Used in ProjectsPage empty state.
...
- **Overview** (`/overview`) — `StatCard` components (with skeleton loading) + tabbed management console:
  ...
  - **MCP Servers tab**: Controls `~/.kiro/settings/mcp.json` (global config that kiro-cli ACP loads at runtime). Server-level enable/disable sets `disabled: true/false` in global config and syncs `@server` to kiroclaw.json `tools`/`allowedTools`. Per-tool enable/disable sets `disabledTools` array in global config. Probe All discovers tools per server, preserves enabled/disabledTools state across probes. Enable All / Disable All bulk buttons. Tool chips: green = enabled (clickable to disable), strikethrough = disabled (clickable to enable). Apply & Restart at top bar resets all active sessions. Live server badges (🔌 color-coded by status).
  - **Slack tab**: STT (Speech-to-Text) settings card — toggle enabled/disabled, model selector (turbo ~1.6 GB), status badge (ready/not installed), install button (`brew install openai-whisper`). Endpoints: `GET/PUT /api/config/stt`, `POST /api/stt/install`.
  - **Agent Config tab**: JSON editor with Save + warning about `kiroclaw setup --agent-only`
...
- **Logs** (`/logs`) — live gateway log stream via WebSocket (subscribe/unsubscribe via `WsContext`), server-side log level control (DEBUG/INFO/WARNING/ERROR buttons). Falls back to SSE for legacy `dashboard.html`.
...
- `AimBadge` source pills (aim=purple, kiroclaw=amber, builtin=gray) using `--aim` design token
...
- Calls `GET /api/mcp/active?agent=<name>` — for non-kiroclaw agents, reads from agent config in `~/.kiro/agents/`; for kiroclaw, reads from global `~/.kiro/settings/mcp.json`
- Per-agent MCP scoping works: `kiro-cli acp --agent <name>` loads only that agent's `mcpServers`
- Footer note explains scoping: custom agents load only their own MCPs; kiroclaw loads all global MCPs
...
For the default kiroclaw agent, kiro-cli ACP loads MCP servers from the global config. For non-kiroclaw agents (e.g. AIM-installed agents), kiro-cli loads only the `mcpServers` defined in the agent's own config file in `~/.kiro/agents/`. The dashboard MCP tab controls the global config used by kiroclaw:
- `disabled: true` on a server prevents kiro-cli from loading it (kiroclaw only)
- `disabledTools: [...]` on a server prevents specific tools from being registered (kiroclaw only)
- `kiroclaw-cron` and `kiroclaw-core` are synced to global mcp.json at gateway startup (`_sync_kiroclaw_mcps_to_global`)

#### `kiroclaw-cron` Notable Parameters

- `cron_add`: accepts optional `silent` (boolean), `script` (Python callable path), `command` (shell command — mutually exclusive with `script`), `timeout` (seconds), `strict_schedule` (boolean, disables jitter), `timezone` (IANA name), `skip_dates` (list of YYYY-MM-DD strings). When `silent=true`, results are not auto-delivered — the agent decides when to notify via `send_message`. When `script` or `command` is set, the job bypasses the LLM entirely (deterministic execution).
- `cron_update`: accepts `agent_id`, `timezone`, `skip_dates`, `strict_schedule` in addition to existing fields.
...
#### `kiroclaw-core`: `send_message` Tool
...
- Agent name (purple) for custom agents, hidden for kiroclaw
...
- **Standalone fallbacks**: Memory and Skills APIs work even without `ContextBuilder` — handlers create standalone `MemoryStore` and `SkillsLoader` instances pointing to `~/.kiroclaw/` defaults
...
`kiroclaw setup` uses `kiroclaw.localhost` (RFC 6761 reserved, resolves to loopback natively) for `http://kiroclaw.localhost:7777` (macOS/Linux via `sudo tee -a /etc/hosts`). Gateway startup also prints the hostname URL for remote desktop access.

...
- **Surfaces**: MCP tool `knowledge_dedup` (`apply` bool, default false = dry-run preview; schema `KNOWLEDGE_DEDUP_SCHEMA`) and CLI `kiroclaw knowledge dedup [--apply]` (dry-run by default). Both emit SEL audit events and redact credentials/exfiltration URLs in their output.

...
- **`LoopStallWatchdog`** — armed only when `faulthandler.is_enabled()` (the real `gateway` entrypoint; not `chat`/`tui`). The async heartbeat (`dashboard/server.py`, 5s interval) `beat()`s it each tick, re-arming a C-level `dump_traceback_later(exit=True)` timer that dumps all thread stacks and `_exit()`s if the loop goes silent. Desktop launches default to 25s, close to Electron's independent liveness window. Managed systemd/launchd gateways translate that unchanged default to 90s because they have no Electron probe and WSL, VM, Docker, or heavy disk pressure can suspend scheduling long enough to make 25s a false death; any non-default `dashboard.loop_stall_exit_after_secs` value is preserved. The managed path still emits a non-fatal all-thread dump at `stall_after=30s`, then exits at its service budget if the loop has not recovered. `KIROCREW_SERVICE_MANAGED=1` marks newly generated services; systemd's `INVOCATION_ID` recognizes older installed units after a package-only upgrade.

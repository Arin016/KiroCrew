...
The allowlist is curated for read-only / observation tools — workspace search, internal page reads, CRUX / ticketing / pipeline / Apollo / on-call read APIs, knowledge base reads, and side-effect-free KiroClaw-core tools (`learn_list`, `cron_list`, `spawn_list`, `recall`, etc.). Write tools (`send_message`, `file_send`, `cron_add`, `Edit`, `Write`, shell `execute`/`run`) are not in the list and are rejected.

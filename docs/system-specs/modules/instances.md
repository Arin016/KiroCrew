...
**Nor is the chat proxy.** It forwards a peer reply body verbatim, so a peer
route that mints a token would make it a third — which is precisely why its path
policy is an allowlist (§6): the reachable surface is `api/chat`, and the peer's
`api/token/local`, `api/apps/{name}/token` and `api/webhooks/tokens` are
unreachable by construction rather than by a rule someone must remember to
extend. Widening `_PROXY_ALLOWED_PREFIXES` to a surface that mints or returns a
credential would break this invariant, and is the one review question a new
prefix has to answer.

...
| `backend-test` | 4 duration-balanced pytest-split shards (4 jobs) on Python 3.12, `-n auto` within each. Coverage on every shard; the only `--no-cov` path left is the reduced (frontend-only diff) scope, whose subset coverage is not comparable to the repo floor |

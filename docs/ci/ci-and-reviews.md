...
Eight of the nine upsert sites now let exactly one kind of run claim that
slot — a **completed verdict for the PR's current head**. The two other kinds
each lose a live verdict, so each leaves an existing comment untouched (#8344):
...
comment is recoverable, an unposted verdict is not). The same-repo GPT lane carries
#8342's own inline preserve-and-prepend shape in `codex-review.yml`, pinned
byte-for-byte by its own tests and unchanged here — it is the one remaining
site with a read-modify-write window on the slot. The other eight define the
guard as a `guarded_comment_upsert` bash function that
